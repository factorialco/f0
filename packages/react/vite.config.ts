  ...
  extraPlugins.push(
    dts({
      ...
      afterBuild: () => {
        ...
        // These paths like '../lib/data-testid' don't exist in the flat dist/ structure
        // Replace them with self-references to the bundle which works correctly
        const dtsFiles = ["f0.d.ts", "experimental.d.ts"];
        for (const file of dtsFiles) {
          const filePath = resolve(__dirname, `dist/${file}`);
          if (existsSync(filePath)) {
            let content = readFileSync(filePath, "utf-8");
            // Match patterns like '../lib/data-testid', '../../lib/data-testid', etc.
            // and replace with self-reference to the bundle entry
            const brokenImportPattern =
              /from ['"](?:\.\.\/)+lib\/data-testid(?:\/index\.tsx)?['"]/g;
            const bundleName = file.replace(".d.ts", "");
            const replacement = `from './${bundleName}'`;
            ...
            if (newContent !== content) {
              ...
              consola.success(`Fixed broken imports in ${file}`);
            }
          }
        }
      },
