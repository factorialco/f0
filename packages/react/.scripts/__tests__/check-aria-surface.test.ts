...
describe("diffStory", () => {
  ...
  it("pairs a role change with the name held constant", () => {
    // button -> link keeps the name, so reporting only the removal would read
    // as "the element vanished" when it actually moved.
    const d = diffStory(
      story("a", { 'button "Save"': 1 }),
      story("a", { 'link "Save"': 1 })
    )
    expect(d?.renamed).toEqual([
      { kind: "role", before: 'button "Save"', after: 'link "Save"' },
    ])
    expect(d?.removed).toEqual([])
    expect(d?.added).toEqual([])
  })

  it("does not pair two unnamed nodes just because both lack a name", () => {
    const d = diffStory(story("a", { button: 1 }), story("a", { textbox: 1 }))
    expect(d?.renamed).toEqual([])
    expect(d?.removed).toEqual([{ key: "button", before: 1, after: 0 }])
    expect(d?.added).toEqual([{ key: "textbox", before: 0, after: 1 }])
  })

  it("prefers the same-role pairing when both readings are possible", () => {
    const d = diffStory(
      story("a", { 'button "Open"': 1 }),
      story("a", { 'button "Launch"': 1, 'link "Open"': 1 })
    )
    expect(d?.renamed).toEqual([
      { kind: "name", before: 'button "Open"', after: 'button "Launch"' },
    ])
    expect(d?.added).toEqual([{ key: 'link "Open"', before: 0, after: 1 }])
  })
...
describe("buildMarkdown", () => {
  ...
  it("labels a role change distinctly from a rename", () => {
    const md = buildMarkdown(
      {
        ...empty,
        changed: [
    ...
    expect(md).toContain("🔀 role changed")
    expect(md).toContain('`button "Open"` | `link "Open"`')
  })
})
