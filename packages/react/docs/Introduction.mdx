...
F0 is built and maintained by the Foundations team **with** contributions from every product team. Every component follows the same lifecycle (proposal → design → build → real use → promote → eventually deprecated).

- New here? Start with [How to contribute](/?path=/docs/lifecycle-how-to-contribute--docs).
- Building a component? See the [Definition of Done](/?path=/docs/lifecycle-definition-of-done--docs) and the [Component Maturity model](/?path=/docs/lifecycle-components-maturity--docs).
- Promoting a component to stable? Use the [`f0-component-promotion`](https://github.com/factorialco/factorial-one/blob/main/packages/react/.skills/f0-component-promotion/SKILL.md) skill.
- Deprecating something? See the [Deprecation & Removal policy](/?path=/docs/lifecycle-release-and-versioning--docs).
