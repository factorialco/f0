...
// Generated by `pnpm --filter @factorialco/f0-components-dashboard` gen-dod-debt:
// stable components with open Definition-of-Done gaps, keyed by story id.
import dodDebt from "./dod-debt.json"

type DodDebtEntry = { name: string; gaps: string[] }
...
export const DocsContainer: FC<PropsWithChildren<DocsContainerProps>> = (
  ...
  // Stable-but-with-DoD-debt: show an info banner with the open gaps WITHOUT
  // changing the maturity tag (honest transparency, no breaking demotion).
  const debtKey = storyId ? storyId.split("--")[0] : null
  const debt: DodDebtEntry | null =
    debtKey && !experimental
      ? ((dodDebt as Record<string, DodDebtEntry>)[debtKey] ?? null)
      : null
      ...
      <I18nProvider translations={storybookTranslations}>
        ...
        {debt && (
          <div className="sb-unstyled mb-8">
            <F0Alert
              variant="warning"
              title="Stable — with Definition of Done debt"
              description={`Tagged stable, but these DoD gaps are still open: ${debt.gaps.join("; ")}.`}
              link={{
                href: "/?path=/docs/components-maturity--documentation",
                label: "Component Maturity Model",
              }}
