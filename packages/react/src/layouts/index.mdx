...
- The layout grows to fit its content and declares no `overflow` of its own, so
  the page container around it does the scrolling. That keeps a
  `position: sticky` child — a section rail, a table header — pinned against the
  container that really scrolls.
