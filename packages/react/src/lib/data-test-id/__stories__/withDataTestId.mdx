...
`withDataTestId` is a Higher-Order Component (HOC) utility that wraps React components to automatically add `data-test-id` attributes. This allows consumers to set test IDs at runtime, which is particularly useful for end-to-end testing.
...
```html
<div data-test-id="my-component-123">Content</div>
```
...
| :--- | :--- | :--- |
| `dataTestId` | `string` | The value for the `data-test-id` attribute. |
...
- **Memoization**: Works correctly with `React.memo` components.
