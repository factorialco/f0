  ...
  it("generates dimensions from the shared scale", async () => {
    const { root } = await postcss([
      tailwindcss({
        ...
        content: [
          {
            raw: "p-1.25 p-2.25 p-35 p-70 p-75 p-90 p-100 p-140 p-160 p-180 w-1.25 w-2.25 w-35 w-70 w-75 w-90 w-100 w-140 w-160 w-180",
            extension: "html",
          },
        ],
      }),
    ...
    expect(declarations.get(".p-160:padding")).toBe("640px")
    expect(declarations.get(".p-180:padding")).toBe("720px")
    ...
    expect(declarations.get(".w-160:width")).toBe("40rem")
    expect(declarations.get(".w-180:width")).toBe("45rem")
  })
})
