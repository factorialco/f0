...
import { DialogInternal } from "@/components/F0Dialog/internal/F0DialogInternal";
...
const materializeActionValue = async (
  ...
  return Promise.resolve(typeof action.value === "function" ? await action.value() : action.value);
};
...
export const Dialogs = ({ dialogs }: DialogsProps) => {
  ...
  const f0Dialogs = useMemo(() => {
     ...
     */
    const toF0Action = (dialog: DialogDefinitionInternal, action: DialogAction): F0DialogAction => {
      return {
        ...action,
        value: nanoid(),
        onClick: async () => {
    ...
    return dialogs.map((dialog) => ({
      ...dialog,
      actions: {
        primary: toArray(dialog.actions.primary).map((action) => toF0Action(dialog, action)),
        secondary: toArray(dialog.actions.secondary).map((action) => toF0Action(dialog, action)),
      },
    }));
  }, [dialogs]);
  ...
  const f0DialogsWithBlocks = useMemo(() => {
     ...
     */
    const toF0ActionWithBlocks = (dialogId: DialogId, action: F0DialogAction): F0DialogAction => {
      return {
        ...action,
    ...
    return f0Dialogs.map((dialog) => ({
      ...dialog,
      actions: {
        primary: dialog.actions.primary.map((action) => toF0ActionWithBlocks(dialog.id, action)),
