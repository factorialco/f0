...
```tsx
import { dialog } from "@factorialco/f0-react"
import { F0Button } from "@/components/F0Button"
...
function MyComponent() {
  ...
  const showAlert = async () => {
    await dialog.alert({ title: "Alert Title", msg: "This is an alert message" })
...
function MyComponent() {
  ...
  const handleSave = async () => {
    const result = await dialog.confirm("Save Changes", "Do you want to save?", {
      confirm: {
        value: async () => {
          // Simulate API call
