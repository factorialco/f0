...
<Meta title="Primitives/F0InputField" />

# F0InputField

`F0InputField` is a wrapper component that enhances native HTML `<input>` or
...
Use `F0InputField` whenever you need to capture text input from a user. It's
...
To use `F0InputField`, you need to pass an `<input>` or `<textarea>` element as
`children`. `F0InputField` will then clone this element and add the necessary
props to it.
...
`F0InputField` accepts a variety of props to customize its behavior and
appearance.
...
The `F0InputField` is designed to be composed with other components. The
...
props. The `F0InputField` will automatically provide them.
