...
// Non-PDF panes are heavy in their own way (SheetJS, docx-preview, the remark
// pipeline) — each lives in its own chunk, fetched the first time that kind
// renders.
...
const PdfViewerBase = forwardRef<
  HTMLDivElement,
  Omit<F0PdfViewerProps, "kind" | "mimeType">
>((props, ref) => {
  const {
    url,
  ...
  const file = useMemo(() => ({ url, withCredentials }), [url, withCredentials])

  const goToPage = useCallback(
    (targetPageIndex: number) => {
      const lastPageIndex = (totalPages ?? 1) - 1
      const destination = Math.min(Math.max(targetPageIndex, 0), lastPageIndex)
...
PdfViewerBase.displayName = "PdfViewerBase"
