...
// Name pools drawn from the canonical fake roster, so this large dataset only
// ever uses our fake cast's names (combinatorially, for variety/volume).
const FIRST_NAMES = FAKE_PEOPLE.map((person) => person.firstName)

const LAST_NAMES = FAKE_PEOPLE.map((person) => person.lastName)
