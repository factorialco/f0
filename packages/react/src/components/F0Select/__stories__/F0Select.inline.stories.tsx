...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "The detail-row presentation of F0Select. At rest the selection reads as plain text — avatar and icon included — with no chevron, no button and no hover background; the dropdown replaces it only while `editing` is true. `editing` is controlled and the component never changes it: it reports `commit`, `escape` and `popupClose` through `onDismiss` and keeps the dropdown up until the owner says otherwise. Single selection calls `onChange` before `onDismiss`, so the owner can unmount the editor on commit without losing the selected value. Prop and option refreshes do not emit `onChange`. Both presentations fill the box the row declares and start their text at the same inset, so the value does not move when the row is activated.",
      },
    },
  },
