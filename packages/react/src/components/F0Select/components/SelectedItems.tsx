 ...
 */
export const SelectedItems = forwardRef<HTMLDivElement, SelectValueProps>(function SelectValue(
    ...
    if (allSelected === "indeterminate") {
      ...
      return (
        <div className="flex w-full items-center gap-1 text-left">
          ...
          {remainingCount > 0 && <ItemsCounter count={remainingCount} items={remainingItems} />}
        </div>
      )
    }
  ...
  if (!selectedItem && totalSelectedCount && totalSelectedCount > 0) {
    return (
      <div className="flex min-w-0 flex-1 justify-start gap-1.5" ref={ref}>
        <OneEllipsis tag="span" className="text-left text-f1-foreground-secondary">
          ...
        </OneEllipsis>
      </div>
    )
