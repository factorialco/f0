...
describe("Select", () => {
  ...
  it("throws in development when multiple status options share the same variant", () => {
    expect(() =>
      render(
        <F0Select
          {...defaultSelectProps}
    ...
    ).toThrow(/Only one option per status variant is allowed/)
  })

  it("filters duplicate status variants in production", async () => {
    ...
    try {
      ...
      await openSelect(user)
      expect(screen.queryByText("Approved 1")).toBeInTheDocument()
