...
<DoDonts
  do={{
    description:
      "Pick `as` from the page structure and `variant` from the size you need, so the outline stays correct whatever the heading looks like.",
    guidelines: [
      "One h1 per page, then h2 for its sections, h3 inside those",
      "Never skip a level — an h2 followed by an h4 breaks screen-reader navigation",
    ],
    children: <F0Heading as="h1" variant="heading-large" content="Time off" />,
  }}
  dont={{
    description: "Don't choose the heading level for its font size.",
    guidelines: [
      "Jumping to a deeper level to get smaller text corrupts the outline",
      'For a field or section label use F0Text variant="label", not a heading',
    ],
    children: <F0Heading as="h4" variant="heading" content="Time off" />,
  }}
/>
