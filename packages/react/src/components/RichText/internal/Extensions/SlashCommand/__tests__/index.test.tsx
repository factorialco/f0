...
// editor's `EditorContent`, never by its own React root: a standalone root is
// invisible to the React tree above the editor, so an enclosing dialog's Radix
// `DismissableLayer` reads the focus the popover takes on mousedown as an
...
/**
 * Reaches the suggestion lifecycle without mounting an editor: tiptap keeps the
 * whole config object on the extension, so `addProseMirrorPlugins` can be
 * called with a stub `this` and the captured `Suggestion` config gives us
 * `render()`.
 */
const startPopover = () => {
