...
const CommandList = forwardRef<CommandListHandle, CommandListProps>(
  ({ items, groups, command }, ref) => {
    ...
    const commandsToRender = useMemo(
      () => groups || [{ title: "", commands: items }],
      [groups, items]
    )
    const allCommands = useMemo(
      () => commandsToRender.flatMap((group) => group.commands),
      [commandsToRender]
    )
    // Offset of each group's first command within allCommands
    const groupOffsets = useMemo(() => {
      const offsets: number[] = []
      let offset = 0
      for (const group of commandsToRender) {
        offsets.push(offset)
    ...
    // Global index of a command within all commands
    const getGlobalIndex = (groupIndex: number, commandIndex: number) =>
      groupOffsets[groupIndex] + commandIndex
