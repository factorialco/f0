...
// Guards the fix: a standalone root is invisible to the React tree above the
// editor, so a dialog's `DismissableLayer` treats the focus the popover takes
// on mousedown as an outside interaction and closes itself.
...
// The popover is rendered by a second `ReactRenderer` (portaled from the
// editor's `EditorContent`) rather than by its own `createRoot`, so the mock
// records every renderer that gets constructed. `renderers[0]` is the mention
// list, `renderers[1]` the popover around it.
...
describe("createSuggestionConfig", () => {
  ...
  const startPopover = () => {
    ...
    // The popover's DOM still lives on `document.body`, unchanged by the move
    // off a standalone React root.
  ...
  it("tears down only once when Escape is followed by the exit callback", () => {
    ...
    // tiptap still calls `onExit` after an Escape that returned `true`.
