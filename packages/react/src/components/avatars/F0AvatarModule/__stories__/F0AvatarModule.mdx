import { Canvas, Meta, Controls, Unstyled } from "@storybook/addon-docs/blocks"
import * as ModuleStories from "./F0AvatarModule.stories"
import { DoDonts } from "@/lib/storybook-utils/do-donts"

import { F0AvatarModule } from "../index"

<Meta of={ModuleStories} />

# AvatarModule

A compact avatar that represents a **product module** (Time Tracking, Goals,
Kudos…) as a colored squircle with the module's icon.

---

## Overview

### Purpose

- Identify which product module an item, action or surface belongs to
- Give modules a consistent, recognizable visual across the product
- Serve as the module **badge** layered on the corner of other avatars

### Anatomy

A module avatar is a squircle filled with the module's brand gradient and its
icon centered on top. It is used both **standalone** (cards, breadcrumbs,
upselling surfaces) and as the **module badge** rendered automatically in the
corner of entity avatars.

<Canvas of={ModuleStories.Default} />

<Controls of={ModuleStories.Default} />

### Variants

#### Modules

Each module maps to its own icon.

<Canvas of={ModuleStories.Snapshot} />

#### Sizes

Module uses the shared avatar size scale for standalone use (`xs`–`lg`, same
pixels as every other avatar). It also defines three smaller sub-sizes (`2xs`,
`3xs`, `4xs`) reserved for the corner badge described below.

<Unstyled>
  <table className="mb-8 w-full dark:text-f1-foreground-inverse/80">
    <thead>
      <tr>
        <th className="text-left">Size</th>
        <th className="text-left">Dimensions</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>
          <code>lg</code>
        </td>
        <td>40px</td>
      </tr>
      <tr>
        <td>
          <code>md</code>
        </td>
        <td>32px</td>
      </tr>
      <tr>
        <td>
          <code>sm</code> (default)
        </td>
        <td>24px</td>
      </tr>
      <tr>
        <td>
          <code>xs</code>
        </td>
        <td>20px</td>
      </tr>
      <tr>
        <td>
          <code>2xs</code> · badge
        </td>
        <td>16px</td>
      </tr>
      <tr>
        <td>
          <code>3xs</code> · badge
        </td>
        <td>12px</td>
      </tr>
      <tr>
        <td>
          <code>4xs</code> · badge
        </td>
        <td>10px</td>
      </tr>
    </tbody>
  </table>
</Unstyled>

The `2xs`, `3xs` and `4xs` sub-sizes exist **only** so the module can render as
a small badge in the corner of an entity avatar — marking which product module
the item belongs to. As a badge the module is sized to roughly **half** its host
avatar (`lg` 40px → `xs` 20px, `md` 32px → `2xs` 16px, `sm` 24px → `3xs` 12px,
`xs` 20px → `4xs` 10px). These sub-sizes are too small to read on their own, so
for a standalone module avatar use `xs` (20px) or larger.

<Canvas of={ModuleStories.AsBadge} />

The badge sub-size follows its host avatar, so you never set it by hand:

<Canvas of={ModuleStories.AsBadgeSizes} />

---

## Guidelines

### Design best practices

#### When to use

- Showing the product module behind a card, breadcrumb or upsell surface
- Marking the owning module on top of an entity avatar (as a badge)

#### When NOT to use

- **`2xs` / `3xs` / `4xs` for a standalone module avatar**: these sub-sizes
  exist only so the module reads as a tiny corner badge on another avatar. On
  their own they are too small to be legible — use `xs` (20px) or larger for
  standalone.
- Representing a person, team or other entity: use the matching `F0Avatar*`
  component.

<DoDonts
  do={{
    description:
      "Use xs (20px) or larger for a standalone module avatar, so the icon stays legible",
    children: <F0AvatarModule module="home" size="xs" />,
  }}
  dont={{
    description:
      "Don't use the badge sub-sizes (2xs, 3xs, 4xs) on their own — they only read as a corner badge on another avatar",
    children: <F0AvatarModule module="home" size="3xs" />,
  }}
/>

#### Adding a new module icon

Module icons are a curated set. Add one only when a real product module needs
it, and keep the set consistent:

- **No duplicates.** Every module must have its own distinct icon. Never reuse
  an icon that already represents another module, and never add a second copy of
  an icon that is already in the set — one concept, one icon.
- **Full colour, filled style.** Module icons are intentionally filled and
  render in white on the module's colour squircle, unlike the mostly-outline
  icon system. A new icon must be drawn in this filled style so it reads
  consistently with the rest of the set.
- **Factorial icon system only.** Match the style of Factorial's existing
  icons. Don't import icons from external libraries (e.g. Lucide) or legacy
  sources (e.g. Gamma) — they won't match in style.
- **Source vs. usage.** The raw module SVGs live in the module icon set; in the
  UI always render them through `F0AvatarModule`, never the raw SVG.

---

## Accessibility

The module avatar is **always decorative**. It renders `aria-hidden="true"`
unconditionally, so assistive technology never reaches it.

- The module name has to exist as visible text, or on the labelled element the
  avatar sits inside. The squircle alone conveys nothing to a screen reader.
- As a corner badge this needs care: the host entity avatar is itself
  `aria-hidden` unless the consumer passes `aria-label`, so by default neither
  the badge nor the avatar it sits on has a name. The surrounding row has to
  carry both.
- The `aria-label` and `aria-labelledby` props are currently **inert**: they land
  on the same element as the hardcoded `aria-hidden`, which wins. Don't rely on
  them to name anything.
