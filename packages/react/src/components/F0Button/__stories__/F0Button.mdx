...
When the visible label does not fit, the button truncates it and uses the full label as an automatic tooltip. Set `noAutoTooltip` only when surrounding context already communicates the complete action.
