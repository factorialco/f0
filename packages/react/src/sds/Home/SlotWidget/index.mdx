...
| `describeOnHover`     | `true`                                                                                                         | rows keep their second line AND offer it on hover — for descriptions that truncate  |
...
### The second line on hover

`describeOnHover: true` gives the rows a tooltip carrying their description
**while still drawing it**. The second line is one truncating line — the table
above puts three facts at the edge of what fits — so a row with more to say than
fits ends in an ellipsis with nowhere to read the rest. This is that somewhere,
and unlike `compact` it costs the line nothing: the parts stay drawn, and the
critical one stays red.

Only for lists whose second line carries facts a reader acts on (an amount, a
due date, the project a task belongs to). A list whose descriptions always fit
gains nothing from it — a tooltip that repeats what is already legible is noise.
Under `compact` it adds nothing either: the line has folded into that same
tooltip already.

<Canvas of={SlotWidgetStories.DescribeOnHover} />
