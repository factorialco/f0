 ...
 */
function downloadAsExcel(dataset: DataDownloadDataset, filename: string): void {
 ...
 */
function downloadAsCsv(dataset: DataDownloadDataset, filename: string): void {
  const { columns, rows, columnLabels } = dataset
  const headers = columns.map((col) => columnLabels?.[col] ?? col)
  ...
  const handleDownload = useCallback(
    (format: string, _item: unknown) => {
      if (format === "excel") {
        ...
        console.warn(`[DataDownload] Unknown download format: "${format}"`)
      }
    },
    [dataset, filename]
  )
