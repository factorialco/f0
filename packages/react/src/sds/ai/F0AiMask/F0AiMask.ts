...
export class F0AiMask {
  ...
  private options: Required<
    Pick<MaskOptions, "ratio" | "borderWidth" | "glowWidth" | "borderRadius">
  > &
    Omit<MaskOptions, "ratio" | "borderWidth" | "glowWidth" | "borderRadius">
  ...
  private checkGLError(gl: WebGL2RenderingContext, context: string): void {
    ...
    if (error !== gl.NO_ERROR) {
      console.error(`WebGL Error in ${context}`)
