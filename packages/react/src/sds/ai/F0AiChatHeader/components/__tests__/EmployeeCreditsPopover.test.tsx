...
describe("EmployeeCreditsPopover", () => {
  ...
  it("renders the trigger when employeeCredits is provided", () => {
    render(<EmployeeCreditsPopover employeeCredits={makeEmployeeCredits()} />)
  ...
  it("fetches usage and displays remaining credits when opened", async () => {
    ...
    render(<EmployeeCreditsPopover employeeCredits={{ fetchUsage }} />)
    await openPopover()
  ...
  it("renders the company header when companyName is provided", async () => {
    render(
      <EmployeeCreditsPopover
        employeeCredits={makeEmployeeCredits({
          companyName: "Acme",
          planName: "Free",
        })}
      />
  ...
  it("clamps the remaining count at 0 when used exceeds total", async () => {
    render(
      <EmployeeCreditsPopover
        employeeCredits={makeEmployeeCredits({}, { used: 300, total: 250 })}
      />
    )
    await openPopover()
