  ...
  <table className="dark:text-f1-foreground-inverse/80 mb-8 w-full">
    ...
    <tbody>
      <tr>
        ...
        <td>—</td>
        <td>
          Stable accent shared by the sender name and incoming bubble. For a
          photo avatar, choose the palette hue closest to the image&apos;s
...
- WHEN a user provides <code>avatarColor</code> → THEN incoming bubbles and the
  sender name share that stable accent; when omitted, generated person avatars
  use their existing name-derived colour. Own-message bubbles remain neutral.
...
- WHEN the transcript is actively scrolling → THEN heavy map, document, video, and voice previews keep stable placeholders instead of initializing work that could interrupt the scroll.
- WHEN scrolling ends → THEN deferred heavy previews initialize progressively across idle frames while already-mounted previews remain unchanged.
