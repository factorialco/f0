...
import { ChatTypingChip } from "./ChatTypingChip"
...
export const ChatComposer = (): ReactNode => {
  ...
  const [attachments, setAttachments] = useState<F0ChatAttachment[]>([])
  ...
  const canSend =
    (value.trim().length > 0 || attachments.length > 0) && !isTranscribing
  ...
  return (
    <div className="shrink-0 p-3 pt-1">
      ...
      <div className="mx-auto w-full max-w-content">
        <div className="rounded-xl border border-solid border-f1-border-secondary bg-f1-background">
          {replyTo && (
            <ChatReplyChip
              message={replyTo}
          ...
          {/* "Writing…" sits above the textarea, reply-chip style. */}
          <ChatTypingChip />

          {/* Pending attachments — F0FileItem chips with a remove action,
              same as the AI chat composer. */}
              ...
              {attachments.map((att, i) => (
                <F0FileItem
                  ...
                  file={{
                    name: att.name,
                    type:
                      att.mimeType ?? (att.kind === "image" ? "image/png" : ""),
                  }}
                  actions={[
                    {
