...
const Tags = ({ tags, right }: { tags: Tag[]; right?: boolean }) => (
  <div className={cn("flex flex-1 flex-row items-center gap-1.5", right && "justify-end")}>
    {tags.map((tag) => {
      const Content = (
        <div>
          <F0TagRaw icon={tag.icon} additionalAccesibleText={`${tag.label}: ${tag.description}`} />
        </div>
      )
...
export const CalendarEvent = forwardRef<HTMLDivElement, CalendarEventProps>(function CalendarEvent(
  {
    label,
    title,
    subtitle,
      ...
      <div className="z-10 flex flex-1 flex-col gap-2">
        <div className="flex flex-row items-start gap-2.5">
          <div className="flex flex-1 flex-col gap-0.5">
            {!!label && (
              <p className="line-clamp-1 text-sm text-f1-foreground-secondary">{label}</p>
            )}
          ...
          <div className="flex flex-row items-center">
            {fromDate && (
              <>
                ...
                <F0Icon icon={ChevronRight} size="sm" className="text-f1-foreground-tertiary" />
              </>
            )}
