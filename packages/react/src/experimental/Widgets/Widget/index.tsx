...
const Container = forwardRef<HTMLDivElement, WidgetProps & { children: ReactNode }>(
  function Container(
    { header, children, action, summaries, alert, status, fullHeight = false },
    ref
  ) {
    const { enabled: privacyModeEnabled, toggle: togglePrivacyMode } = usePrivacyMode()
    ...
    return (
      <Card
        className={cn(fullHeight ? "h-full" : "", "relative flex gap-3 border-f1-border-secondary")}
        style={{
            ...
            <div className="flex w-full flex-1 flex-col gap-4">
              <div className="flex flex-1 flex-row flex-nowrap items-center justify-between gap-2">
                <div className="flex min-h-6 grow flex-row items-center gap-1 truncate">
                  {header.title && <CardTitle className="truncate">{header.title}</CardTitle>}
                  {header.subtitle && (
                    <div className="flex flex-row items-center gap-1">
                      <InlineDot />
                      <CardSubtitle className="truncate">{header.subtitle}</CardSubtitle>
                    </div>
                  )}
                  {header.info && (
            ...
            <div className="flex flex-row">
              {summaries.map((summary, index) => (
                <div key={index} className="grow">
                  <div className="mb-0.5 text-sm text-f1-foreground-secondary">{summary.label}</div>
                  <div className="flex flex-row items-end gap-0.5 text-2xl font-semibold">
                    {!!summary.prefixUnit && (
                      <div className="text-lg font-medium">{summary.prefixUnit}</div>
                    )}
                    {summary.value}
                    {!!summary.postfixUnit && (
                      <div className="text-lg font-medium">{summary.postfixUnit}</div>
                    )}
                  </div>
                </div>
              ))}
...
const Skeleton = forwardRef<HTMLDivElement, WidgetSkeletonProps>(function Skeleton(
  { header, height },
  ref
) {
  return (
    <Card
      className={cn("flex gap-4 border-f1-border-secondary", height === "full" && "h-full")}
      ref={ref}
      aria-live="polite"
      aria-busy={true}
    >
      <CardHeader className="-mr-1 -mt-1">
        <div className="flex h-6 w-full flex-row items-center gap-1.5" aria-hidden={true}>
          {header?.title ? (
