...
- **A dialog takes the screen back.** Opening a dialog while a coachmark is up stands the coachmark down — the panel and the dim both — for as long as the dialog is there, and it returns on the same step once the dialog closes. It is not dismissed and `onEnd` hears nothing, because the reader ended nothing. Focus is left alone while it is stood down — a dialog that is not modal closes itself when focus leaves it — and the step is announced again when it comes back. A popover, a select or a dropdown opening inside the step being explained is not a dialog and changes nothing.
...
Pass `steps` and the coachmark sequences itself: a row of dots in the centre of the footer tracks where the reader is, the action on the right reads `Next` until the last step, a `Back` button appears on the left from the second step on, and `onComplete` fires when the user reaches the end. Each step can point at a different element.
...
From the second step on there is somewhere to go back to, so a `Back` button appears at the bottom left. The dots keep the panel's centre line whether or not it is there:

<Canvas of={Stories.WithBack} />
