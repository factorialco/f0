...
function TOCContent({
  ...
  // Handle drag over from Item component with stabilized debounce
  const handleDragOver = useCallback(
    (itemId: string, position: "before" | "after" | "inside") => {
      // CRITICAL: Cancel any pending handleDragLeave timeout
      // This ensures that handleDragLeave doesn't clear state while we're still dragging over
      if (dragOverTimeoutRef.current) {
        clearTimeout(dragOverTimeoutRef.current)
        dragOverTimeoutRef.current = null
      }

      // Removed verbose handleDragOver logs - too noisy
      ...
      // Check if this is the same as the current React state
      const currentStateKey =
        dragOverItemId && dragOverPosition
          ? `${dragOverItemId}-${dragOverPosition}`
          : null

      // Check if this is the same as the last pending update
      const lastPendingKey = lastDragOverRef.current
        ? `${lastDragOverRef.current.itemId}-${lastDragOverRef.current.position}`
        : null

      // Skip only if this matches the current React state
      // If current state is different, we should update even if there's a pending update
      if (dragOverKey === currentStateKey) {
        // Skipping (same as current state)
        // Still update the call time to keep state alive
        lastDragOverCallTimeRef.current = Date.now()
        return
      }

      // If current state is null and we have a pending update with the same key,
      // apply it immediately instead of waiting for the timeout
      // This prevents the state from being stuck in a pending state
      if (currentStateKey === null && dragOverKey === lastPendingKey) {
        // Clear the timeout since we're applying immediately
        if (dragOverTimeoutRef.current) {
          clearTimeout(dragOverTimeoutRef.current)
          dragOverTimeoutRef.current = null
        }
        ...
        if (itemId === firstItem?.id && position === "before") {
      ...
      // Reduce debounce significantly - onDrag is already throttled in Item component
      // We need to respond quickly to keep placeholder visible
      // Use shorter debounce when moving up to prevent glitchy behavior
      const debounceDelay = isMovingUp ? 50 : 30

      // Add persistence: if current state was set recently, wait a bit longer
      // This prevents rapid flickering when moving quickly between items
      const timeSinceLastState = Date.now() - currentStateSetTimeRef.current
      const persistenceDelay = isMovingUp && timeSinceLastState < 100 ? 30 : 0
      const finalDebounceDelay = debounceDelay + persistenceDelay
      ...
      // Also update the call time immediately to track active dragging
      lastDragOverCallTimeRef.current = Date.now()
    },
    [
      dragOverItemId,
  ...
  const handleDrop = useCallback(
    (targetItemId: string, position: "before" | "after" | "inside") => {
      ...
      // Cancel any pending drag leave timeout
      if (dragOverTimeoutRef.current) {
        clearTimeout(dragOverTimeoutRef.current)
        dragOverTimeoutRef.current = null
      }
