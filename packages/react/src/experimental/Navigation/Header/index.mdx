...
- **In-dropdown filters** (`showFilters`): opt in to render the source's
  filter definitions as an editable picker inside the dropdown, pre-applied
  with the seeded persisted filters, so users can refine the jump-to list in
  place. Off by default — the seeded filters are still applied, just not
  editable. Pair it with `onFiltersChange` → the navigation hook's
  `currentFilters` so the header arrows + counter follow the same refined
  context after the user jumps to an item from it.
...
The same wiring extends to the header's prev/next arrows: feed
`useDataCollectionItemNavigation` (from `OneDataCollection`) the same
`source` + `collectionId` and pass its render-ready `navigation` result to
PageHeader's `navigation` prop — arrows and the `current/total` counter then
walk the same filtered, sorted set the breadcrumb select shows, navigating
through each item's `itemUrl`. When the adapter also implements the
id-relative `fetchItemNeighbors` capability, items beyond the loaded page —
including deep direct links — resolve their neighbors backend-side, so the
