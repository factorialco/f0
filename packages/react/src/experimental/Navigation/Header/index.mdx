...
Mark an action with `kind: "ai"` to render it as the branded `AIButton`. Use an
icon from `@/icons/ai` (`ChartLine` for analytics/dashboard launchers, `One` for
generic "open AI chat" entry points). The tooltip is auto-derived from `label`.
