...
But you can also disable persistence by passing `storage={false}`; the component will still use the ID for other purposes (like column configuration keys), but won't persist state to storage.
