...
type TableVisualizationOptions<T> = {
   ...
   */
  columns:
    | ReadonlyArray<TableColumnDefinition<T>>
    | ((data: T[]) => ReadonlyArray<TableColumnDefinition<T>>)
...
You can also define the columns dynamically based on the data:
...
<Canvas of={TableStories.TableDynamicColumns} meta={TableStories} />
