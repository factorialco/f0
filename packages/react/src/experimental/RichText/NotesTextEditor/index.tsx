...
import "./index.css"
import { actionType, MetadataItemValue, NotesTextEditorHandle, secondaryActionsType } from "./types"
...
const NotesTextEditorComponent = forwardRef<NotesTextEditorHandle, NotesTextEditorProps>(
  function NotesTextEditor(
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertAIBlock: () => {
        ...
        const cfg = aiBlockLabels ? { ...aiBlockConfig, labels: aiBlockLabels } : undefined
        editor
          .chain()
          .focus()
    ...
    return (
      <div className="relative flex h-full w-full flex-col" ref={containerRef} id={editorId}>
        {showHeader && (
          <Header actions={actions} metadata={metadata} secondaryActions={secondaryActions} />
        )}
        {!readonly && (
        ...
        <ScrollArea className="h-full gap-6">
          ...
          <div className="notes-text-editor h-full" onClick={() => editor.commands.focus()}>
            {!readonly && (
              <DragHandle
                editor={editor}
...
export const NotesTextEditorSkeleton = ({
  ...
  return (
    <div className="relative flex h-full w-full flex-col" aria-busy="true" aria-live="polite">
      {withHeader && (
...
export type { NotesTextEditorHandle, NotesTextEditorProps, NotesTextEditorSkeletonProps }
