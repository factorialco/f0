...
The `compareTo` prop is used to compare the selected date range to a predefined date range used
based on the current select value (but it can be an absolute value).

The `compareTo` options are associated with the granularity of the selected date range.

The `compareTo` options are defined as an object with the granularity as the key and the options as
the value.
...
The `defaultCompareTo` prop is used to set the default compare to value or to change the default
compare from outside the component.
