...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, ButtonInternalProps>(
...
const CarouselDots = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const scrollTo = currentSlide * dotWidth - container.clientWidth / 2 + dotWidth / 2
    ...
    const getScale = (index: number) => {
      ...
      if (distance === 2)
        return currentSlide === 0 || currentSlide === numberOfSlides - 1 ? "scale-100" : "scale-75"
      if (distance === 3)
        return currentSlide === 0 || currentSlide === numberOfSlides - 1 ? "scale-75" : "scale-50"
      if (distance >= 4) return "scale-50"
    }

    return (
      <div ref={ref} className={cn("flex justify-center", props.className)}>
        <div className="relative overflow-hidden" style={{ width: `${visibleDotsWidth}px` }}>
          <div
              ...
              >
                <div
                  className={cn(
                    ...
                    i === currentSlide && "rounded-[3px] opacity-100 group-hover/dot:opacity-100",
                    getScale(i)
                  )}
                />
              </button>
