...
describe("isSameDatePickerValue", () => {
  ...
  it("handles edge case with same timestamps", () => {
    const timestamp = new Date(2023, 0, 15, 12, 0, 0).getTime()
    const value1 = createDatePickerValue(
      new Date(timestamp),
      new Date(timestamp + 86400000) // +1 day
    )
    const value2 = createDatePickerValue(
      new Date(timestamp),
      new Date(timestamp + 86400000)
    )
