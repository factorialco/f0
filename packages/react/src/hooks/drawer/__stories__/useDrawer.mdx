...
When `primary` or `secondary` is given an **array** of actions instead of a single one, the footer renders them as a dropdown menu. Each item resolves the `openDrawer` promise with its own `value`, and items can be individually `disabled`.
...
function MyComponent() {
  ...
  return <button onClick={showDrawer}>Open Drawer with Dropdown</button>
}
```

<Canvas of={UseDrawerStories.DrawerWithDropdown} />

### Promise-Based Actions

A `DialogActionValue` can be a primitive value **or** a function that returns a Promise. When a function is provided, the drawer stays open until the promise resolves, the triggering button shows a loading state, and all actions are disabled during the async operation (unless `nonBlocking` is set to `true`). The resolved value becomes the result of `openDrawer`.
...
function MyComponent() {
  ...
  return <button onClick={showDrawer}>Save</button>
}
```

<Canvas of={UseDrawerStories.DrawerWithPromises} />
