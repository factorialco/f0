...
The `useSelectable` hook manages selection state for individual items and groups in a data
collection. It provides comprehensive selection functionality including individual item selection,
group selection with cascading, bulk operations support, and partial selection states.

## Overview

`useSelectable` handles the complex logic of managing selection state in data collections,
including:
...
function SelectableUserList() {
  ...
  const {
    ...
    isAllSelected,
    isPartiallySelected,
  } = useSelectable({ data, paginationInfo, source: dataSource })
...
function UserListWithDefaults() {
  ...
  const { selectedItems, handleSelectItemChange, handleSelectAll } =
    useSelectable({
      ...
      source: dataSource,
      selectionMode: "multi",
      defaultSelectedItems: dataSource.defaultSelectedItems,
    })
