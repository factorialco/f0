...
In addition to row-level `itemActions`, each item can expose declarative
actions rendered immediately next to its title via `itemDefinition.titleActions`.
This is useful for affordances that target the title's underlying resource
(e.g. an "open in new tab" link).

- A single action renders as an icon-only ghost button.
- Multiple actions collapse into an overflow dropdown menu.
...
### List with a single title action

<Canvas of={ListStories.ListVisualizationWithSingleTitleAction} />

### List with multiple title actions (overflow menu)

<Canvas of={ListStories.ListVisualizationWithMultipleTitleActions} />
