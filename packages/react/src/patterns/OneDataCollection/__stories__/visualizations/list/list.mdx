...
type ListVisualizationOptions<T> = {
  ...
  itemDefinition: (item: T) => {
    title: string
    description?: string[]
    // Only the `icon` kind takes a tint; every other kind is unchanged.
    avatar?:
...
which is what keeps a column of them legible. A colour says **which kind of
thing** the row is, not how urgent it is — for "this is urgent", use an alert
avatar instead.

`color` also takes a hex — `#RGB` or `#RRGGBB` — for a colour that is already
data (a calendar's own colour, a module's brand, a category a user picked).
...
Without a `color` the row draws the neutral avatar, unchanged. The tint must
...
<Canvas of={ListStories.ListVisualizationWithTintedIcons} />
