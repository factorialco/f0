...
switcher (a segmented control) in the header. Each segment shows the
visualization's icon only: the label carried too much visual weight for a
secondary header control. The name is still available everywhere it matters —
in the segment's tooltip after a moment's hover, as its accessible name for
screen readers, and as visible text on touch devices, where there is no pointer
to hover with.

Each view's icon is fixed in the built-in registry and is not customizable, so
the same view is recognisable across every collection.

## Renaming a view

By default each built-in visualization uses its localized name — "Table",
"Graph", "List", etc. To rename a view for a specific collection, pass an
...
<OneDataCollection
  source={dataSource}
  visualizations={[
    // Renames the view to "Org chart"; the built-in graph icon is kept.
...
- `label` is optional; when omitted the localized built-in label is used, so
  existing collections keep saying "Table" and "Graph".
- Only the text is customizable — the icon always comes from the visualization
  type. For a fully custom icon and label, use a `type: "custom"` visualization.

In the example below the two views are named "Org chart" and "Directory" instead
of the defaults "Graph" and "Table". Hover a segment in the header to see it:
