...
chunks in the collection but they are not the same as the filters. Opposite to
...
Except for a custom range of dates, user is choosing the minimum date chunk the
granularity represents, but this can be translated into a range of dates
(datetimes).

With this criteria, a week is a range of 7 days (starting on the first day of
the week at 00:00:00 and ending on the last day of the week at 23:59:59), a
month is a range of 30 days (starting on the first day of the month at 00:00:00
and ending on the last day of the month at 23:59:59), etc.
...
- `periods`: Consumer-defined ranges (payroll cycles, academic terms, …). They
  become an extra entry in the granularity list, named by the definition's
  `label`, whose view lists every period instead of a calendar. See
  [Consumer-defined periods](#consumer-defined-periods).
...
### Consumer-defined periods

Some collections navigate by ranges no calendar granularity can express —
payroll cycles that run from the 25th to the 24th, academic terms, fiscal
periods. Pass them as `periods` and they become an extra entry in the
granularity list, named by the definition's `label`. Its view lists every
period with its own label and date range, and the navigation arrows step from
one period to the next rather than by a calendar unit.
...
Supplying `periods` is what makes the entry selectable — listing `"periods"` in
`granularity` only controls where it sits in the list. Each period accepts an
optional `description` to override the generated date range, and `header` is
optional.

A period stays reachable as long as it overlaps `min`/`max`, since
consumer-defined periods rarely line up with those bounds. A value that falls
outside every period keeps its own range, so a persisted or out-of-range value
still renders.

<Canvas
  of={DataCollectionNavigationFiltersStories.WithPeriods}
...
> Remember navigation filters always need a value and cannot be empty by
> definition.
...
>  = {
  valueConverter: (value, filterDef) => {
    ...
    // This is useful if you want to accept a value and convert it to the internal state and providing a default value
  },
