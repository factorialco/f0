...
server polling, multi-step wizards), pass a controlled
`bulkActionStatus: "idle" | "loading" | "success" | "error"` prop. When
controlled, it overrides any internal auto-management.
...
<OneDataCollection
  ...
  onBulkAction={(action) => openConfirmModal({
    onConfirm: async () => {
      setStatus("loading")
      try { await api.run(action); setStatus("success") }
