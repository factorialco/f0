...
>(
  ...
  useEffect(() => {
    if (id === undefined || !registerSelectable || !isPresent) return
    registerSelectable(id, item)
    return () => unregisterSelectable?.(id)
  }, [id, item, registerSelectable, unregisterSelectable, isPresent])
...
const FlatRowComponent = forwardRef(FlatRowComponentInner)
FlatRowComponent.displayName = "FlatRow"

const FlatRow = FlatRowComponent as <
  R extends RecordType,
  Filters extends FiltersDefinition,
  Sortings extends SortingsDefinition,
  Summaries extends SummariesDefinition,
