...
const mock = vi.hoisted(() => {
  ...
  class MockMarker {
    // By reference, not index: a removal shifted every later marker's slot.
    entry: {
      element?: HTMLElement
      position: [number, number] | null
      added: boolean
...
// Behind a dynamic import, so it lands a microtask after render.
const engine = async () => {
...
describe("F0Map", () => {
  ...
  describe("current location", () => {
    ...
    it("draws the dot beneath the lines once located", async () => {
      ...
      // Below the lines either way: anchored under them when they already
      // exist, otherwise simply inserted first.
