...
const F0MapBase = forwardRef<F0MapHandle, F0MapProps>(function F0Map(
  ...
  useEffect(() => {
    ...
    // Behind a dynamic import, so the teardown has to cope with an unmount
    ...
    void loadMapAdapterFactory(engine, engineConfigRef.current)
      ...
      .catch((error: unknown) => {
        // A failed chunk is a deployment problem, not a missing capability,
        // so it is logged as well as falling back.
