...
/**
 * Which part of the container external chrome covers, in screen px - typically
 * a side panel opened over the map. Camera moves aim at the centre of what is
 * left, not the centre of the container.
 */
...
export interface CameraOptions {
  inset?: Inset
  /** Breathing room inside the free region, in screen px. */
  gutter?: number
  animate?: boolean
}
 ...
 *
 * - There is no `padding`. That is MapLibre's way of saying "chrome covers this
 *   strip"; Google has no camera padding at all and gets the same result from a
 *   pixel offset. The port asks for the outcome (`centerInFreeRegion`) and each
 *   adapter picks its mechanism.
...
export interface MapAdapter {
  ...
  /**
   * Put a coordinate at the centre of the region left free by `inset`, at the
   * current zoom.
   */
  centerInFreeRegion(at: LngLat, options?: CameraOptions): void
  /**
   * The covered region changed but the target did not: shift the camera so the
   * same ground stays in the free area. Distinct from the moves above, which
   * all aim somewhere new.
   */
  setFreeRegion(options?: CameraOptions): void
  /**
   * Frame every coordinate inside the free region.
   *
   ...
   * Google disagree on the same coordinates and inset by 0.082 deg of longitude
   * (~6.8 km at 41 deg N). Before a second adapter ships, this has to become a
   * neutral computation over `project`/`unproject` - which agree to the pixel -
   * handed down as a plain `easeTo`.
   */
  fitCoordinates(
    coordinates: LngLat[],
