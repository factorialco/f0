...
- **Opening the canvas (Split)** — move focus into the canvas (or its heading), not back to
  the top of the page.
- **Opening the template gallery, or a template preview** — both take the full width and cover the
  chat, so focus must move into them: a user left behind on the covered conversation is focused on
...
- **Closing the canvas** — return focus to whatever opened it: the resource's card; the gallery for
  a preview; and, for the gallery itself, the clarifying panel or welcome screen it came from. Each
  Close during selection is a step back, so focus follows the step.
