...
- **Opening the canvas (Split)** — move focus into the canvas (or its heading), not back to
  the top of the page.
- **Opening the template gallery** — it takes the full width and covers the chat, so focus must
  move into it: a user left behind on the covered conversation is focused on something they can no
  longer see.
...
- **Closing the canvas** — return focus to whatever opened it: the resource's card, or — for the
  template gallery, whose Close is a step back — the clarifying panel or welcome screen it came
  from.
