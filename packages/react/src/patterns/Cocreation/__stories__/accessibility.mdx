  ...
  <table className="mb-8 w-full dark:text-f1-foreground-inverse/80">
    ...
    <tbody>
      <tr>
        ...
        <td>
          Arrow keys move between options, <kbd>Enter</kbd> selects and advances, and{" "}
          <kbd>Esc</kbd> cancels the whole flow. The panel surfaces these hints inline
      ...
      <tr>
        ...
        <td>
          The accept (✓) and reject (✗) actions are real buttons with text labels, focusable
          in reading order. Resolving a proposal must not strand focus (see below).
        </td>
      </tr>
      <tr>
        <td>
        ...
        <td>
          Opening and closing the canvas is a focusable control on the card — never a
          pointer-only target. Superseded cards drop their action and leave the tab order.
        </td>
      </tr>
      <tr>
        <td>
        ...
        <td>
          The One switch is reachable in every phase, so the keyboard user can always open,
          collapse, or return to the conversation.
        </td>
      </tr>
    </tbody>
  </table>
...
<DoDonts
  do={{
    description: "Give accept / reject buttons real text labels for assistive tech.",
    guidelines: [
