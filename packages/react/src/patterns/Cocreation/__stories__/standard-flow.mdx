...
- **The initial draft** — once the clarifying questions are answered, the AI does not write
  anything. It posts a draft confirmation ("Draft a first set of questions") and waits. The canvas,
  the resource card, and the questions all stay hidden until the user chooses. **Apply** reveals the
  survey, runs the [processing overlay](?path=/docs/patterns-co-creation-building-blocks--documentation),
  and drafts the questions; **Discard** hands over a blank resource instead. Either way the user
  ends up in control of a resource they said yes to.
