...
A **Guided Chat** flow has one extra case. Because it has no welcome screen to fall back to, closing
the template gallery or preview **before** a survey has been picked would abandon the creation with
nothing to return to — so that close warns first ("Leave creation?"), where the same close on a
Welcome Screen flow would simply drop back to the welcome screen. Match the warning to whether the
close actually strands the user.
