...
warns first ("Leave creation?"), whether the user closes the template gallery or preview **or
dismisses the opening clarifying panel itself**. **Keep creating** reopens the panel where they left
off; **Leave** exits and notes that nothing was created. On a Welcome Screen flow the same actions
simply drop back to the welcome screen, so they don't warn — match the warning to whether backing
out actually strands the user.
