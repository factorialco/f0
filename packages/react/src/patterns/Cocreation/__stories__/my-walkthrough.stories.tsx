 ...
 */
const meta = {
  title: "Co-creation/My Walkthrough",
  // Local playground copy of creation-with-ai.stories.tsx — edit freely.
  // The original Walkthrough stays untouched for reference.
...
const TemplatesOverlayContext = createContext<{
  templatesOverlayOpen: boolean
  openTemplatesOverlay: () => void
  closeTemplatesOverlay: () => void
}>({
...
// custom "survey" entity this story demos, so we widen at the provider
// boundary (the registry/panel match on the `type` string at runtime).
...
const toCanvasContent = (content: SurveyCanvasContent): CanvasContent =>
  content as unknown as CanvasContent

const toCanvasEntity = (
  entity: CanvasEntityDefinition<SurveyCanvasContent>
...
/**
 * Creates a survey copy from a template and opens it in the EDIT canvas —
 * the moment co-creation actually starts, so this is where the chat docks
 * beside the resource. Extracted from the old preview canvas header so the
 * templates overlay's "Use this template" action can drive the same flow.
 */
function useCreateSurveyFromTemplate() {
  ...
  return (templateName: string, description?: string) => {
    ...
    const surveyId = createSurvey(templateName, { empty: false })
    openCanvas(
      toCanvasContent({
        type: "survey",
        title: templateName,
    ...
    armProposal(surveyId, templateName)
  }
}
...
function TemplatesOverlay() {
  const { templatesOverlayOpen, closeTemplatesOverlay } = useContext(
    TemplatesOverlayContext
  )
  const [previewed, setPreviewed] = useState<Template | null>(null)
  const createSurveyFromTemplate = useCreateSurveyFromTemplate()

  if (!templatesOverlayOpen) return null
  ...
  const useThisTemplate = () => {
    ...
    if (!item) return
    // Open the edit canvas BEHIND the still-covering overlay: the
    ...
    // already docked next to the resource, with no visible animation.
    createSurveyFromTemplate(item.name, item.description)
        ...
        {previewed ? (
          <>
            <div className="flex flex-row items-center gap-3 border border-x-0 border-b border-t-0 border-solid border-f1-border-secondary px-4 py-3">
              ...
              {/* ButtonGroup owns the responsive collapse: when the row is wide
                  both actions show; when the title grows or the window narrows,
                  the secondary "Edit Template" sheds into the "⋯" menu while the
                  primary "Use this template" stays pinned. */}
              <ButtonGroup
          ...
          <>
            <div className="flex flex-row items-center justify-between gap-3 border border-x-0 border-b border-t-0 border-solid border-f1-border-secondary px-4 py-3">
              <F0Heading content="Templates" as="h2" />
              <ButtonInternal
                variant="outline"
                hideLabel
                label="Close"
...
function SurveyWelcomeCardsRegistrar() {
  ...
  const { openTemplatesOverlay } = useContext(TemplatesOverlayContext)
  const {
    createSurvey,
    draftQuestions,
    nextCardId,
...
function CreationWithAIFlow({ initialTabId }: { initialTabId?: string }) {
  ...
  const templatesOverlayValue = useMemo(
    () => ({
      ...
      openTemplatesOverlay: () => setTemplatesOverlayOpen(true),
      closeTemplatesOverlay: () => setTemplatesOverlayOpen(false),
    }),
    [templatesOverlayOpen]
  )
