...
export type OperatorFilterOptions = {
   ...
   * Invalid entries (e.g. non-numeric text for "number") are rejected with
   * an inline error and never emitted to the controlled filter state.
   * @default "string"
   */
...
const isInvalidNumber = (
  ...
  valueType === "number" &&
  raw.trim().length > 0 &&
  coerceValue(raw, valueType) === undefined
...
const hasInvalidDraft = (
  ...
  switch (mode) {
    case "none":
      return false
    case "single":
      return isInvalidNumber(draft.single, valueType)
    case "multiple":
      return draft.multiple
        .split(",")
        .some((entry) => isInvalidNumber(entry, valueType))
    case "range":
      return (
        isInvalidNumber(draft.from, valueType) ||
        isInvalidNumber(draft.to, valueType)
      )
  }
}
...
export function OperatorFilter({
  ...
  const emit = (next: DraftInputs) => {
    ...
    const nextMode = operatorValueMode(nextOperator)
    if (hasInvalidDraft(next, nextMode, valueType)) return
    onChange({
      operator: nextOperator.value,
      values: draftValues(next, nextMode, valueType),
    })
  }
  ...
  const renderSingleInput = (
    ...
    valueType === "boolean" ? (
      ...
      <InputInternal
        ...
        error={
          isInvalidNumber(draft[key], valueType)
            ? copy.invalidNumber
            : undefined
        }
        aria-invalid={
          isInvalidNumber(draft[key], valueType) ? "true" : undefined
        }
        clearable
      />
    )
