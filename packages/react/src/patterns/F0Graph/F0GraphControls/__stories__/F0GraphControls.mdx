...
When `tagTypes.length > 0` and `onToggleTagType` is provided, a metadata
popover button is rendered in the toolbar. The popover lists each
tag type with a toggle switch bound to `visibleTagTypes`.

If `tagTypes` is empty (or omitted), or `onToggleTagType` is not
provided, the metadata popover button is hidden.

<Canvas of={Stories.WithMetadataToggle} />
