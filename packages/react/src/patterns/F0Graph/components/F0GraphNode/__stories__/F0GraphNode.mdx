...
Use `visibleTagTypes` to filter which tag **columns** are rendered. A tag's
column is its `column` key when set, otherwise its visual `type`. `tagLabels`
provides per-column labels used as metadata row labels in the node hover card.

By default a tag's column is its `type`, so two tags of the same `type` (e.g.
two `raw` pills) share one column, toggle and label. Give a tag an explicit
`column` to split it into its own independent column — for example a "Hire
date" `raw` pill that must toggle separately from a "Workplace" `raw` pill:
...
visibleTagTypes={new Set(["workplace"])} // "hireDate" hidden by default
```
...
<DoDonts
  do={{
    description:
      "Answer \"who is this and what do they do\" and stop there — a node is scanned, not read.",
    guidelines: [
      "Title is the person or team name; subtitle is the job title or the headcount",
      "Spread ctx so selection, focus and the zoom levels keep working",
      "Let the zoom level drop the subtitle rather than shortening it yourself",
    ],
  }}
  dont={{
    description: "Don't pack a whole record into a node.",
    guidelines: [
      "Tag rows are for the one or two facts that change the reader's next click",
      "An id, an email and a start date belong in the detail panel, not on the canvas",
      "Don't hand-roll a node div — you lose keyboard focus and the treeitem semantics",
    ],
  }}
/>
...
| `tags`            | `F0GraphNodeTag[]`                                     | —           | Tag row rendered in detail variant; every tag shown individually (no grouping). Each tag may set `column` to occupy its own column.           |
| `visibleTagTypes` | `ReadonlySet<F0GraphNodeTagColumn>`                    | —           | Filters rendered tags by column (`column ?? type`) when provided                                                                              |
| `tagLabels`       | `F0GraphNodeTagLabels`                                 | —           | Per-column labels used as metadata row labels in the node hover card                                                                          |
