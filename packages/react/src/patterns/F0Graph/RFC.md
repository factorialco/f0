...
**What it unblocks:** A future v2 composition API (documented in [`REFACTOR-PLAN.md`](REFACTOR-PLAN.md)) targeting both read-mode hierarchical exploration AND workflow editing (DAG with branching, fork/join, multiple node types). The current v1 is the read-mode foundation.
...
| ---------------------------- | ------------------------------------- | ----------------------------------- |
| `F0GraphZoomContext`         | `{ zoomLevel, currentZoom }`          | Node wrappers (variant selection)   |
...
| `F0GraphActionsContext`      | `{ toggleExpand, selectNode }`        | Node wrappers (click handlers)      |
| `F0GraphRenderConfigContext` | `{ renderEdge? }`                     | Edge wrapper (custom edge override) |
...
3. **The `REFACTOR-PLAN.md` describes a v2 composition API** (`F0Graph.Canvas`, `F0Graph.Node`, etc.) that's fundamentally different from the current render-prop approach. What's the migration timeline? Should consumers build against v1 knowing it will break?
...
| React Flow as rendering engine  | Custom canvas / SVG      | `REFACTOR-PLAN.md` non-goals: "Not replacing React Flow"                                                                                                                                                           |
...
| `REFACTOR-PLAN.md`               | Future v2 composition API design document                |
