...
| Story                | What it shows                                               |
| -------------------- | ----------------------------------------------------------- |
| `BasicTree`          | Default tree rendering with standard node content           |
| `CompactView`        | Starting in compact zoom variant                            |
| `DotView`            | Starting in dot zoom variant                                |
| `WithControls`       | Visible zoom and canvas controls                            |
| `Lazy`               | On-demand subtree loading with `rootNodes` + `loadChildren` |
| `LargeTree`          | Higher node-count tree rendering                            |
| `CustomNode`         | Fully custom node UI via `renderNode`                       |
| `CustomEdge`         | Custom edge rendering via `renderEdge`                      |
| `Controlled`         | Controlled expansion and selection state                    |
| `RawSearch`          | Raw controlled search input driven by consumer state        |
...
<Canvas of={F0GraphStories.RawSearch} />
...
| ------------------ | ----------------- | -------------------- | ------------------------------------------------------------------ |
| `direction`        | `LayoutDirection` | `"TB"`               | Tree layout direction (`"TB"`, `"LR"`, `"BT"`, `"RL"`).            |
| `defaultDirection` | `LayoutDirection` | `"TB"`               | Initial layout direction when `direction` is uncontrolled.         |
