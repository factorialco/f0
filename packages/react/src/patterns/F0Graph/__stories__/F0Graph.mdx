...
| `StackedNodes`       | Leaf children rendered as a column under their parent       |
...
A parent that sets `stackNodes` renders its children as a tight
...
```tsx
const nodes = [
  { id: "role", parentId: "root", data: role, childrenCount: 3, stackNodes: true },
...
<Canvas of={F0GraphStories.StackedNodes} />
...
| `stackedNodeHeight` | `number`       | `44`                 | Height of one stacked node row (see `GraphNode.stackNodes`).       |
| `stackedNodeGap`    | `number`       | `16`                 | Vertical gap between two stacked node rows.                        |
