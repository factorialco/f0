...
renderNode={(node, ctx) =>
  ctx.stacked ? (
    <F0GraphStackedNode {...ctx} title={node.data.name} trailing={<F0Checkbox … />} />
  ) : (
...
so there is nowhere to hang a subtree. Rows do not morph with zoom
either: the layout reserves a fixed band per row (`stackedNodeHeight`,
default `40`, plus `stackedNodeGap`, default `8`), so
`F0GraphStackedNode` must render at that same height.
