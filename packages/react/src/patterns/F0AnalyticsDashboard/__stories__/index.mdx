...
Bar, line, heatmap, and scatter widgets add **Select chart area** when an AI
chat or host `onAskAi` handler is available. Activating it lets the reader
either draw around chart data or choose exact data points from a checkbox menu.
Both paths attach the selected values to the composer; neither sends a message,
so the reader can add a question such as “What happened here?” first.

<Canvas of={AskOneStories.ChartAreaSelectionMode} />

For keyboard, touch, or single-pointer selection without dragging, choose
**Choose data points**, toggle one or more values, then choose **Use selected
data points**.

<Canvas of={AskOneStories.ChartAreaSelectionControls} />
...
are mutually exclusive. `selection.source` is `"pointer"` for a drawing and
`"control"` for the checkbox path.
...
- WHEN selection mode starts → THEN the stable toggle name announces its pressed state and a persistent live region explains the drawing, checkbox, and cancellation paths.
- WHEN the completed polygon contains data → THEN the built-in behavior exits fullscreen, attaches a bounded formatted quote, opens chat, and focuses the composer without sending.
- WHEN one or more values are confirmed through **Choose data points** → THEN the same bounded quote or host callback runs without requiring a dragging gesture.
...
- WHEN the reader presses <kbd>Escape</kbd> or chooses the active toggle → THEN selection mode closes without attaching anything.
- WHEN data, loading state, chart type, or responder ownership changes → THEN any in-progress drawing is cancelled so it cannot resolve against stale data.
- WHEN a widget uses pie, funnel, radar, or gauge → THEN no area-selection action is shown because those coordinate systems cannot produce the same exact data contract.
  ...
  <table className="mb-8 w-full dark:text-f1-foreground-inverse/80">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          Opens controls, activates focused filter options or actions, toggles a
          **Choose data points** checkbox, and confirms **Use selected data
          points**.
        </td>
      </tr>
      <tr>
