...
Review multiple selected options, a date range, and a numeric range. Boolean and valueless conditions are intentionally not part of the shared report-filter contract; an opt-in widget operator filter can represent them when its data source supports those operators.
...
`undefined` everywhere else. F0 owns the picker draft and responsive header;
the host owns the applied state, data refetch, and persistence.
...
- WHEN space is constrained → THEN the icon's count remains visible; wider widgets progressively reveal up to three editable chips.
- WHEN a numeric condition is invalid → THEN its inline error is announced and the picker keeps focus in the editor until it is corrected.
...
**Responsive filter signals**

The three fixed-width widgets exercise the count-only, two-chip, and
three-chip states with browser-visible assertions.

<Canvas of={Stories.ResponsiveItemFilterSignals} />
