...
>({
  ...
  const activeFilterKeys = useMemo(
    () => getActiveFilterKeys(filters, value, i18n),
    [filters, value, i18n]
  )

  const appliedCount = useMemo(() => {
    const count = activeFilterKeys.length
  ...
  const appliedFilterLabel = useMemo(() => {
    return activeFilterKeys.length > 0
      ? i18n.t("filters.activeFilters", {
          filters: activeFilterKeys.map((key) => filters[key].label).join(", "),
        })
      : i18n.filters.label
  }, [activeFilterKeys, filters, i18n])
  ...
  useLayoutEffect(() => {
    ...
    const buttons = Array.from(content.querySelectorAll<HTMLElement>("button"))
    const label = returnFocusLabelRef.current
    const row = label
      ? buttons.find((button) => button.textContent?.trim().includes(label))
      : undefined
    // The saved row may have disappeared while the picker was open. In that
    // case, move to the first remaining control instead of dropping focus on
    // the unnamed scroll viewport (or all the way back to <body>).
    ;(row ?? buttons[0])?.focus()
    returnFocusLabelRef.current = undefined
  ...
  const returnToList = () => {
    const invalidInput = contentRef.current?.querySelector<HTMLElement>(
      '[aria-invalid="true"]'
    )
    if (invalidInput) {
      invalidInput.focus()
          ...
          <div className="flex shrink-0 items-center gap-2 py-1.5 pl-1.5 pr-3">
            {activeSelectedFilterKey ? (
              <>
                ...
                />
                <h4 className="m-0 truncate text-base font-medium text-f1-foreground">
