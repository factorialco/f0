...
/**
 * Roast-demo switch. When ON, the Submit ▸ Expenses table starts
 * CLEARED — it shows ONLY the rows born from Factorial One (the
 * chat-created drafts + the folders One groups them into). All the
 * seeded fixture cohorts (folders, draft re-casts, changes-requested,
 * real expenses, demo drafts) are suppressed so the table is empty
 * until the user drops receipts and One drafts them. The Manage
 * source is untouched — only this Submit view is cleared.
 */
const CLOSED_CIRCUIT_DEMO = true
 ...
 *
 * BR-010 / AC-008: the ways to create an expense live in a single
 * "New expense" DROPDOWN button above the table (mirrors production My
 * spending — the menu drops from the New expense button, not an
 * overflow). "Submit with One" leads the list as the promoted
 * conversational path; Regular expense / Mileage / Per diem / Folder
 * follow.
 *
...
export function useSubmitExpensesRows(): SpendingRow[] {
  ...
  // Re-derive when an expense's status is overridden (e.g. "Send for
  // approval" flips a To-Do draft to `pending`) so it moves from the
  // To-Do preset to Submitted live, the same way the Manage tabs do.
  const overridesVersion = useStatusOverridesVersion()

  return useMemo(() => {
    // Rows born from Factorial One. Chat folders first, then the
    // loose drafts that aren't inside a folder (grouped drafts are
    // represented by their folder row, not as duplicate loose rows).
    const chatFolderRows = chatFolders.map((f) =>
      chatFolderToRow(f, chatDrafts)
    )
    const groupedDraftIds = new Set(
      chatFolders.flatMap((f) => f.memberIds)
    )
    const chatRows = chatDrafts
      .filter((d) => !groupedDraftIds.has(d.id))
      .map(chatDraftToRow)

    // CLOSED-CIRCUIT DEMO: the To-Do list starts cleared — only
    // One-created rows show. Skip the entire fixture pipeline below.
    if (CLOSED_CIRCUIT_DEMO) {
      return [...chatFolderRows, ...chatRows]
    }
  ...
  }, [chatDrafts, chatFolders, overridesVersion])
}
...
export function useSubmitExpensesSource(args: {
  ...
  onNewFolder: () => void
  /**
   * "Submit with One" — opens Factorial One ready to turn one or more
   * dropped receipts into draft expenses (single or bulk). Sits at the
   * top of the New-expense dropdown as the promoted, conversational
   * alternative to picking an expense type and filling the form by hand.
   */
  ...
  return useDataCollectionSource<SpendingRow>(
    {
      ...
      // Returning an ARRAY from primaryActions (plus `primaryActionsLabel`)
      // is what makes OneDataCollection render the trigger as a "New
      // expense" DROPDOWN button — the production My-spending pattern —
      // instead of a single button with the rest in an overflow menu.
      // Each item carries the same sub-description as production.
      // "Submit with One" leads (promoted, One-branded icon).
      primaryActions: () => [
        {
          // Action-led title (what you do) + benefit subtitle (faster, via
          // One). No awareness banner for now, so this one line carries the
          // whole pitch; receipt-specific until One creates non-receipt types.
          label: "Upload receipts",
          icon: ReceiptAiIcon,
          description: "Submit expenses automatically with One",
          onClick: onSubmitWithOne,
        },
        {
        ...
        },
        {
          ...
          onClick: onNewPerDiem,
        },
        {
          label: "Folder",
          icon: Folder,
          description: "Multiple related expenses",
          onClick: onNewFolder,
        },
      ],
