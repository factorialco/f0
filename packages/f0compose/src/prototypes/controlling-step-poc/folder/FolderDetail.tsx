...
import { useChatDrafts, useChatFolders } from "../copilot/useChatDrafts"
...
export function FolderDetail(props: {
  ...
  // Chat-created folders (One grouped these) take precedence — they
  // carry their real member ids. Fixture groups fall back to slicing
  // the first N expenses (they don't store a member list).
  const chatFolders = useChatFolders()
  const chatDrafts = useChatDrafts()
  const chatFolder = chatFolders.find((f) => f.id === props.folderId)

  const fixtureFolder: ExpenseGroup | undefined = expenseGroups.find(
    (g) => g.id === props.folderId
  )

  // Normalise both kinds into the same ResourceHeader-ready shape.
  const folder = chatFolder
    ? {
        id: chatFolder.id,
        name: chatFolder.name,
        status: chatFolder.status,
      }
    : fixtureFolder
    ? {
        id: fixtureFolder.id,
        name: fixtureFolder.name,
        status: fixtureFolder.status,
      }
    : undefined
  ...
  const memberRows = useMemo(() => {
    if (chatFolder) {
      return chatDrafts
        .filter((d) => chatFolder.memberIds.includes(d.id))
        .map(chatDraftToRow)
    }
    if (fixtureFolder) {
      return expenses.slice(0, fixtureFolder.expenseCount).map(expenseToRow)
    }
    return []
  }, [chatFolder, fixtureFolder, chatDrafts])

  const folderTotal = useMemo(
    () => memberRows.reduce((s, r) => s + r.amount, 0),
    [memberRows]
  )
