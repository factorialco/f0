...
import { costCenters, employees, projects, vatRates } from "@/fixtures"
...
/**
 * Employees keyed by id, so the owner column can resolve a row's
 * `ownerEmployeeId` to a name + avatar in O(1). Built once at module
 * load (the directory is a static fixture).
 */
const EMPLOYEE_BY_ID = new Map(employees.map((e) => [e.id, e]))

/**
 * Owner-flavoured first column. Used by the Approve, Pending payment
 * and Ready to export stages (see `ownerColumns`) where the reviewer
 * cares about WHO submitted the expense, not which vendor it was paid
 * to. The header reads "Owner" and the cell renders the owning
 * employee as an avatar + name via the built-in `person` cell type
 * (same renderer the Recruitment / Time-off tables use).
 *
 *   - Folder rows have no owner — they keep showing the folder name
 *     via the `folder` cell type, identical to `spendingColumns[0]`.
 *   - Expense rows resolve `ownerEmployeeId` → employee. Every row
 *     carries a stable owner (assigned in `expenseToRow`), so the
 *     lookup effectively always hits; if it somehow misses we fall
 *     back to plain text so the cell never renders blank.
 *
 * The detail page H1 is unaffected — it always shows the vendor
 * (`row.headerTitleOverride ?? row.name`), independent of this column.
 */
const ownerNameColumn = {
  id: "name",
  label: "Owner",
  sorting: "name",
  render: (item: SpendingRow) => {
    if (item.kind === "folder") {
      return { type: "folder" as const, value: { name: item.name } }
    }
    const owner = item.ownerEmployeeId
      ? EMPLOYEE_BY_ID.get(item.ownerEmployeeId)
      : undefined
    if (!owner) return item.name
    const [firstName, ...rest] = owner.fullName.split(" ")
    return {
      type: "person" as const,
      value: {
        firstName: firstName ?? owner.fullName,
        lastName: rest.join(" "),
        src: owner.avatarUrl,
      },
    }
...
/**
 * Approve / Pending payment / Ready to export column set. Identical
 * to `spendingColumns` except the first column shows the OWNER
 * (avatar + name, header "Owner") instead of the vendor name. Submit
 * and Finance control keep `spendingColumns` / `controllingColumns`
 * (vendor name, header "Name") — those personas reconcile against the
 * provider, not the submitter.
 */
...
export function policyStateFor(row: SpendingRow): PolicyStateKey | null {
  ...
  if (a.includes("missing-receipt")) return "missing-receipt"
...
/**
 * Approve column set — owner-first (like `ownerColumns`) PLUS the
 * Alerts column. The Approve queue exists to triage policy signals,
 * so the alert tag is the primary thing a reviewer scans; we place it
 * second (right after Owner) for prominence, mirroring `submitColumns`.
 * Result: Owner · Alerts · Category · Date · Amount · Status.
 */
export const ownerAlertColumns = [
  ownerNameColumn,
  {
    id: "policyState",
    label: "Alerts",
