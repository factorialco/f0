...
import { ADD_RELATED_RECEIPTS_PROMPT } from "../../copilot/oneEntryPoints"
import { useOnePromo } from "../../copilot/useOnePromo"
import { useChatDrafts } from "../../copilot/useChatDrafts"
import { updateChatDraft } from "../../copilot/chatDraftsStore"
import { setOpenForEditId } from "../../copilot/chatFocusStore"
...
import { OneNudgeBanner } from "./OneNudgeBanner"
...
export function ExpenseDetailPage(props: {
  ...
  /** Approve the row — approval-tab primary CTA. */
  onApprove: () => void
  /** Mark the row as paid — pay-tab primary CTA. */
  onMarkPaid: () => void
  /** Send a draft / changes-requested row for approval — submitter primary. */
  onSendForApproval: () => void
  /** Reject the row (terminal). Carries the manager's optional reason. */
  onReject: (reason?: string) => void
  /** Return the row to the submitter for changes, with an optional note. */
  ...
  // One-created drafts live in `chatDraftsStore`, not the fixtures.
  // When the open row is a chat draft we resolve its current store
  // record and merge the editable fields (vendor, amount, date,
  // description, project, cost center, participants) into a "live"
  // row, so:
  //   - the summary opens fully populated instead of a skeleton, and
  //   - edits made via One OR inline click-to-edit re-render the page
  //     immediately (the store notifies → `useChatDrafts` re-renders).
  // Non-chat (fixture) rows pass through untouched.
  const chatDrafts = useChatDrafts()
  const isChatDraft = rowProp.id.startsWith("chat-draft-")
  const chatDraft = isChatDraft
    ? chatDrafts.find((d) => d.id === rowProp.id)
    : undefined
  const row: SpendingRow = useMemo(() => {
    if (!chatDraft) return rowProp
    const controllingMerged: ControllingFields = {
      ...(rowProp.controlling ?? {}),
      ...(chatDraft.description ? { description: chatDraft.description } : {}),
      ...(chatDraft.project ? { project: chatDraft.project } : {}),
      ...(chatDraft.costCenter ? { costCenter: chatDraft.costCenter } : {}),
    }
    return {
      ...rowProp,
      name: chatDraft.vendor ?? chatDraft.provider,
      description: chatDraft.category,
      amount: chatDraft.amount,
      date: chatDraft.date,
      alerts: chatDraft.alerts,
      participants: chatDraft.participants,
      controlling:
        Object.keys(controllingMerged).length > 0
          ? controllingMerged
          : undefined,
    }
  }, [rowProp, chatDraft])
  ...
  // "Add with One" in-context nudge. Shown on a To-Do expense the user
  // is still assembling — the moment the grouping wedge is relevant.
  // Visibility is orchestrated (see useOnePromo / ONE_PROMO_TIMING.md):
  // it graduates in only after the discovery banner is retired, and
  // switches off once One has created an expense.
  const { sendMessage } = useChatRuntime()
  const { showCallout, dismissCallout } = useOnePromo()

  // Edit-via-One: for One-created drafts the Edit pencil does NOT
  // open the form — it records which draft is "open for edit" (so
  // `useEditExpenseAction` knows what to mutate) and posts a kickoff
  // into the chat ("Editing <vendor> — what would you like to
  // change?"). Cleared on navigate-away so a stray "change …" later
  // doesn't mutate a stale draft.
  useEffect(() => {
    return () => setOpenForEditId(null)
  }, [])
  const editViaOne = useCallback(() => {
    setOpenForEditId(rowProp.id)
    sendMessage(
      `Editing ${row.name} — what would you like to change?`
    )
  }, [rowProp.id, row.name, sendMessage])

  // Inline click-to-edit commit handlers (chat drafts only). Plain-
  // text fields write straight back to the store via updateChatDraft;
  // coded fields (category/cost center/project) are intentionally NOT
  // inline-editable — those stay One-only.
  const commitInline = useCallback(
    (patch: Parameters<typeof updateChatDraft>[1]) => {
      if (isChatDraft) updateChatDraft(rowProp.id, patch)
    },
    [isChatDraft, rowProp.id]
  )

  // Reject / Request-changes reason modal. Both reuse one F0Dialog;
  // `reasonFlow` records which action opened it (so the title, copy
  // and confirm label switch) and which handler fires on confirm.
  // Reject pre-fills a short rejection message the manager can edit;
  // Request changes starts blank so they write what needs fixing.
  const [reasonFlow, setReasonFlow] = useState<
    null | "reject" | "request-changes"
  >(null)
  const [reasonText, setReasonText] = useState("")
  const DEFAULT_REJECT_REASON =
    "This expense doesn't meet our expense policy, so I'm returning it as rejected. Reach out if you'd like to discuss."
  const openReason = useCallback((mode: "reject" | "request-changes") => {
    setReasonText(mode === "reject" ? DEFAULT_REJECT_REASON : "")
    setReasonFlow(mode)
  }, [])
  const confirmReason = useCallback(() => {
    const mode = reasonFlow
    setReasonFlow(null)
    if (mode === "reject") onReject(reasonText)
    else if (mode === "request-changes") onRequestChanges(reasonText)
  }, [reasonFlow, reasonText, onReject, onRequestChanges])
  ...
  const missingRequired = useMemo(
    ...
    // Also recompute when the chat draft's description changes (inline
    // click-to-edit / edit-via-One fill it), which clears the gate.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [row.id, editMode, chatDraft?.description]
  )
  ...
  const receiptUrl: string | null = useMemo(() => {
    // One-created drafts carry the exact receipt One saw — always
    // render it (don't gate on the synthetic `hasReceipt` hash).
    if (chatDraft?.receiptDataUrl) return chatDraft.receiptDataUrl
    if (isChatDraft) return buildReceiptDataUrl(row)
    if (!rowHasReceipt(row.id)) return null
    return buildReceiptDataUrl(row)
  }, [row, chatDraft, isChatDraft])
  ...
  // Action-role pages (Approve / Control / Pending payment) render
  // their CTAs as a CUSTOM cluster — the role's primary button plus a
  // Reject SPLIT dropdown (main = Reject, chevron = Request changes) —
  // sitting beside the ResourceHeader. We render it ourselves because
  // ResourceHeader's action slots only draw plain buttons; they can't
  // host an f0 split DropdownButton. Edit mode and the submitter/view
  // roles keep using ResourceHeader's own action props unchanged.
  const isActionRole =
    !editMode &&
    (tabRole === "approval" ||
      tabRole === "controlling" ||
  ...
  const clusterPrimary =
    tabRole === "controlling"
      ...
      : tabRole === "pay"
        ? { label: "Mark as paid", icon: CheckCircle, onClick: onMarkPaid }
        : { label: "Approve", icon: CheckCircle, onClick: onApprove }

  // Lifted so both the action-role layout and the default layout pass
  // the identical metadata block to ResourceHeader.
  const headerMetadata = [
    {
      label: "Creation date",
      value: { type: "text" as const, content: formatDate(row.date) },
    },
    {
      label: "Total reimbursement",
      value: { type: "text" as const, content: formatEUR(row.amount) },
    },
  ]
          ...
          ? [
              {
                ...
                variant: "outline" as const,
                onClick: () => {
                  // One-created drafts edit via One (the chat), not the
                  // form. Fixture rows keep the in-place form toggle.
                  if (isChatDraft) {
                    editViaOne()
                    return
                  }
        ...
        <>
          ...
          {isActionRole ? (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              ...
              {/* Custom CTA cluster — pinned flush-right (marginLeft auto)
                  and lifted to the top of the header band so its top edge
                  rides with the One button / navigation row above, instead
                  of floating low against the title block. The Reject
                  control is a real f0 split DropdownButton: main button
                  rejects, the chevron reveals "Request changes". Both open
                  the reason modal. */}
              <div
                style={{
                  display: "flex",
                  ...
                  flexShrink: 0,
                  marginLeft: "auto",
                  // Push the cluster's right edge out to line up with the
                  // One button / nav controls above, which sit slightly
                  // further right than this row's default content edge.
                  // Tunable: increase the magnitude to move further right.
                  marginRight: -10,
                  marginTop: -4,
                }}
              >
                ...
                <F0ButtonDropdown
                  ...
                  size="md"
                  items={[
                    { value: "reject", label: "Reject", icon: Cross },
                    {
                  ...
                  onClick={(value) =>
                    openReason(
                      value === "request-changes" ? "request-changes" : "reject"
                    )
                  }
                />
                <F0Button
                  variant="default"
                  size="md"
                  icon={clusterPrimary.icon}
                  label={clusterPrimary.label}
                  onClick={clusterPrimary.onClick}
                />
              </div>
            </div>
          ) : (
    ...
    >
      <StandardLayout>
        {activeTabId === "comments" ? (
          ...
          // Split-pane: left (form/summary) and right (receipt) scroll
          // INDEPENDENTLY within the page height — neither overscrolls
          // past its own content, so there's no awkward empty space at
          // the bottom. The page itself doesn't scroll. Applies to both
          // edit and summary views.
          <div
            className="absolute flex flex-col gap-6 overflow-hidden md:flex-row"
            // Absolutely fill StandardLayout's (position:relative) content
            // box. Escaping the flow lets that `min-h-full` wrapper
            // collapse to the bounded scroll viewport, so the PAGE no
            // longer scrolls — the two panes below scroll on their own.
            // Inset matches StandardLayout's py-5 px-6 padding.
            style={{ top: 20, right: 24, bottom: 20, left: 24 }}
          >
            <div
              className="flex min-h-0 flex-col gap-4 overflow-y-auto"
              // inline flex: composer's precompiled CSS lacks `md:flex-1`,
              // so the panes wouldn't grow to fill the width otherwise.
              style={{ flex: "1 1 0%", minWidth: 0 }}
            >
              {/* In-context One nudge — sits ABOVE the policy-agent
                  alert and is intentionally muted (see OneNudgeBanner)
                  so it doesn't compete with it. Scoped to THIS expense
                  (add more receipts, grouped). To-Do only, dismissable. */}
              {!editMode && isSubmitterTodo && showCallout && (
                <OneNudgeBanner
                  onAddWithOne={() => {
                    // Engaging is a positive signal — One takes over in
                    // the chat, so the nudge retires (same as dismiss).
                    sendMessage(ADD_RELATED_RECEIPTS_PROMPT)
                    dismissCallout()
                  }}
              ...
              {editMode ? (
                ...
                <ReadView
                  row={row}
                  tabRole={tabRole}
                  // Inline click-to-edit is offered ONLY on One-created
                  // drafts (the rows the store can mutate). Fixture rows
                  // get the read-only summary.
                  inline={
                    isChatDraft
                      ? {
                          onAmount: (raw) => {
                            ...
                            if (Number.isFinite(n) && n > 0) {
                              commitInline({ amount: Math.round(n * 100) / 100 })
                            }
                          },
                          onDescription: (raw) => {
                            commitInline({ description: raw })
                            // Filling the description clears the
                            // required-fields gate for this draft.
                            markFieldsFilled(rowProp.id)
                          },
                          onVendor: (raw) =>
                            commitInline({ vendor: raw, provider: raw }),
                          onDate: (raw) => commitInline({ date: raw }),
                        }
                      : undefined
                  }
                />
            ...
            {/* Receipt pane — fills the column height; its toolbar stays
                fixed at the top while the preview scrolls inside it. */}
            <div
      ...
      {/* Reject / Request-changes reason modal. One F0Dialog, two
          modes: Reject pre-fills a short rejection message (editable);
          Request changes opens blank. Confirm routes to the matching
          handler with the manager's note. */}
      {reasonFlow !== null && (
        <F0Dialog
          isOpen={reasonFlow !== null}
          onClose={() => setReasonFlow(null)}
          title={reasonFlow === "reject" ? "Reject expense" : "Request changes"}
          description={
            reasonFlow === "reject"
              ? "Let the employee know why this expense is being rejected. They'll see your note."
              : "Tell the employee what needs to change before they resubmit. They'll see your note."
          }
          width="md"
          primaryAction={{
            label: reasonFlow === "reject" ? "Reject" : "Request changes",
            onClick: confirmReason,
          }}
          secondaryAction={{
            label: "Cancel",
            onClick: () => setReasonFlow(null),
          }}
        >
          <textarea
            autoFocus
            ...
            placeholder={
              reasonFlow === "reject"
                ...
                : "What needs to change before resubmitting…"
            }
            rows={4}
            style={{
              width: "100%",
...
type InlineHandlers = {
  ...
  onDescription: (raw: string) => void
  onVendor: (raw: string) => void
...
function ReadView({
  ...
  row: SpendingRow
  tabRole: "approval" | "controlling" | "pay" | "view"
  /** When set, amount/description/vendor/date render as inline
   *  click-to-edit fields (One-created drafts only). */
  ...
  // When inline editing is active, the editable date/amount/vendor
  // rows are rendered as custom `InlineEditRow`s (so they can host an
  // input on click); only Payment method stays in this list. In the
  // read-only case all four flow through `DetailsItemsList`.
  ...
  const afterReimbursable: DetailsItemType[] = [
    ...
    // Description stays in the list ONLY in read-only mode; with
    // inline editing it's rendered as a custom InlineEditRow below.
    ...(inline
      ? []
      : ([
          {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="overflow-hidden rounded-md border border-solid border-f1-border-secondary expense-readview">
        ...
        {inline && (
          <>
            <InlineEditRow label="Document date">
              <InlineEditField
                label="Document date"
                type="date"
                value={formatDate(row.date)}
                editValue={row.date}
                onCommit={inline.onDate}
              />
            </InlineEditRow>
            <div className="h-[1px] w-full bg-f1-border-secondary" />
            <InlineEditRow label="Amount">
              <InlineEditField
                label="Amount"
                type="number"
                value={formatEUR(row.amount)}
                editValue={String(row.amount)}
                onCommit={inline.onAmount}
              />
            </InlineEditRow>
            <div className="h-[1px] w-full bg-f1-border-secondary" />
            <InlineEditRow label="Vendor name">
              <InlineEditField
                label="Vendor name"
                value={row.headerTitleOverride ?? row.name}
                onCommit={inline.onVendor}
              />
            </InlineEditRow>
            <div className="h-[1px] w-full bg-f1-border-secondary" />
          </>
        ...
        {inline && (
          <>
            <div className="h-[1px] w-full bg-f1-border-secondary" />
            <InlineEditRow label="Description">
              <InlineEditField
                label="Description"
                value={
                  c?.description && c.description.length > 0
                ...
                editValue={c?.description ?? ""}
                onCommit={inline.onDescription}
              />
            </InlineEditRow>
          </>
        )}
...
/**
 * Custom label/value row that matches `DetailsItemsList` table-view
 * layout, used to host an `InlineEditField` for the click-to-edit
 * plain-text fields (amount, description, vendor, date). Layout mirrors
 * `CategoryRow` so the rows align pixel-perfectly with the rest.
 */
...
function ReceiptPanel(props: {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative flex min-h-0 flex-1 overflow-auto rounded-md bg-f1-background"
        // `safe` centering: centered while the receipt fits, but falls
        // back to start-aligned once it overflows (zoomed), so the TOP
        // stays scrollable — plain center/items-center makes the
