...
// Must cover EVERY ExpenseCategory — a seeded value not in this list
// fails the Category select's enum ("Invalid enum value … received
// 'Per diem'"). Mileage + Per diem were missing.
...
const sections = {
  ...
  attendees: {
    title: "Attendees",
    description:
      "Tell us who joined this expense and how many guests were present.",
  },
  payment: {
    title: "Payment",
  ...
  tax: {
    title: "Tax",
    description: "Add any applicable taxes for this expense.",
  },
  budget: {
  ...
  costcenters: {
    title: "Cost centers",
    description: "Split this expense across one or more cost centers.",
  },
  additional: {
  ...
  files: {
    title: "Files",
    description: "Attach the receipt or any supporting documents.",
  },
} as const
...
/**
 * Per-expense field preset.
 *
 * exp-016 is the "everything shown" worst case. exp-017 demos a
 * recommended best-practice form: hide advanced / back-office fields
 * and relax a couple to optional, so the form is far more digestible.
 * Keyed by row id; the default preset shows everything (unchanged
 * behaviour for every other expense).
 *
 *  - `hidden`: configurable fields dropped from the form. Their seeded
 *    values still submit; documentNumber/vendorTIN stay pre-filled, so
 *    hiding them never blocks Save.
 *  - `optional`: fields forced to optional (no asterisk) even though
 *    they're required in the maximal form.
 *  - `droppedSections`: sections whose every field is hidden — removed
 *    entirely so there's no empty collapsed header.
 */
type FormPreset = {
  hidden: ReadonlySet<string>
  optional: ReadonlySet<string>
  droppedSections: ReadonlySet<string>
}

const MAXIMAL_PRESET: FormPreset = {
  hidden: new Set(),
  optional: new Set(),
  droppedSections: new Set(),
}

// Best practice (exp-017): keep the Factorial-default core (incl.
// Budgets) + Cost centers (optional); hide the advanced/back-office
// fields; relax Vendor name + Payment method to optional.
const BEST_PRACTICE_PRESET: FormPreset = {
  hidden: new Set([
    "subcategory",
    "documentNumber",
    "vendorTIN",
  ...
  optional: new Set(["vendorName", "paymentMethod"]),
  droppedSections: new Set(["tax"]),
}

// The "best practice" demo expense. `draft-exp-017` isn't a draft in
// the mock data (drafts are the 13 lowest-alert pending rows), so we
// point at a real draft — Greystone Brunch House 2. Repoint freely.
const BEST_PRACTICE_EXPENSE_ID = "draft-exp-024"

/** The best-practice expense gets the curated form; everyone else maximal. */
export function getFormPreset(rowId: string): FormPreset {
  return rowId === BEST_PRACTICE_EXPENSE_ID ? BEST_PRACTICE_PRESET : MAXIMAL_PRESET
}
...
function buildSchema(
  ...
  // Visibility + collapse combined: a field renders only when the
  // preset doesn't hide it AND (if its section is collapsible) the
  // section is expanded. Use for any field the preset can hide.
  const visGate = (id: string, section: string, base?: () => boolean) => {
    const g = gate(section, base)
    return (): boolean => !preset.hidden.has(id) && g()
  }
  ...
  return z.object({
    ...
    subcategory: f0FormField.select({
      ...
      section: "expense",
      optional: true,
      renderIf: visGate("subcategory", "expense"),
    ...
    documentNumber: f0FormField.text({
      ...
      section: "document",
      renderIf: visGate("documentNumber", "document"),
    ...
    vendorName: f0FormField.text({
      ...
      section: "document",
      row: "vendor",
      optional: preset.optional.has("vendorName"),
      renderIf: visGate("vendorName", "document"),
      placeholder: "As it is on the document",
    }),
    vendorTIN: f0FormField.text({
      ...
      section: "document",
      row: "vendor",
      renderIf: visGate("vendorTIN", "document"),
    ...
    paymentMethod: f0FormField.select({
      ...
      section: "payment",
      row: "pay-method",
      optional: preset.optional.has("paymentMethod"),
      placeholder: "Select option",
    ...
    reimbursableCurrency: f0FormField.select({
      ...
      optional: true,
      placeholder: "Currency",
      renderIf: visGate("reimbursableCurrency", "payment", () => !isCompanyPaid),
      options: CURRENCIES.map((c) => ({ value: c, label: c })),
    }),
    ...
    project: projectRequired
      ? f0FormField(
          z.enum(projects.map((p) => p.id) as [string, ...string[]]),
          {
            ...
            fieldType: "custom",
            placeholder: "Select option",
            renderIf: visGate("project", "budget"),
    ...
    // All-optional → collapsible (gated). When review mode makes
    // Description required, `additional` drops out of collapsibleSections
    // and gate("additional") returns the base (always shown / open).
    description: descriptionRequired
      ? f0FormField.textarea({
          label: "Description",
          section: "additional",
          renderIf: gate("additional"),
    ...
    internalReference: f0FormField.text({
      ...
      section: "additional",
      optional: true,
      renderIf: visGate("internalReference", "additional"),
...
export function SubmitterEditForm(props: {
  ...
  const collapsibleSections = useMemo(() => {
    // A section is collapsible (and starts collapsed) ONLY when every
    // field in it is optional — mirroring the asterisks the form shows.
    // Sections that carry a required field stay open (no toggle):
    //   - expense   → Document type is required
    //   - document  → Document date / number / Vendor name / TIN required
    //   - payment   → Payment / Payment method / Reimbursable amount required
    //   - attendees → always shown (Meals only); never collapsed
    // All-optional sections collapse: Tax, Cost centers, Files always;
    // Budget unless Project is gated-required; Additional unless
    // Description is gated-required (review mode).
    const set = new Set<string>(["tax", "costcenters", "files"])
    if (!requiredSet.has("project")) set.add("budget")
    if (!requiredSet.has("description")) set.add("additional")
    return set
  }, [requiredSet])
  ...
  const effectiveSections = useMemo(() => {
    ...
    // Drop sections the preset fully hides (e.g. Tax on exp-017), so
    // there's no empty collapsed header.
    for (const id of getFormPreset(props.row.id).droppedSections) {
      delete base[id]
    }
  ...
  return (
    // Zero the top padding on the FIRST section header only, so the
    // form's first title aligns with the top of the page beside it.
    // F0Form's SectionRenderer hard-codes `py-5` on every section
    // header; composer doesn't JIT Tailwind for prototypes, so this is
    // a plain scoped CSS rule targeting just the first section's header
    // (form → first section wrapper → section → header div).
    <div className="expense-edit-form-top-align">
      <style>{
        ".expense-edit-form-top-align form > div:first-child > section > div:first-child { padding-top: 0 !important; }"
      }</style>
