...
> = {
  ...
  // Mileage reimbursements are usually backed by a fuel receipt — a
  // petrol-station ticket reads as the most plausible artifact.
  Mileage: {
    es: [
      ["GASOLINA SP95", 0.62],
      ["DIESEL E10", 0.55],
  ...
  // Per diem is a daily travel allowance — the "receipt" reads as an
  // itemised allowance breakdown rather than a store ticket.
  "Per diem": {
    es: [
      ["DIETA COMPLETA", 0.55],
      ["MANUTENCION", 0.25],
      ["ALOJAMIENTO", 0.3],
      ["GASTOS LOCOMOCION", 0.12],
    ],
    en: [
      ["DAILY ALLOWANCE", 0.55],
      ["MEALS ALLOWANCE", 0.25],
      ["LODGING ALLOWANCE", 0.3],
...
/**
 * Vendor → receipt template key. The receipt should reflect the
 * concrete VENDOR identity, not the accounting category: a cab
 * company prints a taxi receipt even when finance later codes the
 * row as "Mileage", and a restaurant prints a meal ticket even if
 * the category drifted to "Shopping". Driving off the provider keeps
 * the artwork coherent with what the expense obviously *is*.
 *
 * Every fixture provider is listed; unknown providers (e.g. chat-
 * created drafts) fall back to the category mapping in
 * `resolveTemplateKey`.
 */
const PROVIDER_TYPE: Record<string, string> = {
  "Birch & Beam Café": "Meals",
  "Lakeside Bistro": "Meals",
  "Tide Lane Eatery": "Meals",
  "Copperleaf Kitchen": "Meals",
  "Marble & Mint Diner": "Meals",
  "Roving Fork Co.": "Meals",
  "Greystone Brunch House": "Meals",
  "Quickline Cabs Ltd.": "Taxis",
  "Northwind Taxi Co.": "Taxis",
  "Skylane Rides": "Taxis",
  "Citystream Transit": "Taxis",
  "Wayloop Transport": "Taxis",
  "Skyfern Airways": "Travel",
  "Silverpath Rail": "Travel",
  "Junction Rail Lines": "Travel",
  "The Brindle Hotel": "Hotel",
  "Fernlight Suites": "Hotel",
  "Cobblestone Inn": "Hotel",
  "Northway Stores": "Shopping",
  "Halcyon Goods": "Shopping",
  "Pellon Office Supplies": "Office",
  "Axiom Labs Inc.": "Software",
  "Quanta Studio": "Software",
  "Veridian Soft Co.": "Software",
}

/**
 * Resolve which item template a row's receipt should use. Provider
 * identity wins (see `PROVIDER_TYPE`); otherwise fall back to the
 * row's category if we have a template for it, else Shopping. Strips
 * a disambiguating numeric suffix ("Greystone Brunch House 2") so
 * such rows still match the base provider.
 */
function resolveTemplateKey(row: SpendingRow): string {
  const baseName = row.name.replace(/\s+\d+$/, "")
  const byProvider = PROVIDER_TYPE[row.name] ?? PROVIDER_TYPE[baseName]
  if (byProvider) return byProvider
  if (ITEM_TEMPLATES[row.description]) return row.description
  return "Shopping"
}
 ...
 */
function pickLanguage(row: SpendingRow, templateKey: string): Lang {
  ...
  if (templateKey === "Meals" || templateKey === "Taxis") {
...
function formatTime(seed: number): string {
  ...
  // Unsigned shift: the hash can exceed 2^31, and a signed `>>` would
  // flip it negative → "17:-39". `>>>` keeps minutes in 0..59.
  const m = (seed >>> 4) % 60
...
export function buildReceiptDataUrl(row: SpendingRow): string {
  const seed = hash(row.id)
  // The receipt's content is driven by the VENDOR (a cab company
  // always prints a taxi receipt), not the accounting category, which
  // may have drifted or been re-coded to Mileage/Per diem.
  const templateKey = resolveTemplateKey(row)
  const lang = pickLanguage(row, templateKey)
  ...
  // When the row carries the "Receipt looks unreadable" alert
  // (`receipt-invalid`), the artwork should actually be illegible so the
  // approver sees what the alert is talking about: we smear + fade the
  // printed content (blur + turbulence displacement + low opacity) while
  // leaving the paper sheet intact, so it reads as a botched scan / faded
  // thermal print rather than a clean ticket.
  const hasInvalid = row.alerts.includes("receipt-invalid")
  ...
  const displayedTotal = hasMismatch
    ...
    : row.amount
  const items = buildItems(templateKey, displayedTotal, seed, lang)
  ...
  const baseRate = templateKey === "Meals" ? 0.1 : 0.21
  ...
  // A table number only makes sense on a restaurant ticket — it would
  // read as nonsense on a taxi/hotel/software receipt. `>>>` keeps it
  // positive (a signed shift on the >2^31 hash gave "MESA -34").
  const showTable = templateKey === "Meals"
  ...
  <defs>
    ...
    <filter id="unreadable" x="-5%" y="-5%" width="110%" height="110%">
      <feGaussianBlur in="SourceGraphic" stdDeviation="1.8" result="blur" />
      <feTurbulence type="fractalNoise" baseFrequency="0.012 0.05" numOctaves="2" seed="${seed % 100}" result="noise" />
      <feDisplacementMap in="blur" in2="noise" scale="6" xChannelSelector="R" yChannelSelector="G" />
    </filter>
  </defs>
  <g transform="rotate(${skew} ${VBW / 2} ${VBH / 2})" filter="url(#paperShadow)">
    ...
    <g transform="translate(${OUTER} ${contentTop - OUTER})"${hasInvalid ? ` filter="url(#unreadable)" opacity="0.45"` : ""}>
