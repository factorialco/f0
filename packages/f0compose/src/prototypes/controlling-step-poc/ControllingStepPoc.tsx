...
import { useAddRelatedReceiptsAction } from "./copilot/useAddRelatedReceiptsAction"
import { useSubmitWithOneAction } from "./copilot/useSubmitWithOneAction"
import { useBulkCreateExpensesAction } from "./copilot/useBulkCreateExpensesAction"
import { useCreateMileageAction } from "./copilot/useCreateMileageAction"
import { useCreatePerDiemAction } from "./copilot/useCreatePerDiemAction"
import { useGroupExpensesAction } from "./copilot/useGroupExpensesAction"
import { useFixExpenseAction } from "./copilot/useFixExpenseAction"
import { useEditExpenseAction } from "./copilot/useEditExpenseAction"
...
import { markExpenseStatus } from "./shared/statusOverrides"
...
export default function ControllingStepPoc() {
  ...
  // tools.
  // Chat action registration order = match priority (the runtime
  // matches the FIRST registered action whose matcher passes). So we
  // register SPECIFIC matchers before GENERIC ones to keep them
  // disjoint:
  //
  //   1. Invitation actions (narrow phrases, don't fabricate).
  //   2. Edit-via-One — only matches while a draft is open-for-edit,
  //      so it must precede the type-creation matchers (an "update the
  //      project" edit would otherwise be caught by nothing useful).
  //   3. Mileage / Per diem — narrow type matchers (drove/km, per diem).
  //   4. Grouping — group/folder/trip/together.
  //   5. Fix / troubleshoot — attendee/description answers (or armed).
  //   6. Bulk create from receipts — the GENERIC "receipt/create/drop"
  //      matcher (closed-circuit set). Must come after the narrow ones.
  //   7. Send for approval — "send … for approval".
  useSubmitWithOneAction()
  useEditExpenseAction()
  useCreateMileageAction()
  useCreatePerDiemAction()
  // Group BEFORE the in-detail "add related receipts" invitation: the
  // grouping offer's natural accept ("yes, group them") would
  // otherwise be caught by addRelatedReceipts' "group them" clause.
  useGroupExpensesAction()
  useAddRelatedReceiptsAction()
  useFixExpenseAction()
  ...
  const PRESET_BUCKETS: Record<string, RowPredicate[]> = useMemo(() => {
    ...
    return {
      ...
      // Expenses > Control — three presets: To control (approved),
      // Controlled (controlled), Missing receipts (any row in the
      // control slice carrying the `missing-receipt` alert).
      //
      // The "Missing receipts" preset overlaps the status presets —
      // its rows are `approved` OR `controlled` that also lack a
      // receipt — so a pure status heuristic can't tell it apart and
      // would walk all 15 status siblings instead of the 5 missing-
      // receipt ones. We therefore place a `missing-receipt` bucket
      // FIRST, so opening a row that carries that alert walks only the
      // missing-receipt set (1/5, not 1/15).
      //
      // Limitation (acceptable — OneDataCollection doesn't expose the
      // selected preset to this component, so we can only infer it
      // from the opened row's signals): a missing-receipt row opened
      // from the "To control" preset will also walk the missing-
      // receipt set rather than the To-control set. In practice a
      // reviewer opening a flagged row wants to step through the other
      // flagged rows, so this errs in the useful direction.
      control: [
        (r) => r.alerts.includes("missing-receipt"),
        byStatus("approved"),
        byStatus("controlled"),
      ],
  ...
  const handleBulkAction = useCallback(
    (actionId: string, ids: string[]) => {
      ...
      if (actionId === "mark-controlled") {
        // Move the selected expenses to "controlled" (session override) so
        // the preset counters update live: "To control" drops, "Controlled"
        // rises, and any "Missing receipts" rows stay flagged (the alert is
        // receipt-based, not status-based).
        if (ids.length > 0) markExpenseStatus(ids, "controlled")
        return
      }
  ...
  // Sub-tabs currently hidden from the row (kept in the type + logic
  // so we can flip them back on — "hidden for now", not removed).
  const HIDDEN_SUB_TABS: ReadonlySet<ExpensesSubTabId> = new Set(["archive"])
  const subTabsWithNav =
    primaryTab === "expenses"
      ? EXPENSES_SUB_TABS.filter((t) => !HIDDEN_SUB_TABS.has(t.id)).map((t) => ({
          ...t,
          onClick: () => setSub(t.id),
        }))
      : []
  ...
  const onMarkControlled = useCallback(() => {
    // Flip the open expense to "controlled" (session override) so it leaves
    // "To control" and joins "Controlled" in the presets, then close.
    if (expenseId) markExpenseStatus([expenseId], "controlled")
    closeSidePanel()
  }, [expenseId, closeSidePanel])
  // Each CTA flips the open expense's status (session override) so it
  // LEAVES the current queue and lands in the next one's preset, then
  // navigates back to the list where the move is visible:
  //   Approve  → approved   (Approve → Control · To control)
  //   Mark paid → paid      (Pending payment → Ready to export · Paid)
  //   Send for approval → pending (Submit · To-Do → Submitted + Approve)
  //   Reject   → rejected   (terminal — leaves every active queue)
  //   Request changes → changes-requested (bounces to Submit · To-Do)
  const onApprove = useCallback(() => {
    if (expenseId) markExpenseStatus([expenseId], "approved")
    closeSidePanel()
  }, [expenseId, closeSidePanel])
  const onMarkPaid = useCallback(() => {
    if (expenseId) markExpenseStatus([expenseId], "paid")
    closeSidePanel()
  }, [expenseId, closeSidePanel])
  const onSendForApproval = useCallback(() => {
    if (expenseId) markExpenseStatus([expenseId], "pending")
    closeSidePanel()
  }, [expenseId, closeSidePanel])
  // `reason` is captured by the modal but not persisted in the
  // prototype (production would write it to the audit trail / notify
  // the employee). We accept it so the signature matches and a future
  // wiring can use it.
  const onReject = useCallback(
    (_reason?: string) => {
      if (expenseId) markExpenseStatus([expenseId], "rejected")
      closeSidePanel()
    },
    [expenseId, closeSidePanel]
  )
  const onRequestChanges = useCallback(
    (_reason?: string) => {
      if (expenseId) markExpenseStatus([expenseId], "changes-requested")
      closeSidePanel()
    },
    [expenseId, closeSidePanel]
  )
