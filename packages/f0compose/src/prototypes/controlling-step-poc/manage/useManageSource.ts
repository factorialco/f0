...
import { useStatusOverridesVersion } from "../shared/statusOverrides"
...
export function useManageRows(): SpendingRow[] {
  ...
  // Recompute when a status override changes (e.g. "Mark as controlled") so
  // the rows — and the preset counters derived from them — update live.
  const overridesVersion = useStatusOverridesVersion()
  return useMemo(() => {
    return [
...
export function useManageSource(args: {
  ...
  onSyncClick?: () => void
  /**
   * Live count of currently-selected rows, read at toolbar-render time. When
   * > 0 the "Bulk control" toolbar button hides (the bottom bulk-action bar
   * takes over). Supplied by the parent, which tracks it via the
   * OneDataCollection `onSelectItems` callback.
   */
  getSelectionCount?: () => number
  /**
   * Open the bulk-controlling flow for the given rows. The toolbar "Bulk
   * control" button calls this with every uncontrolled row on the table (no
   * selection ⇒ act on all), routing into the same inline bulk-edit editor the
   * bottom action bar's "Bulk edit" opens.
   */
  onBulkControl?: (ids: string[]) => void
}) {
  const {
    rows,
    variant,
  ...
  const presetCounts = useMemo(() => {
    ...
    if (variant === "pending-controlling") {
      return {
        ...
        // Company-card expenses in this control slice that have no
        // receipt attached — surfaced as a one-click triage chip so
        // finance can chase the missing documentation.
        missingReceipt: tabRows.filter((r) =>
          r.alerts.includes("missing-receipt")
        ).length,
      }
    }
    if (variant === "pay") {
  ...
  // Filter option lists derived from the data actually present, so the
  // Category / Owner dropdowns never offer a value that matches nothing.
  // Category = distinct expense categories; Owner = distinct owning
  // employees resolved to their display name. Both computed from the
  // full manage dataset so the option set is stable across tabs.
  const categoryOptions = useMemo(() => {
    const present = new Set<string>()
    for (const r of rows) if (r.kind === "expense") present.add(r.description)
  ...
  const ownerOptions = useMemo(() => {
    ...
    for (const r of rows) {
      if (r.kind === "expense" && r.ownerEmployeeId) ids.add(r.ownerEmployeeId)
    }
  ...
  return useDataCollectionSource<SpendingRow>(
    {
        ...
        switch (variant) {
          ...
          case "pending-controlling":
            return [
              ...
              {
                // Company-card expenses missing a receipt — drives the
                // Compliance `missing-receipt` facet, scoped to the
                // control base-status slice (approved + controlled) by
                // STATUS_BY_TAB.
                label: "Missing receipts",
                filter: { alerts: ["missing-receipt"] },
                itemsCount: () =>
                  (presetCounts as { missingReceipt?: number } | null)
                    ?.missingReceipt ?? 0,
              },
            ]
          case "pay":
      ...
      dataAdapter: {
        ...
        fetchData: ({ filters, search, sortings, pagination }) => {
          ...
          const alertFilter = Array.isArray(filters?.alerts)
            ? (filters.alerts as Array<
                "has-alerts" | "no-alerts" | "missing-receipt"
              >)
            : []
          const wantsAlerts = alertFilter.includes("has-alerts")
          const wantsClean = alertFilter.includes("no-alerts")
          const wantsMissingReceipt = alertFilter.includes("missing-receipt")
          // The Compliance filter is OR-combined: a row matches if it
          // satisfies ANY selected facet. "Missing receipt" is a
          // specific alert facet that lives alongside the broad
          // has-alerts/no-alerts toggles, so we build a predicate list
          // and keep rows matching at least one. Selecting nothing (or
          // both has/no, which is a no-op) leaves rows untouched.
          const broadIsNoOp =
            alertFilter.length === 0 || (wantsAlerts && wantsClean)
          const facetPredicates: Array<(r: SpendingRow) => boolean> = []
          if (!broadIsNoOp && wantsAlerts) {
            facetPredicates.push((r) => r.alerts.length > 0)
          }
          if (!broadIsNoOp && wantsClean) {
            facetPredicates.push((r) => r.alerts.length === 0)
          }
          if (wantsMissingReceipt) {
            facetPredicates.push((r) => r.alerts.includes("missing-receipt"))
          }
          let scoped =
            facetPredicates.length > 0
              ? rows.filter((r) => facetPredicates.some((p) => p(r)))
              : rows
          // Category filter — match on the row's expense category
          // (SpendingRow.description). Folders carry a summary string
          // here, so they drop out whenever a category is selected.
          const categorySel = Array.isArray(filters?.category)
            ? (filters.category as string[])
            : []
          if (categorySel.length > 0) {
            scoped = scoped.filter(
              (r) => r.kind === "expense" && categorySel.includes(r.description)
            )
          }
          // Owner filter — match on the resolved owning employee.
          // Folders have no owner, so they drop out when an owner is
          // selected.
          const ownerSel = Array.isArray(filters?.owner)
            ? (filters.owner as string[])
            : []
          if (ownerSel.length > 0) {
            scoped = scoped.filter(
              (r) =>
                r.kind === "expense" &&
                r.ownerEmployeeId != null &&
                ownerSel.includes(r.ownerEmployeeId)
            )
          }
      ...
      // Toolbar PRIMARY action — the red "Sync to Business Central" CTA on the
      // right of the toolbar (after the search/filter icons + divider). Shown
      // on every Approve & Pay view since finance re-syncs from any of them.
      primaryActions: () => ({
        label: "Sync to Business Central",
        icon: ArrowCycle,
        onClick: () => onSyncClick?.(),
      }),
      // Toolbar SECONDARY action — only on the controlling tab: a "Bulk control"
      // button sitting just left of Sync. With NOTHING selected, it controls
      // every uncontrolled row currently in the tab ("control all on the
      // table"). When rows ARE selected it hides — the bottom bulk-action bar
      // ("Mark as controlled" / "Bulk edit") takes over the scoped work.
      secondaryActions: {
        expanded: 1,
        actions: () => {
          if (variant !== "pending-controlling") return []
          if ((getSelectionCount?.() ?? 0) > 0) return []
          return [
            {
              label: "Bulk control",
              icon: Pencil,
              onClick: () => {
                // No selection ⇒ "control all on the table": open the bulk-
                // controlling editor for every uncontrolled row in this tab.
                const ids = tabRows
                  .filter((r) => r.kind === "expense" && r.status === "approved")
                  .map((r) => r.id)
                if (ids.length > 0) onBulkControl?.(ids)
              },
            },
          ]
        },
      ...
      bulkActions: (selected) => {
        ...
        if (variant === "pending-controlling") {
          // BR-003 / BR-005: Mark as controlled vs Bulk edit are distinct
          // affordances. Bulk edit opens the (inline) controlling editor; Mark
          // as controlled is the no-form action. Keep BOTH available for ANY
          // selection — including select-all and folder+expense mixes — so the
          // bulk-controlling controls never collapse to just Reject. Folders in
          // the selection are aggregates; the actions act on their expenses.
          return {
            // Label "Bulk control" to match the toolbar button; id stays
            // "bulk-edit" so it still routes into the inline controlling editor.
            secondary: [{ id: "bulk-edit", label: "Bulk control", icon: Pencil }],
            primary: [
              {
                id: "mark-controlled",
