...
export function ManageTab(props: {
  ...
  // Live selection count — drives whether the "Bulk control" toolbar button
  // shows (0 selected) or hides (≥1 selected, bottom action bar takes over).
  // A ref so the source's toolbar closure reads the live value; a tick state
  // so a selection change re-renders OneDataCollection and re-evaluates it.
  ...
  const source = useManageSource({
    ...
    getSelectionCount: () => selCountRef.current,
    // Toolbar "Bulk control" (no selection) → open the bulk-controlling flow
    // for all rows, same as the bottom bar's "Bulk edit".
    onBulkControl: (ids) => props.onBulkAction("bulk-edit", ids),
  })
        ...
        visualizations={[
          {
            ...
            options: {
              ...
              //   - Finance control (pending-controlling) → the full
              //     accounting coding surface (subcategory, cost
              //     center, project, VAT, description) + alerts, with
              //     the VENDOR name first ("Name").
              //   - Approve (pending-approval) → owner-first AND the
              //     Alerts column (the whole point of the queue is
              //     triaging policy signals), via `ownerAlertColumns`.
              //   - Pending payment / Ready to export (pending-payment /
              //     all) → owner-first lighter layout (no Alerts — those
              //     personas triage by who submitted, post-approval).
              //   - Archive (hidden in the IA today) keeps the vendor
              //     name via `spendingColumns`.
              columns: [
                ...(props.variant === "pending-controlling"
                  ? controllingColumns
                  : props.variant === "pending-approval"
                    ? ownerAlertColumns
                    : props.variant === "pending-payment" ||
                        props.variant === "all"
                      ? ownerColumns
                      : spendingColumns),
              ],
