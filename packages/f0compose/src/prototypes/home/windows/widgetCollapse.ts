...
import { windowIds, type WindowId } from "./types"
...
function read(profile: ProfileId): string {
  try {
    return (
      localStorage.getItem(key(profile)) ?? fallback.get(profile) ?? "[]"
    )
  } catch {
...
export function useWidgetCollapse(profile: ProfileId) {
  ...
  let collapsed: WindowId[] = []
  try {
    const parsed: unknown = JSON.parse(raw)
    if (Array.isArray(parsed))
      collapsed = parsed.filter((id): id is WindowId =>
        windowIds.includes(id)
      )
  } catch {
    /* Old storage. */
  }
  const setCollapsed = (ids: WindowId[]) => {
