...
- **2026-09-14, Angel's review** (`memory/2026-09-14-angel-review.md` has the full mapping): the per-glyph hover animations are GONE (`iconMotion.ts` + `icon-motion.css` deleted); the rail is **68px** with 56x68 targets, no gap between items, a 24px glyph (f0's `lg`, not `md` — at 20 the mark is 12px inside a 36px chip), filled glyphs for the active item from `navigation/filledRailIcons.tsx` (f0 ships no filled set; these are its own outlines redrawn solid), and the tint on a **36x36 square chip** around the glyph (Slack's own rail, measured: 70px rail, 52x68 buttons, 36x36 chip at radius 8, 20px glyph, 11/12 label 4px below); labels are Home / DMs / Inbox / Calendar / **Files** / Tools (ids unchanged — they are persisted and `agent-entry.css` selects on `[data-nav-section]`); **every** section owns a `?view=` now, so a rail click always changes the canvas (`SECTION_VIEW`/`VIEW_SECTION`); Tools rows can be **pinned to the rail** (`railPinsStore`, cap 3) and a small plan **promotes its modules in place of Tools** (`planStore`, switch in Preferences); Notifications folded into the Inbox behind `Chip` presets (All / Requests / Notifications, preset in the URL), Marketplace moved into Tools › More, Help became "Get help" in the profile menu, and the rail logo is the **entity switcher** (`navigation/entityStore`); Messages and Inbox do not offer collapse; Home's panel lost the Activity header button (it is a row) and its Files/Reports rows became **Artifacts** (`?view=artifacts`).
...
- `hub/` — **only `hubSlug.ts` now**: the label → `?view=` slug that HomeNav's Hub rows navigate by, kept as a dependency-free leaf. The rest of this directory — `hubStore`, `hubSections`, `hubPane`, `HubChrome`, `HubPaneBody`, `CursorIcon`, i.e. the Hub as a WINDOW with a two-row browser header (Figma 2787:43433) — is **SHELVED on the branch `oskar/hub-browser-header`**, because Oskar chose PAGES for People and Calendar on 2026-09-09. Start there if the window model ever returns; it was finished and verified.
...
- **Three AppShell backgrounds** (Angel, 2026-09-14, his own values — this REVERSES "one flat surface", Oskar 2026-08-29): sidebars **#F5F5F5**, page **#FAFAFA** (canvas ground, One panel, sticky headers), floating **#FFFFFF** with a secondary border and a soft shadow. Literal hexes, not f0 neutrals: f0's are navy alphas (`--neutral-5` = #F5F6F8, a cool cast) and alpha surfaces compound when stacked. `.f0c-surface-chrome` / `.f0c-surface-page` in FULL_BLEED_CSS are the classes, each with a `.dark` twin. No hairline between panel and page — the tonal step is the separation.
- **Dark mode**: the light values above are experimental customs with no dark pair (and `--neutral-3` does not flip at all — `base.css` keeps it a navy alpha, so it darkens instead of lifting; that is why the page tier was dropped). FULL_BLEED_CSS rebuilds the ground from f0 dark tokens — one formula for rail, panel, `main#content` and the One panel (`linear-gradient(hsl(var(--page)), hsl(var(--page))), hsl(var(--neutral-0))`), dividers flip to neutral-10. Scrollbars use f0's `--scrollbar-*` vars (theme-aware). Never add a raw light hex to the chrome without its `.dark` override.
...
## Done since last handoff (2026-09-09, merged with #5500 — and People/Calendar are PAGES)

Oskar: "juntar la exploracion que ha hecho un compañero (#5500) con estos ultimos cambios en los
sidebars para actualizar el link de vercel con todo" → then "usa la logica de que son paginas y
haz la secuencia". **Published**: `index-Bv3ZOBB7.js` / `index-ClPFbBHm.css`, verified live.

- **The merge was easy; the design decision was the work.** Jonathan branched from this branch's
  own tip (`2264235f5`), so the two sides were siblings and the only textual conflict was ONE
  hunk in HomeNav.tsx — where both of us had independently decided every Hub row should
  navigate. He inlined the slug, this branch had extracted `hubSlug`. Kept the extracted form.
  Eight of his eleven files are new, so they merged without touching anything.
- **PAGES won, and that had to be finished rather than left half-done.** His branch neutered the
  window model with `const windowView = null`, which left the pane reconciliation, `ModulePane`
  and the left stack's module membership as unreachable code held alive by a constant. All
  removed. `?view=` is now simply a screen, with his `ModuleScreen` as the honest fallback.
- **`?view=organization` renders PeopleScreen.** His branch could not know the admin Hub's row
  had been renamed from People to Organization, and that screen IS Organization › People — with
  no mapping the prototype's one real Hub destination would have landed on the generic empty
  state. Verified live: 24 rows, zero window panes.
- **The browser-header work is shelved, not deleted** — `oskar/hub-browser-header`, with its own
  commit message explaining why. It was verified and finished; only its premise died.
- **Deploy gotcha worth writing down:** `~/code/my-project/vercel.json` has
  `outputDirectory: null`, so Vercel serves the project ROOT. Copying the build into
  `my-project/dist` changes nothing that is served — `index.html` and `assets/` go at the root.
  Caught by reading `vercel.json` and the root `index.html` before deploying, not after.
- Everything from the four sidebar frames shipped with it. Verified on the LIVE site: the Hub
  panel's six groups and 27 rows with no iconless row, Inventory present, the Comms panel's
  Communities section, and Jonathan's agent entry contracted at the bottom.
...
> **SUPERSEDED 2026-09-09.** People and Calendar are DESTINATION PAGES again — Oskar's call when
> merging #5500, which had already reverted them. The window machinery below (`ModulePane`, the
> left stack's module membership, Home's pane reconciliation) is deleted, not disabled. Read this
> section as history; the reasoning about stacking and one-column-one-width is still the best
> record of why a module-as-pane behaves the way it does, if it ever comes back.
