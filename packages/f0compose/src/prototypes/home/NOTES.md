...
- **The chrome is ONE GROUND plus what floats on it** (Angel, 2026-09-14): rail, panel, page and the One panel all `--neutral-5` (≈#F5F6F8); module sheets, windows, cards and menus white with a secondary border and a soft shadow. Composited over white, never applied as alpha, so a surface on a surface cannot darken twice. A first pass split the page onto `--neutral-3` and he read the seam as "un cambio de color raro" — do not reintroduce it. The only hairline left is between rail and panel. `.f0c-surface-chrome` / `.f0c-surface-page` in FULL_BLEED_CSS are the classes (same value, kept separate so a future split has somewhere to land).
