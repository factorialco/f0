...
- **The chrome is a THREE-TIER ramp** (Angel, 2026-09-14 — this REVERSES "one flat surface", Oskar 2026-08-29, "que no tenga color, el mismo fondo que Needs you"; his reason: a grey page "invierte la relación entre background y contenido"). Sidebars `--neutral-5` (≈#F5F6F8), page `--neutral-3` (≈#FAFAFB), floating white + secondary border + shadow — all composited over white, never applied as alpha, so a surface on a surface cannot darken twice. The hairline survives only BETWEEN rail and panel, which share a tier; panel against page is a tonal step, and adding a `--neutral-10` seam there would be darker than either side. `.f0c-surface-chrome` / `.f0c-surface-page` in FULL_BLEED_CSS are the classes.
- **Dark mode**: the light values above are experimental customs with no dark pair, AND `--neutral-3` does not flip in dark (`base.css` keeps it a navy alpha, so it would darken instead of lift) — so FULL_BLEED_CSS rebuilds the ramp rather than reusing it: chrome is `hsl(var(--neutral-0))`, page is that lifted by `--page` (`linear-gradient(hsl(var(--page)), hsl(var(--page))), hsl(var(--neutral-0))`), dividers flip to neutral-10. Scrollbars use f0's `--scrollbar-*` vars (theme-aware). Never add a raw light hex to the chrome without its `.dark` override.
