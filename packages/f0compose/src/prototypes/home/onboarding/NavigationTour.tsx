  ...
  useEffect(() => {
    ...
    const point = async (tries = 0) => {
      ...
      target = document.querySelector<HTMLElement>(
        `[data-home-rail] button[aria-label="${steps[current].target}"]`
      )
      if (!target || !target.getClientRects().length) {
        if (tries < 20)
          timer = window.setTimeout(() => void point(tries + 1), 150)
      ...
      if (!spotlight) {
        ...
        spotlight.highlight({
          element:
            document.querySelector<HTMLElement>("[data-home-rail]") ?? target,
        })
      }
      showCursor()
    ...
    const explain = (clicked: number, tries = 0) => {
      ...
      if (
        !panel ||
        panel.getBoundingClientRect().width < 20 ||
        panel.hasAttribute("inert")
      ) {
        if (tries < 20)
          timer = window.setTimeout(() => explain(clicked, tries + 1), 150)
        else {
          waiting = false
          setMissing(true)
        }
      ...
      spotlight.highlight({
        ...
        popover: {
          ...
          side: "right",
          align: "center",
          showButtons: last ? ["next"] : [],
          nextBtnText: "Set up Home",
          onNextClick: finish,
        },
      })
      current = clicked + 1
    ...
    const waitForMenu = (clicked: number) => {
      ...
      const inspect = () => {
        ...
        const ready =
          ...
          rect.width > 20 &&
          !panel.hasAttribute("inert") &&
          panel.dataset.navSection === expected
        const geometry = rect
        ...
        stableFrames =
          ready && !moving && geometry === previous ? stableFrames + 1 : 0
        previous = geometry
        if (stableFrames >= 4) {
          explain(clicked)
          return
    ...
    >
      ...
      >
        ...
        <F0Box data-home-tour-controls display="flex" flexWrap="wrap" gap="md">
          <F0Button
            label="Skip tour and set up Home"
            variant="outline"
            onClick={finish}
          />
          <F0Button
            label={state.tourPaused ? "Resume tour" : "Pause tour"}
            variant="ghost"
            onClick={() =>
              updateOnboarding(profile, { tourPaused: !state.tourPaused })
            }
          />
          {missing && (
            <F0Button
