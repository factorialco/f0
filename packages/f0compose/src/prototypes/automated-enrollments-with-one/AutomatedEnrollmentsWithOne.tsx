...
type ExactCourse = Omit<Training, "objectives" | "courseType" | "validity"> & {
...
export const meta: PrototypeMeta = {
  slug: "automated-enrollments-with-one",
  title: "Automated Enrollments with ONE",
  description:
    "Trainings enrollments flow rebuilt on top of the ONE layer — exploring automated enrollment management with ONE components.",
  category: "Talent",
  module: "trainings",
  audience: ["admin"],
  tags: ["training", "courses", "enrollments", "admin", "one"],
...
const moduleInfo = {
  ...
  name: "Trainings",
  href: "/p/automated-enrollments-with-one",
}

const routes = {
  home: "/p/automated-enrollments-with-one",
  courses: "/p/automated-enrollments-with-one",
  budgets: "/p/automated-enrollments-with-one?tab=budgets",
  surveyTemplates: "/p/automated-enrollments-with-one?sub=survey-templates",
  myCourses: "/p/automated-enrollments-with-one?view=free-course",
  course: (courseId: string, tab: CourseDetailTabId = "overview") =>
    `/p/automated-enrollments-with-one?view=detail&course=${encodeURIComponent(courseId)}&dtab=${tab}`,
  group: (courseId: string, groupName: string, tab: GroupDetailTabId = "sessions") =>
    `/p/automated-enrollments-with-one?view=group-detail&course=${encodeURIComponent(courseId)}&group=${encodeURIComponent(groupName)}&gtab=${tab}`,
  budget: (budgetId: string) =>
    `/p/automated-enrollments-with-one?view=budget-detail&budget=${encodeURIComponent(budgetId)}`,
}
...
export default function AutomatedEnrollmentsWithOne() {
  ...
  // Builds the course, adds it to state and returns its id WITHOUT navigating,
  // so the ONE chat flow can keep the conversation open and offer next steps.
  const createCourseDraft = (values: NewCourseValues): string => {
    ...
    const assignment: "direct" | "waitlist" = values.enrollmentAssignment === "direct" ? "direct" : "waitlist"
    const enrollmentRule = autoEnroll
      ? {
          // A brand-new course has no history, so the rule enrolls everyone who
          // already matches plus future joiners — there's no appliesTo choice at creation.
          criteria,
          appliesTo: "everyone" as const,
          matchCount,
          assignment,
          ...(assignment === "waitlist" ? { waitlisted: matchCount } : {}),
        }
      : undefined
    const validityYears = typeof values.validityYears === "number" ? values.validityYears : 1
    const validity = values.courseValidity === true ? `${validityYears} ${validityYears === 1 ? "year" : "years"}` : "No expiration"
  ...
  const openCourse = (courseId: string, dtab?: string) => {
    setSearchParams({ view: "detail", course: courseId, ...(dtab ? { dtab } : {}) })
  }

  const handleCreateCourse = (values: NewCourseValues) => {
    openCourse(createCourseDraft(values))
  }
  ...
  if (view === "new-course") {
    return (
      <OneCourseCreateView
        ...
        onCreateCourseDraft={createCourseDraft}
...
type NewCourseSectionId = "details" | "enrollment" | "review"

const newCourseSectionIds: NewCourseSectionId[] = ["details", "enrollment", "review"]
const newCourseSectionLabels: Record<NewCourseSectionId, string> = {
...
// ---------------------------------------------------------------------------
// ONE course creation — full-width chat flow
// ---------------------------------------------------------------------------
// Clicking "Create course" opens ONE at full width. The primary path is
// conversational: describe the course, ONE drafts it, you confirm, and a
// single clarifying question settles who gets enrolled. A "From scratch" card
// drops into the manual editor (FromScratchCourseEditor) as a fallback.
...
type CourseDraft = {
  ...
  courseValidity: boolean
  validityYears: number
  reasoning: string
}

const courseSuggestions: { icon: ComponentType<{ className?: string }>; text: string }[] = [
  { icon: Sparkles, text: "Mandatory onboarding for every new engineer, valid for 1 year" },
  { icon: People, text: "Annual cybersecurity awareness for the whole company, renewed yearly" },
  { icon: ChartLine, text: "Leadership program with editions and live sessions for managers" },
]

function draftFromPrompt(prompt: string): CourseDraft {
  ...
  if (lower.includes("onboarding")) {
    return {
      ...
      validityYears: 1,
      reasoning:
        "This reads like a recurring requirement for new joiners, so I set it up as a self-paced course (no editions), marked it mandatory, and gave it a 1-year validity so people retake it on schedule.",
    }
  }

  if (lower.includes("cybersecurity") || lower.includes("security") || lower.includes("compliance")) {
    return {
      ...
      validityYears: 1,
      reasoning:
        "Company-wide compliance training works best as a mandatory, self-paced course that everyone renews every year, so I enabled validity at 1 year.",
    }
  }

  if (lower.includes("leadership") || lower.includes("manager") || lower.includes("session") || lower.includes("edition")) {
    return {
      ...
      validityYears: 1,
      reasoning:
        "Cohort-based programs with live sessions map to a course with editions and scheduled sessions. I left it optional and without an expiry date.",
    }
  }

  return {
    name: prompt.trim().length > 0 ? prompt.trim().replace(/\.$/, "").slice(0, 60) : "New course",
    courseType: "no-editions",
    hasSessions: "no",
    mandatoryCourse: false,
    courseValidity: false,
    validityYears: 1,
    reasoning:
      "I set up a self-paced course as a starting point. You can refine the type, validity and whether it's mandatory at any time.",
  }
}

// Audience options for the "Who should be enrolled?" clarifying step. Each maps
// to the values shape consumed by handleCreateCourse.
const audienceChoices: {
...
}[] = [
  {
    ...
    description: "Auto-enroll all employees, now and as new people join.",
    enableAutoEnrollment: true,
    audienceCriteria: [["wp-bcn", "wp-mad", "wp-remote", "wp-lon", "wp-ber"]],
  },
  {
    id: "engineering",
    label: "Engineering team",
    description: "Auto-enroll everyone in Engineering.",
    enableAutoEnrollment: true,
    audienceCriteria: [["tm-eng"]],
  },
  {
    id: "managers",
    label: "Managers and team leads",
    description: "Auto-enroll people in a management role.",
    enableAutoEnrollment: true,
    audienceCriteria: [["rl-manager", "rl-lead"]],
  },
  {
    id: "manual",
    label: "No one yet — I'll add people manually",
    description: "Create the course empty and enroll participants later.",
    enableAutoEnrollment: false,
    audienceCriteria: [],
  },
]

// ONE textarea — replica of the F0AiChat input with the conic-gradient glow.
function OneCourseTextarea({
  value,
  onChange,
  onSubmit,
  placeholder,
  ...
  return (
    <div className="relative isolate flex flex-col items-stretch gap-2 rounded-lg border border-solid border-f1-border p-0 transition-all hover:cursor-text before:pointer-events-none before:absolute before:inset-0 before:z-[-1] before:rounded-[inherit] before:bg-f1-background before:content-[''] after:pointer-events-none after:absolute after:inset-0.5 after:z-[-2] after:scale-90 after:rounded-md after:bg-[conic-gradient(from_0deg,#E55619,#A1ADE5,#E51943,#E55619)] after:opacity-0 after:blur-[6px] after:transition-all after:delay-200 after:duration-300 after:content-[''] focus-within:border-f1-background-tertiary focus-within:after:scale-100 focus-within:after:opacity-100">
      ...
      <div className="flex shrink-0 items-center justify-between p-3">
        <div />
        <button
          onClick={() => hasData && onSubmit()}
          disabled={!hasData}
          className={`flex h-8 w-8 items-center justify-center rounded-full ${hasData ? "bg-f1-foreground text-f1-foreground-inverse" : "bg-f1-background-tertiary text-f1-foreground-disabled"}`}
        >
...
// Right-aligned user bubble — matches the real F0AiChat UserMessage.
function OneUserBubble({ content }: { content: string }) {
  return (
    <div className="my-4 flex w-full flex-col items-end gap-2 first:mt-0 last:mb-0">
      <div className="w-fit max-w-[90%] self-end whitespace-pre-wrap rounded-2xl border border-solid border-f1-border-secondary bg-f1-background-tertiary px-4 py-3">
        <F0Text variant="body" content={content} />
      </div>
    </div>
  )
}

type CoursePhase = "welcome" | "thinking" | "draft" | "clarifying" | "done" | "scratch"

function OneCourseCreateView({
  onCancel,
  onCreateCourse,
  onCreateCourseDraft,
  onOpenCourse,
  ...
  onCreateCourseDraft: (values: NewCourseValues) => string
  onOpenCourse: (courseId: string, dtab?: string) => void
  onToast: (toast: ToastId) => void
}) {
  const [phase, setPhase] = useState<CoursePhase>("welcome")
  ...
  const runPrompt = (text: string) => {
    ...
    setPhase("thinking")
    window.setTimeout(() => {
      setDraft(draftFromPrompt(trimmed))
      setPhase("draft")
    }, 2200)
  }

  const confirmDraft = () => setPhase("clarifying")

  // Create the course but stay in the chat so ONE can offer next steps.
  const finishCreate = () => {
    if (!draft) return
    const choice = audienceChoices.find((c) => c.id === selectedAudience) ?? audienceChoices[3]
    onToast("draft")
    const id = onCreateCourseDraft({
      name: draft.name,
      courseType: draft.courseType,
      hasSessions: draft.hasSessions,
      mandatoryCourse: draft.mandatoryCourse,
      courseValidity: draft.courseValidity,
      validityYears: draft.validityYears,
      enableAutoEnrollment: choice.enableAutoEnrollment,
      audienceCriteria: choice.audienceCriteria,
      enrollmentAssignment: draft.courseType === "with-editions" ? "waitlist" : "direct",
  ...
  if (phase === "scratch") {
    return <FromScratchCourseEditor onCancel={onCancel} onCreateCourse={onCreateCourse} onToast={onToast} />
  }
    ...
    <Page
      header={
        <PageHeader
          ...
          actions={[{ label: "Cancel", icon: Cross, onClick: onCancel }]}
        />
      }
    >
      <StandardLayout>
        <div className="mx-auto flex h-full w-full max-w-[712px] flex-1 flex-col">
          {phase === "welcome" && (
            <OneCourseWelcome
              prompt={prompt}
              onPromptChange={setPrompt}
              onSubmit={runPrompt}
              onScratch={() => setPhase("scratch")}
            />
          )}

          {phase === "thinking" && (
            <div className="flex flex-1 flex-col justify-end gap-4 pb-3">
              <OneUserBubble content={prompt} />
              <div className="flex items-center gap-2">
                <F0OneIcon size="sm" className="animate-pulse" />
                <F0Text variant="body" content="Thinking…" />
              </div>
            </div>
          )}

          {phase === "draft" && draft && (
            <OneCourseDraft draft={draft} prompt={prompt} onConfirm={confirmDraft} onCancel={() => setPhase("welcome")} />
          )}

          {phase === "clarifying" && draft && (
            <OneCourseClarifying
              draft={draft}
              prompt={prompt}
          ...
          {phase === "done" && draft && createdCourseId && (
            <OneCourseNextSteps
              draft={draft}
              onOpenCourse={(dtab) => onOpenCourse(createdCourseId, dtab)}
            />
          )}
        </div>
      </StandardLayout>
...
// Phase 5 — course created: ONE confirms and offers the next steps.
function OneCourseNextSteps({
  draft,
  onOpenCourse,
}: {
  draft: CourseDraft
  onOpenCourse: (dtab?: string) => void
}) {
  const nextSteps: { id: string; label: string; description: string; icon: ComponentType<{ className?: string }>; dtab?: string }[] = [
    { id: "content", label: "Create content", description: "Add modules and lessons to the course.", icon: File, dtab: "content" },
    { id: "groups", label: "Course groups", description: "Set up editions and training groups.", icon: People, dtab: "training-groups" },
    { id: "view", label: "View the course", description: "Open the course overview.", icon: ExternalLink },
  ]

  return (
    <div className="flex flex-1 flex-col justify-end gap-4 pb-3 pt-2">
      <div className="flex w-full flex-col items-start gap-3">
        <div className="flex items-center gap-2">
          <F0Icon icon={CheckCircle} size="sm" />
          <F0Text variant="label" content="Done" />
        </div>
        <F0Text
          variant="body"
          content={`Your draft course "${draft.name}" is created. What do you want to do next?`}
        />
      </div>
      ...
      <div className="flex flex-col items-start gap-[6px]">
        {nextSteps.map((step) => (
          <button
            key={step.id}
            onClick={() => onOpenCourse(step.dtab)}
            className="flex w-full items-center gap-3 rounded-lg border border-solid border-f1-border-secondary bg-f1-background px-4 py-3 text-left shadow transition-colors hover:bg-f1-background-secondary-hover"
          >
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-f1-background-tertiary">
              <step.icon className="h-4 w-4 text-f1-foreground-secondary" />
            </div>
            <div className="flex flex-col gap-0.5">
              <F0Text variant="label" content={step.label} />
              <F0Text variant="description" content={step.description} />
            </div>
          </button>
        ))}
      </div>
...
// Phase 1 — welcome: greeting, suggestions, textarea, and a "From scratch" card.
function OneCourseWelcome({
  prompt,
  onPromptChange,
  onSubmit,
  onScratch,
  ...
  return (
    <div className="flex flex-1 flex-col justify-end gap-6 pb-3 pt-2">
      <div className="flex flex-1 flex-col justify-end gap-6">
        <div className="px-1">
          ...
          <p className="text-2xl font-semibold leading-[28px] text-f1-foreground">Let&apos;s create a new course!</p>
          <p className="mt-2 text-base font-normal text-f1-foreground-secondary">
            Describe the course you have in mind and I&apos;ll draft it for you — or start from scratch.
          </p>
        </div>
        ...
        <button
          onClick={onScratch}
          className="flex w-full items-center gap-3 rounded-lg border border-dashed border-f1-border bg-f1-background px-4 py-3 text-left transition-colors hover:bg-f1-background-secondary-hover"
        >
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-f1-background-tertiary">
            <F0Icon icon={Sliders} size="sm" />
          </div>
          <div className="flex flex-col gap-0.5">
            <F0Text variant="label" content="Create from scratch" />
            <F0Text variant="description" content="Skip the chat and fill in the course form yourself." />
          </div>
        </button>
      </div>
      ...
      <div className="pt-4">
        <OneCourseTextarea
          ...
          placeholder="e.g. 'A mandatory data-protection course everyone renews each year'"
        />
      </div>

      <div className="flex w-full flex-row items-center justify-center gap-1">
        <F0Text variant="description" content="One works within your permissions." />
      </div>
    </div>
  )
}

// Phase 3 — drafted course with Confirm / Cancel.
function OneCourseDraft({
  draft,
  prompt,
  onConfirm,
  onCancel,
  ...
  const summaryParts = [
    draft.courseType === "with-editions" ? "With editions" : "No editions",
    ...(draft.courseType === "with-editions"
      ? [draft.hasSessions === "yes" ? "With sessions" : "Without sessions"]
      : []),
    draft.mandatoryCourse ? "Mandatory" : "Optional",
    draft.courseValidity
      ? `Valid ${draft.validityYears} ${draft.validityYears === 1 ? "year" : "years"}`
      : "No expiration",
  ]
  ...
  return (
    <div className="flex flex-1 flex-col justify-end gap-4 pb-3">
      ...
      <div className="flex w-full flex-col items-start gap-3">
        <F0Text variant="body" content={draft.reasoning} />

        <div className="w-full">
          <OneCourseCanvasCard title={draft.name} description={summaryParts.join(" · ")} />
        </div>

        <div className="flex items-center gap-3 pt-1">
          <F0Button variant="default" label="Confirm" onClick={onConfirm} />
          <F0Button variant="outline" label="Cancel" onClick={onCancel} />
        </div>
      </div>
    </div>
  )
...
// Inline canvas card — replica of the F0AiChat CanvasCard idiom: a module
// avatar, title and a single-line description inside a bordered card.
function OneCourseCanvasCard({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex flex-row items-center gap-3 rounded-lg border border-solid border-f1-border-secondary px-3 py-2 transition-colors hover:border-f1-border-hover">
      <F0AvatarModule module="company_trainings" size="lg" />
      <div className="flex min-w-0 flex-1 flex-col">
        <span className="truncate text-lg font-semibold text-f1-foreground">{title}</span>
        <span className="truncate text-base text-f1-foreground-secondary">{description}</span>
      </div>
    </div>
  )
}

// Phase 4 — clarifying question: who should be enrolled?
function OneCourseClarifying({
  draft,
  prompt,
  selected,
  onSelect,
      ...
      <div className="relative isolate flex flex-col items-stretch rounded-lg border border-solid border-f1-background-tertiary p-0 transition-all before:pointer-events-none before:absolute before:inset-0 before:z-[-1] before:rounded-[inherit] before:bg-f1-background before:content-[''] after:pointer-events-none after:absolute after:inset-0.5 after:z-[-2] after:scale-100 after:rounded-md after:bg-[conic-gradient(from_0deg,#E55619,#A1ADE5,#E51943,#E55619)] after:opacity-100 after:blur-[6px] after:content-['']">
        <div className="flex flex-col gap-3 pt-3">
            ...
            {audienceChoices.map((choice) => {
              ...
              return (
                <button
                  ...
                  className="flex cursor-pointer items-start gap-2 rounded-md px-2 py-2 text-left transition-colors hover:bg-f1-background-secondary"
                >
                  <div
                    className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 border-solid transition-colors ${isSelected ? "border-f1-border-selected-bold bg-f1-background-selected-bold" : "border-f1-border bg-f1-background"}`}
                  >
                    {isSelected && <div className="h-1.5 w-1.5 rounded-full bg-f1-background" />}
                  </div>
                  <div className="flex flex-col gap-0.5">
                    <span className="text-base font-medium text-f1-foreground">{choice.label}</span>
                    <span className="text-sm text-f1-foreground-secondary">{choice.description}</span>
                  </div>
                </button>
              )
            })}
          ...
          <div className="flex items-center justify-end gap-3 p-3">
            <F0Button disabled={!selected} variant="default" label="Submit" onClick={onSubmit} />
          </div>
        </div>
      </div>

      <div className="flex w-full flex-row flex-wrap items-center justify-center gap-x-3 gap-y-1 pt-2 text-sm font-medium text-f1-foreground-tertiary">
        <span>
...
function FromScratchCourseEditor({
  onCancel,
  onCreateCourse,
  onToast,
}: {
  ...
  const [activeSection, setActiveSection] = useState<NewCourseSectionId>("details")
  ...
  const update = (field: string, value: unknown) => {
    ...
    setValues((current) => {
      ...
      // No-editions enrolls people directly into the course; with-editions has
      // no groups yet, so matches stay pending group assignment by default.
      if (field === "courseType") {
        next.enrollmentAssignment = value === "with-editions" ? "waitlist" : "direct"
      }
      return next
    })
  }

  const withEditions = values.courseType === "with-editions"
  const autoEnroll = values.enableAutoEnrollment !== false
  const groups = readAudienceGroups(values.audienceCriteria)
  const matchCount = peopleMatchingGroups(groups)
  const criteriaSelected = flattenAudienceGroups(groups).length > 0
  ...
  const handleCreate = () => {
    ...
    if (autoEnroll && !criteriaSelected) {
    ...
    <Page
      header={
        <>
          ...
          <ResourceHeader
            title="New course"
            description="Set up the course and define who takes it — with One by your side."
            status={{ label: "Status", text: "Draft", variant: "neutral" }}
            primaryAction={{ label: "Create course", onClick: handleCreate }}
            secondaryActions={[{ label: "Cancel", icon: Cross, onClick: onCancel }]}
          />
        </>
      }
    >
      <StandardLayout>
        <div className="flex gap-8">
          ...
          <div className="w-56 shrink-0">
            <F0TableOfContent
              title="Course setup"
              items={newCourseSectionIds.map((id) => ({
                id,
                label: newCourseSectionLabels[id],
          ...
          {/* Center: the form */}
          <div className="flex min-w-0 max-w-2xl flex-1 flex-col gap-8 pb-12">
            {activeSection === "details" && (
              <div className="flex flex-col gap-6">
                <div className="flex flex-col gap-2">
                  <F0Heading content="Course details" variant="heading" as="h2" />
                  <F0Text content="Basic information to identify this course." variant="body" />
                </div>
                <Input
                  label="Course name"
                  ...
                  error={nameError ? "Course name is required" : undefined}
                />
                <CourseTypeField values={values} onUpdate={update} />
                {withEditions && (
                  <div className="flex flex-col gap-3">
                    <F0Text variant="label" content="Does it have sessions?" />
                    <CardSelectableContainer
                      label="Does it have sessions?"
                      value={(values.hasSessions as string) ?? "no"}
                      onChange={(val) => update("hasSessions", val ?? "no")}
                      items={[
                        {
                          value: "yes",
                          title: "With sessions",
                          description: "Each edition runs scheduled sessions people attend.",
                        },
                        {
                          value: "no",
                          title: "Without sessions",
                          description: "Editions group people, but there are no scheduled sessions.",
                        },
                      ]}
                    />
                  </div>
                ...
                <div className="flex items-start gap-3">
                  <Switch
                    checked={values.mandatoryCourse === true}
                    onCheckedChange={(checked) => update("mandatoryCourse", checked)}
                    title="Mandatory course"
                    hideLabel
                  />
                  <div className="flex flex-col gap-0.5">
                    <F0Text variant="label" content="Mandatory course" />
                    <F0Text
                      variant="description"
                      content="Mark this course as mandatory to track completion and meet compliance requirements."
                    />
                  </div>
                </div>
                <div className="flex flex-col gap-3">
                  <div className="flex items-start gap-3">
                    <Switch
                      checked={values.courseValidity === true}
                      onCheckedChange={(checked) => update("courseValidity", checked)}
                      title="Course validity"
                      hideLabel
                    />
                    <div className="flex flex-col gap-0.5">
                      <F0Text variant="label" content="Course validity" />
                      <F0Text
                        variant="description"
                        content="This course is valid for a limited time and must be retaken afterward."
                      />
                    </div>
                  </div>
                  {values.courseValidity === true && (
                    <div className="max-w-xs">
                      <NumberInput
                        label="Valid for (years)"
                        value={typeof values.validityYears === "number" ? values.validityYears : 1}
                        onChange={(value) => update("validityYears", value ?? 1)}
                        size="md"
                        locale="en-US"
                      />
                    </div>
            ...
            {activeSection === "enrollment" && (
              <div className="flex flex-col gap-6">
                ...
                <InscripcionStep values={values} onUpdate={update} audienceError={audienceError} />
              </div>
            )}

            {activeSection === "review" && (
              <CourseReviewSummary values={values} matchCount={matchCount} />
            )}
          </div>

          {/* Right: One chat assistant */}
          <div className="w-80 shrink-0">
            <OneCourseChatSidebar values={values} matchCount={matchCount} />
          </div>
        </div>
      </StandardLayout>
    </Page>
...
// Review section — a plain read-out of the current configuration.
function CourseReviewSummary({ values, matchCount }: { values: Record<string, unknown>; matchCount: number }) {
  const withEditions = values.courseType === "with-editions"
  const autoEnroll = values.enableAutoEnrollment !== false
  const groups = readAudienceGroups(values.audienceCriteria)
  const { criteria } = audienceSummary(groups)
  const validityYears = typeof values.validityYears === "number" ? values.validityYears : 1
  const rows: { label: string; value: string }[] = [
    { label: "Course name", value: typeof values.name === "string" && values.name.trim() ? values.name.trim() : "Untitled course" },
    { label: "Course type", value: withEditions ? "With editions" : "No editions" },
  ]
  if (withEditions) {
    rows.push({ label: "Sessions", value: values.hasSessions === "yes" ? "With sessions" : "Without sessions" })
  }
  rows.push({ label: "Mandatory", value: values.mandatoryCourse === true ? "Yes" : "No" })
  rows.push({
    label: "Validity",
    value: values.courseValidity === true ? `${validityYears} ${validityYears === 1 ? "year" : "years"}` : "No expiration",
  })
  rows.push({ label: "Enrollment", value: autoEnroll ? "Automatic rule" : "Manual" })
  if (autoEnroll) {
    rows.push({ label: "Who", value: criteria.length > 0 ? criteria.join(", ") : "No criteria yet" })
    rows.push({ label: "People matching", value: `${matchCount} ${matchCount === 1 ? "person" : "people"}` })
    if (withEditions) {
      rows.push({
        label: "Assignment",
        value: values.enrollmentAssignment === "group" ? "Assign to a group" : "Pending group assignment",
      })
    }
  }
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <F0Heading content="Review" variant="heading" as="h2" />
        <F0Text content="Check everything before creating the course." variant="body" />
      </div>
      <F0Box display="flex" flexDirection="column" gap="md" padding="lg" borderRadius="lg" background="secondary">
        {rows.map((row) => (
          <div key={row.label} className="flex items-start justify-between gap-6">
            <F0Text variant="label" content={row.label} />
            <div className="text-right">
              <F0Text variant="body" content={row.value} />
            </div>
          </div>
        ))}
      </F0Box>
...
// One chat assistant docked on the right of the editor. Static for the
// prototype: it reflects the live course config and offers an (idle) input.
function OneCourseChatSidebar({ values, matchCount }: { values: Record<string, unknown>; matchCount: number }) {
  const [inputValue, setInputValue] = useState("")
  const name = typeof values.name === "string" && values.name.trim() ? values.name.trim() : "this course"
  const withEditions = values.courseType === "with-editions"
  const autoEnroll = values.enableAutoEnrollment !== false
  const groups = readAudienceGroups(values.audienceCriteria)
  const { criteria } = audienceSummary(groups)
  const criteriaStr = criteria.length > 0 ? criteria.join(", ") : null
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden rounded-xl border border-solid border-f1-border-secondary bg-f1-background">
      <header className="flex items-center gap-2 px-4 py-3">
        <F0OneIcon size="sm" />
        <F0Text variant="label" content="One" />
      </header>

      <div className="flex flex-1 flex-col gap-3 overflow-y-auto px-4 pb-4">
        <F0Text
          variant="body"
          content={`I'll help you set up ${name}. Tell me who should take it and I'll draft the enrollment for you.`}
        />
        {autoEnroll ? (
          criteriaStr ? (
            <F0Text
              variant="body"
              content={`Right now ${matchCount} ${matchCount === 1 ? "person matches" : "people match"} (${criteriaStr}). They'll be enrolled automatically${withEditions ? ", pending group assignment" : ""}.`}
            />
          ) : (
            <F0Text variant="body" content="Pick who should take this course in the Enrollment section, or just describe them to me." />
          )
        ) : (
          <F0Text variant="body" content="Automatic enrollment is off — you'll add participants manually once the course exists." />
        )}
      </div>
      ...
      <div className="px-3 pb-3 pt-2">
        ...
        <div className="mt-2 flex justify-end">
          <F0Button label="Send" icon={ArrowUp} hideLabel variant="default" size="sm" disabled onClick={() => undefined} />
        </div>
      </div>
    </div>
  )
...
function CoursesList({
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="2xl">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/courses/v1"
        storage={false}
        source={source}
        visualizations={[
          {
  ...
  return useDataCollectionSource<ExactCourse>(
    {
      ...
      itemUrl: (course: ExactCourse) => `/p/automated-enrollments-with-one?view=detail&course=${course.id}`,
...
function CategoriesTab({
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/categories/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function SurveyTemplatesTab({
  ...
  const source = useDataCollectionSource<SurveyTemplateRow>(
    {
      ...
      itemUrl: (template) => `/p/automated-enrollments-with-one?view=survey-template-detail&template=${template.id}`,
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/survey-templates/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function RequestsTab({
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/requests/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function BudgetsTab({
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/budgets/v1"
      storage={false}
      source={budgetSource}
      visualizations={[
        {
    ...
    >
      <StandardLayout>
        <F0Box display="flex" flexDirection="column" gap="xl" height="full">
          ...
          <OneDataCollection
            id={`automated-enrollments-with-one/budget-${budget.id}/groups/v1`}
            storage={false}
            source={source}
            fullHeight
            visualizations={[
...
function BudgetParticipantsList() {
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="xl" paddingX="sm">
      <OneDataCollection
        id="automated-enrollments-with-one/budget-training-group-participants/v1"
        storage={false}
        source={source}
        visualizations={[
          {
    ...
    <Page
      header={
        <PageHeader
          module={moduleInfo}
          breadcrumbs={[
            { id: "training", label: "Training", href: "/p/automated-enrollments-with-one" },
            { id: "courses", label: "Courses", href: "/p/automated-enrollments-with-one" },
            { id: "course", label: course.name, href: `/p/automated-enrollments-with-one?view=detail&course=${course.id}` },
...
function CourseGroupsTab({
  ...
  const source = useDataCollectionSource<TrainingGroupRow>(
    {
      ...
      itemUrl: (group) => `/p/automated-enrollments-with-one?view=group-detail&course=${course.id}&group=${encodeURIComponent(group.name)}`,
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/course-groups/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function CourseParticipantsTab({ onOpenDialog }: { onOpenDialog: (dialog: CourseActionDialogId) => void }) {
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/course-participants/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function CourseMaterialsTab({ onOpenDialog }: { onOpenDialog: (dialog: CourseActionDialogId) => void }) {
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="3xl">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/course-materials/v1"
        storage={false}
        source={source}
        emptyStates={{
          "no-data": {
...
function CourseDocumentsTab({ onOpenDialog }: { onOpenDialog: (dialog: CourseActionDialogId) => void }) {
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="3xl">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/course-documents/v1"
        storage={false}
        source={source}
        emptyStates={{
          "no-data": {
...
function GroupSessionsTab({
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="lg">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/group-sessions/v1"
        storage={false}
        source={source}
        visualizations={[
          {
...
function SessionAttendanceTab() {
  ...
  return (
    <F0BoxWithClassName className="ml-4 mt-4 w-[574px]">
      <OneDataCollection
        id="automated-enrollments-with-one/session-attendance/v1"
        storage={false}
        source={source}
        visualizations={[
          {
...
function GroupParticipantsTab({ onOpenDialog }: { onOpenDialog: (dialog: GroupActionDialogId) => void }) {
  ...
  return (
    <OneDataCollection
      id="automated-enrollments-with-one/group-participants/v1"
      storage={false}
      source={source}
      visualizations={[
        {
...
function GroupMaterialsTab({ onOpenDialog }: { onOpenDialog: (dialog: GroupActionDialogId) => void }) {
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="3xl">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/group-materials/v1"
        storage={false}
        source={source}
        emptyStates={{
          "no-data": {
...
function GroupDocumentsTab({ onOpenDialog }: { onOpenDialog: (dialog: GroupActionDialogId) => void }) {
  ...
  return (
    <F0Box display="flex" flexDirection="column" gap="3xl">
      ...
      <OneDataCollection
        id="automated-enrollments-with-one/group-documents/v1"
        storage={false}
        source={source}
        emptyStates={{
          "no-data": {
