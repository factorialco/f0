...
export const expensesColumns = [
  ...
  },
  {
    ...
    label: "Status",
    render: (e: Expense) => {
      // Child rows show a dash instead of a status tag.
      if (e.isChild) return "—"
      return {
  ...
  },
  {
    ...
    sorting: "amount",
    render: (e: Expense) => {
      // Refund child rows display the refunded amount in green with a
      // leading "+" — visually mirrors a positive movement against the
      // original charge above it. Plain text, no tag chrome.
      if (e.isRefund) {
        return {
