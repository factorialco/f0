...
- Budget detail now shows a minimal `Budget needs update` warning above the groups table, with only the affected group count and `Update budget` action. Affected group rows are subtly marked through the table's `referenceRowType` pattern instead of adding a new column.
