...
- Budget detail now shows a minimal `Budget needs update` warning above the groups table without a global update action. Affected group rows show `Needs update` inside the existing training group name cell, avoiding both an extra column and a second table.
- The group sidepanel now has a single update block at the top of the Cost tab; it combines the change summary, detailed changes, and `Update budget` action instead of showing a separate alert plus a lower changes box.
