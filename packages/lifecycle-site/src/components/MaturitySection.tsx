...
export function MaturitySection() {
  return (
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-3">
        {maturityLevels.map((m) => {
          ...
          return (
            <article
              key={m.id}
              className={`flex flex-col rounded-2xl border ${c.border} bg-white p-6 shadow-sm`}
            >
              <span
      ...
      <aside className="rounded-2xl border border-ink/10 bg-paper p-5 text-sm">
        ...
        <pre className="mt-3 overflow-x-auto rounded-lg bg-ink/[0.04] p-3 font-mono text-xs">
