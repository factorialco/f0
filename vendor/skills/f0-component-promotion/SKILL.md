...
> **Who can run this skill:** members of `@factorialco/foundations`. Other contributors can request promotion in `#f0-support`, but only Foundations executes the move.

---

## Phase 4 — Validate real-world adoption

The component owner (or a Foundations engineer) starts a **promotion request in `#f0-support`** and proves all DoD criteria are met.
...
Document the findings in the `#f0-support` promotion thread with links.

Also check: no "would you remove this?" or "this should be deprecated" feedback in the last 30 days (search the component name in `#f0-support`).
...
- **Zero open critical or high-severity bugs.** Search `#f0-support` (and any bug tracker) for the component name; check severity.
...
- **Owner declares API stable** in the `#f0-support` promotion thread (explicit message).
...
For **Domain specific** components in `src/sds/<area>/`, promotion stays inside the domain folder and ownership stays with the domain team — only the story tag changes (experimental → stable). Foundations does not take ownership of Domain specific components.
...
- Link the `#f0-support` promotion thread.
...
- Post in `#f0-support` Slack with a short changelog entry.
...
An F0 designer + Foundations engineer respond to promotion requests based on team availability and as quickly as possible — we don't commit to a fixed SLA today, as it depends on team size and current focus. For urgent or blocking cases, ping `#f0-support` in Slack.
