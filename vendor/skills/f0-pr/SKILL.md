...
{Skills attribution block — see below. Omit only if no skill assisted this work.}
```
...
- Append the block exactly as defined in the root `AGENTS.md` § Skill Attribution, listing **every** `f0-*` skill used anywhere in this session — not just the ones invoked in a formal workflow
- Include `f0-pr` itself, since it wrote this body
- Never edit or reword the block's header line; it is what makes skill usage queryable
- If a skill assists after the PR exists, append it with `gh pr edit {number} --body`
...
> - f0-pr
> - {every other f0-* skill used in this session}
EOF
)"
```
...
- [ ] Skills attribution block present and lists every `f0-*` skill used, `f0-pr` included
