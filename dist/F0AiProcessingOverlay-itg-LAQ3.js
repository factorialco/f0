...
${y}`));
    ...
    if (o)
      for (const L of Object.keys(o)) {
        ...
        } else H6(D, M) && (U.arraySize = `[${Math.floor(M.length / Number.parseInt(D.charAt(0)))}]`);
        N.current[L] = {
          type: A,
          isNeeded: !1,
          value: M,
      ...
      if (n && n.length > 0) {
        ...
        const y = n.map(
          (M, D) => (N.current[`${V6}${D}`] = {
            type: "sampler2D",
            isNeeded: !1
          }, B(M, D), E.current[D] = new I6(L), E.current[D]?.load(M).then((A) => {
            B(A, D);
          }))
        );
        Promise.all(y).then(() => {
