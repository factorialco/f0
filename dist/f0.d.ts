 ...
 */
export declare interface DashboardItemAction {
 ...
 *
 * When provided for an item, its header shows a filter icon (between the
 ...
 *
 * Like `itemActions`, this lives on `F0AnalyticsDashboardProps` — not on the
 * serializable item definition — so dashboard configs remain JSON-compatible.
 */
...
export declare const F0AnalyticsDashboard: {
    <Filters extends FiltersDefinition_2 = FiltersDefinition_2>({ filters, presets, defaultFilters, items, itemActions, itemFilters, editMode, onLayoutChange, enableExport, exportFilename, onExportReady, resetKey, onTransformChart, navigationFilters, filtersLoading, }: F0AnalyticsDashboardProps_2<Filters>): JSX_2.Element;
    displayName: string;
};
...
export declare interface F0AnalyticsDashboardProps<Filters extends FiltersDefinition = FiltersDefinition> {
     ...
     * showing an error.
     */
    itemActions?: (item: DashboardItem<Filters>) => ReadonlyArray<DashboardItemAction> | undefined;
    /**
     * Resolve the per-widget filter configuration for each dashboard item.
     *
     * Return a config to show a filter icon in that widget's header (between
