...
declare type ActionConfig = Pick<F0ButtonProps, "label" | "icon" | "disabled" | "loading"> & {
    onClick?: {
        bivarianceHack(event?: MouseEvent_2<HTMLElement, MouseEvent>): void | Promise<unknown>;
    }["bivarianceHack"];
};
...
declare type CollectionVisualizations<Record extends RecordType, Filters extends FiltersDefinition, Sortings extends SortingsDefinition, Summaries extends SummariesDefinition, ItemActions extends ItemActionsDefinition<Record>, NavigationFilters extends NavigationFiltersDefinition, Grouping extends GroupingDefinition<Record>> = {
    ...
    graph: VisualizacionTypeDefinition<GraphCollectionProps<Record, Filters, Sortings, Summaries, ItemActions, NavigationFilters, Grouping>>;
};
...
declare interface DefaultVariantProps extends BaseProps {
    variant?: "default";
...
declare interface DeferredNodesPayload<T = unknown> {
    nodes: GraphNode<T>[];
    edges?: GraphEdge[];
}
...
declare type EdgeVariant = (typeof edgeVariants)[number];

declare const edgeVariants: readonly ["default", "hover", "highlighted", "dimmed"];
...
declare type F0GraphDetailPanelProps = DefaultVariantProps | ResourceVariantProps;
 ...
 */
declare interface F0GraphNodeRenderContext {
    zoomLevel: ZoomLevel;
...
declare interface F0GraphProps<T = unknown> {
    nodes?: GraphNode<T>[];
    edges?: GraphEdge[];
     ...
     */
    detailPanel?: (node: GraphNode<T>) => Omit<F0GraphDetailPanelProps, "open" | "onClose" | "width" | "ariaLabel">;
 ...
 */
declare type GraphEdgeAdapter<R extends RecordType> = (records: ReadonlyArray<R>) => ReadonlyArray<GraphEdge>;
 ...
 */
declare type GraphGraphPropsPassthrough<R extends RecordType> = Omit<F0GraphProps<R>, "nodes" | "edges" | "rootNodes" | "loadChildren" | "renderNode" | "selectionMode" | "onNodeSelect" | "onSelectedNodesChange" | "detailPanel" | "selectedNodes" | "highlightedNodes" | "searchValue" | "onSearchChange" | "searchable">;

declare interface GraphNode<T = unknown> {
    id: string;
    /**
 ...
 */
declare type GraphNodeAdapter<R extends RecordType> = (record: R) => {
...
declare type GraphNodeState = (typeof graphNodeStates)[number];

declare const graphNodeStates: readonly ["default", "selected", "highlighted", "dimmed"];

declare type GraphNodeVariant = (typeof graphNodeVariants)[number];

declare const graphNodeVariants: readonly ["detail", "compact", "dot"];
 ...
 */
declare type GraphRenderNodeExtras = {
    /** Ready-to-render dropdown trigger; spread into `F0GraphNode.actions`. */
    actions?: ReactNode;
};

declare type GraphVisualizationOptions<R extends RecordType, _Filters extends FiltersDefinition, _Sortings extends SortingsDefinition> = {
    /**
     ...
     */
    renderNode: (record: R, ctx: F0GraphNodeRenderContext, extras: GraphRenderNodeExtras) => ReactNode;
    /**
     ...
     */
    loadChildren?: (nodeId: string, opts: {
        /**
     ...
     */
    detailPanel?: (record: R) => Omit<F0GraphDetailPanelProps, "open" | "onClose" | "width" | "ariaLabel">;
    /**
     ...
     */
    highlightedNodes?: (records: readonly R[]) => ReadonlyArray<string> | ReadonlySet<string>;
    /**
...
declare type LayoutDirection = "TB" | "LR" | "BT" | "RL";
...
declare interface PositionedEdge {
    id: string;
    source: string;
    target: string;
    points: Array<{
...
declare interface PositionedNode {
    id: string;
    x: number;
    y: number;
    width: number;
...
declare interface ResourceVariantProps extends BaseProps {
    variant: "resource";
    /**
 ...
 * still searchable.
 */
declare interface Searchable<T> {
 ...
 */
declare type SearchMatcher<T> = (normalizedQuery: string, nodes: ReadonlyArray<GraphNode<T>>, helpers: SearchMatcherHelpers<T>) => ReadonlyArray<SearchMatch>;
 ...
 * built-in default.
 */
declare interface SearchMatcherHelpers<T> {
    getLabel: (node: GraphNode<T>) => string;
    getSecondaryLabel?: (node: GraphNode<T>) => string | undefined;
    /**
...
declare type SearchMatchRange = readonly [number, number];
...
declare interface TreeNode<T = unknown> {
    id: string;
    parentId: string | null;
    data: T;
    children: TreeNode<T>[];
...
declare type ZoomLevel = (typeof zoomLevels)[number];

declare const zoomLevels: readonly ["detail", "compact", "dot"];
...
declare interface ZoomThresholds {
    detail: number;
    compact: number;
    dot: number;
}
