...
import { M as He, N as je, Q as Ke, R as Qe, V as Me, W as Ze, X as Je, Y as et, Z as tt, _ as nt, $ as Ne, a0 as rt, a1 as ot, a2 as it, a3 as st, a4 as at, a5 as ct, a6 as ut, a7 as se, a8 as lt, a9 as ft, aa as ht, ab as dt, ac as mt, ad as pt, ae as fe, af as gt } from "./F0ChatDashboard-DENcelIr.js";
...
function Lt(t, { defaultTransition: e = {}, ...n } = {}, r, o) {
  ...
  for (let l = 0; l < t.length; l++) {
    ...
    const U = (y, w, _, P = 0, V = 0) => {
      const A = It(y), { delay: f = 0, times: C = Ze(A), type: O = "keyframes", repeat: W, repeatType: k, repeatDelay: H = 0, ...z } = w;
      let { ease: M = e.ease || "easeOut", duration: L } = w;
      const N = typeof f == "function" ? f(P, V) : f, j = A.length, J = tt(O) ? O : o?.[O];
      if (j <= 2 && J) {
        let K = 100;
        if (j === 2 && Ft(A)) {
          const Z = A[1] - A[0];
      ...
      if (ne > 0 && et(C, ne), A.length === 1 && A.unshift(null), W) {
        ...
        for (let Z = 0; Z < W; Z++) {
          ...
          for (let i = 0; i < K.length; i++)
            C.push(Q[i] + (Z + 1)), M.push(i === 0 ? "linear" : Be(ee, i - 1));
        }
        At(C, W);
      }
      const re = $ + L;
...
function pe(t, e, n, r) {
  const o = Math.max(1, Math.min(t, e)), c = Math.min(n, 20), E = Math.min(c + r, o), g = Math.min(E, Math.floor(t / 2)), R = Math.min(E, Math.floor(e / 2)), p = ($) => $ / t * 2 - 1, v = ($) => $ / e * 2 - 1, x = 0, l = t, T = 0, F = e, d = g, S = t - g, I = R, U = e - R, y = p(x), w = p(l), _ = v(T), P = v(F), V = p(d), A = p(S), f = v(I), C = v(U), O = 0, W = 0, k = 1, H = 1, z = g / t, M = 1 - g / t, L = R / e, N = 1 - R / e, j = new Float32Array([
    // Top strip
    y,
    _,
    w,
...
}`, Te = "iTime", ye = "iTimeDelta", Ee = "iDate", be = "iFrame", Ae = "iMouse", Re = "iResolution", Kt = "iChannel", xe = "iChannelResolution", Se = "iDeviceOrientation";
function Qt(t, e) {
...
${a}`));
    ...
    if (r)
      for (const i of Object.keys(r)) {
        ...
        } else Zt(h, s) && (b.arraySize = `[${Math.floor(s.length / Number.parseInt(h.charAt(0)))}]`);
        f.current[i] = {
          type: u,
          isNeeded: !1,
          value: s,
...
`,
  ...
  return ie(() => {
    ...
    function a() {
      W();
      const s = T.current;
      s && l.current && (s.clearColor(...o), s.clearDepth(1), s.enable(s.DEPTH_TEST), s.depthFunc(s.LEQUAL), s.viewport(0, 0, l.current.width, l.current.height), l.current.height = l.current.clientHeight, l.current.width = l.current.clientWidth, $(), ne(), J(re(t || jt), e || we), k(), requestAnimationFrame(Q), ee(), N());
    }
