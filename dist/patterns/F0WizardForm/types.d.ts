import { z, ZodRawShape, ZodEffects, ZodType } from 'zod';
import { ModuleId } from '../../components/avatars/F0AvatarModule';
import { InitialFile } from '../F0Form/fields/file/types';
import { F0FormErrorTriggerMode, F0FormSubmitConfig, F0FormSubmitResult, F0PerSectionSubmitConfig, F0SectionConfig, F0PerSectionSectionConfig, RenderCustomFieldFunction } from '../F0Form/types';
import { F0DialogSize } from '../../components/dialog-alike/F0Dialog';
import { DialogWidth } from '../F0Dialog';
export type F0FormSchema<T extends ZodRawShape = ZodRawShape> = z.ZodObject<T> | ZodEffects<z.ZodObject<T>>;
...
export type F0FormDefinition<T extends F0FormSchema | F0PerSectionSchema = F0FormSchema | F0PerSectionSchema> = T extends F0FormSchema ? F0FormDefinitionSingleSchema<T> : T extends F0PerSectionSchema ? F0FormDefinitionPerSection<T> : never;
...
export interface F0WizardFormSingleSchemaProps<TSchema extends F0FormSchema> extends F0WizardFormBaseProps {
...
export interface F0WizardFormPerSectionProps<T extends F0PerSectionSchema> extends F0WizardFormBaseProps {
