import { FiltersDefinition, FiltersState, GroupingDefinition, GroupingState, RecordType, SortingsDefinition, SortingsState } from '../../../../hooks/datasource';
...
export type DataCollectionStatusComplete<CurrentFiltersState extends FiltersState<FiltersDefinition>> = DataCollectionStatus<CurrentFiltersState> & {
    settings?: DataCollectionSettings;
};
/**
 * The available features of the data collection status storage
 */
export declare const dataCollectionStorageFeatures: readonly ["filters", "navigationFilters", "sortings", "grouping", "visualization", "search", "visualizationFilters"];
export type DataCollectionStorageFeature = (typeof dataCollectionStorageFeatures)[number];
export type DataCollectionStorageFeaturesDefinition = ("*" | `all` | `!${DataCollectionStorageFeature}` | `${DataCollectionStorageFeature}`)[];
/**
 * The feature providers for the data collection storage
 */
export type FeatureProviders<R extends RecordType, Grouping extends GroupingDefinition<R>, Sortings extends SortingsDefinition, Filters extends FiltersDefinition, NavigationFilters extends NavigationFiltersDefinition> = {
    settings?: {
