...
import { useInsertionEffect as ke, createContext as Ge, useContext as $e, useRef as _, useEffect as ne, useState as _e, useCallback as Ye, useMemo as ze } from "react";
import { M as qe, N as We, Q as He, R as je, V as Ne, W as Ke, X as Qe, Y as Ze, Z as Je, _ as et, $ as Ce, a0 as tt, a1 as rt, a2 as nt, a3 as it, a4 as ot, a5 as at, a6 as st, a7 as ie, a8 as ct, a9 as ut, aa as lt, ab as ft, ac as mt, ad as dt, ae as fe, af as ht } from "./F0ChatDashboard-Dnq7QeIk.js";
...
function bt(t, { defaultTransition: e = {}, ...r } = {}, n, l) {
  ...
  for (let c = 0; c < t.length; c++) {
    ...
    const C = (A, S, I, L = 0, V = 0) => {
      const T = Rt(A), { delay: d = 0, times: P = Ke(T), type: W = "keyframes", repeat: G, repeatType: ue, repeatDelay: oe = 0, ...H } = S;
      let { ease: U = e.ease || "easeOut", duration: M } = S;
      const $ = typeof d == "function" ? d(L, V) : d, Q = T.length, Z = Je(W) ? W : l?.[W];
      if (Q <= 2 && Z) {
        let Y = 100;
        if (Q === 2 && Ft(T)) {
          const q = T[1] - T[0];
      ...
      if (ee > 0 && Ze(P, ee), T.length === 1 && T.unshift(null), G) {
        Ce(G < xt, "Repeat count too high, must be less than 20"), M = gt(M, G);
        const Y = [...T], z = [...P];
      ...
      const te = J + M;
      Tt(I, T, U, P, J, te), F = Math.max($ + M, F), x = Math.max(te, x);
    };
    if (Ne(N)) {
      const A = de(N, w);
      C(m, R, he("default", A));
...
function pr({
  ...
  return /* @__PURE__ */ k(Pe.Provider, { value: e, children: t });
}
function vr() {
  const t = $e(Pe);
  if (t === null)
...
const pe = ["lowp", "mediump", "highp"], kt = `
void main(void){
    vec4 color = vec4(0.0,0.0,0.0,1.0);
    mainImage( color, gl_FragCoord.xy );
    gl_FragColor = color;
}`, Gt = `void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
...
`,
    ...
    return i.includes("mainImage") && (u = u.concat(kt)), u;
  }, Y = (i) => {
    const a = v.current;
  ...
  return ne(() => {
    ...
    function a() {
      G();
      const o = v.current;
      o && c.current && (o.clearColor(...l), o.clearDepth(1), o.enable(o.DEPTH_TEST), o.depthFunc(o.LEQUAL), o.viewport(0, 0, c.current.width, c.current.height), c.current.height = c.current.clientHeight, c.current.width = c.current.clientWidth, J(), ee(), Z(te(t || Gt), e || ve), ue(), requestAnimationFrame(z), j(), $());
    }
...
function Ve({
  ...
  const c = ze(() => sr(h), [h]);
  return /* @__PURE__ */ k("div", { ref: y, className: E, ...x, children: /* @__PURE__ */ k(
    Zt,
    {
      fs: Jt,
