import { NodeSelection as D, Selection as he, TextSelection as M, AllSelection as Cn } from "../../../../prosemirror-state@1.4.3/node_modules/prosemirror-state/dist/index.js";
import { Slice as J, DOMSerializer as Re, Fragment as W, DOMParser as Kt, Mark as On } from "../../../../prosemirror-model@1.25.0/node_modules/prosemirror-model/dist/index.js";
import { dropPoint as Mn } from "../../../../prosemirror-transform@1.10.3/node_modules/prosemirror-transform/dist/index.js";
const S = function(t) {
  for (var e = 0; ; e++)
...
function yo(t) {
  ...
  return e ? (qe || (qe = e.createPolicy("ProseMirrorClipboard", { createHTML: (n) => n })), qe.createHTML(t)) : t;
}
function bo(t) {
  let e = /^(\s*<meta [^>]*>)*/.exec(t);
...
k.dragstart = (t, e) => {
  ...
  (!n.dataTransfer.files.length || !O || Wt > 120) && n.dataTransfer.clearData(), n.dataTransfer.setData(De ? "Text" : "text/html", a.innerHTML), n.dataTransfer.effectAllowed = "copyMove", De || n.dataTransfer.setData("text/plain", c), t.dragging = new gn(d, !n[yn], r);
};
k.dragend = (t) => {
  let e = t.dragging;
  window.setTimeout(() => {
...
class N {
  ...
  */
  mapInner(e, n, o, i, s) {
    ...
    return this.children.length ? Ho(this.children, r || [], e, n, o, i, s) : r ? new N(r.sort(se), de) : C;
  }
  /**
...
class Yo {
  ...
  flush() {
    ...
    let c = null;
    s < 0 && i && e.input.lastFocus > Date.now() - 200 && Math.max(e.input.lastTouch, e.input.lastClick.time) < Date.now() - 300 && Be(o) && (c = ve(e)) && c.eq(he.near(e.state.doc.resolve(0), 1)) ? (e.input.lastFocus = 0, H(e), this.currentSelection.set(o), e.scrollToSelection()) : (s > -1 || i) && (s > -1 && (e.docView.markDirty(s, r), Go(e)), this.handleDOMChange(s, r, l, a), e.docView && e.docView.dirty ? e.updateState(e.state) : this.currentSelection.eq(o) || H(e), this.currentSelection.set(o));
  }
...
function vo(t, e, n, o, i) {
  ...
  if (h && t.input.domChangeCount++, (pe && t.input.lastIOSEnter > Date.now() - 225 || K) && i.some((b) => b.nodeType == 1 && !wo.test(b.nodeName)) && (!h || h.endA >= h.endB) && t.someProp("handleKeyDown", (b) => b(t, te(13, "Enter")))) {
  ...
  if ((pe && t.input.lastIOSEnter > Date.now() - 225 && (!E || i.some((b) => b.nodeName == "DIV" || b.nodeName == "P")) || !E && g.pos < c.doc.content.size && !g.sameParent(m) && (me = he.findFrom(c.doc.resolve(g.pos + 1), 1, !0)) && me.head == m.pos) && t.someProp("handleKeyDown", (b) => b(t, te(13, "Enter")))) {
    t.input.lastIOSEnter = 0;
    return;
  }
  ...
  if (E) {
    if (g.pos == m.pos)
      ...
      h.endA == h.endB && (xe = ei(g.parent.content.cut(g.parentOffset, m.parentOffset), y.parent.content.cut(y.parentOffset, h.endA - y.start())))
    )
...
class ai {
  ...
  */
  ...
  updateStateInner(e, n) {
    ...
    if (r) {
      ...
      let p = f && (A || O) && !this.composing && !i.selection.empty && !e.selection.empty && oi(i.selection, e.selection);
      if (f) {
  ...
  */
  domSelectionRange() {
    ...
    return e ? x && this.root.nodeType === 11 && Pn(this.dom.ownerDocument) == this.dom && Xo(this, e) || e : { focusNode: null, focusOffset: 0, anchorNode: null, anchorOffset: 0 };
  }
  /**
  @internal
  */
