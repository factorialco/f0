import { BROWSER as v } from "../../../../esm-env-runtime@0.1.1/node_modules/esm-env-runtime/browser.js";
const m = (n, t, e) => {
...
}, O = (n) => n.offsetWidth > 0 && n.offsetHeight > 0, it = (n, t) => {
  v && !customElements.get(n) && customElements.define(n, t);
};
function I(n, t, { reverse: e = !1 } = {}) {
  const i = n.length;
...
function st(n, t, e, i) {
  ...
  const r = [], a = [], o = [], p = [], d = {}, c = (h) => `${h}:${d[h] = (d[h] ?? -1) + 1}`;
  let x = "", g = !1, y = !1;
  for (const h of s) {
    x += h.value;
    const l = h.type === "minusSign" || h.type === "plusSign" ? "sign" : h.type;
    l === "integer" ? (g = !0, a.push(...h.value.split("").map((C) => ({ type: l, value: parseInt(C) })))) : l === "group" ? a.push({ type: l, value: h.value }) : l === "decimal" ? (y = !0, o.push({ type: l, value: h.value, key: c(l) })) : l === "fraction" ? o.push(...h.value.split("").map((C) => ({
      type: l,
      value: parseInt(C),
      key: c(l),
      pos: -1 - d[l]
...
const D = String.raw, V = String.raw, z = v && (() => {
  try {
  ...
  return !0;
})(), H = v && typeof CSS < "u" && CSS.supports && CSS.supports("line-height", "mod(1,1)"), T = v && typeof matchMedia < "u" ? matchMedia("(prefers-reduced-motion: reduce)") : null, $ = "--_number-flow-d-opacity", R = "--_number-flow-d-width", S = "--_number-flow-dx", j = "--_number-flow-d", Y = (() => {
  try {
...
})(), U = "var(--number-flow-char-height, 1em)", u = "var(--number-flow-mask-height, 0.25em)", A = `calc(${u} / 2)`, k = "var(--number-flow-mask-width, 0.5em)", f = `calc(${k} / var(--scale-x))`, _ = "#000 0, transparent 71%", N = V`:host{display:inline-block;direction:ltr;white-space:nowrap;isolation:isolate;line-height:${U} !important}.number,.number__inner{display:inline-block;transform-origin:left top}:host([data-will-change]) :is(.number,.number__inner,.section,.digit,.digit__num,.symbol){will-change:transform}.number{--scale-x:calc(1 + var(${R}) / var(--width));transform:translateX(var(${S})) scaleX(var(--scale-x));margin:0 calc(-1 * ${k});position:relative;-webkit-mask-image:linear-gradient(to right,transparent 0,#000 ${f},#000 calc(100% - ${f}),transparent ),linear-gradient(to bottom,transparent 0,#000 ${u},#000 calc(100% - ${u}),transparent 100% ),radial-gradient(at bottom right,${_}),radial-gradient(at bottom left,${_}),radial-gradient(at top left,${_}),radial-gradient(at top right,${_});-webkit-mask-size:100% calc(100% - ${u} * 2),calc(100% - ${f} * 2) 100%,${f} ${u},${f} ${u},${f} ${u},${f} ${u};-webkit-mask-position:center,center,top left,top right,bottom right,bottom left;-webkit-mask-repeat:no-repeat}.number__inner{padding:${A} ${k};transform:scaleX(calc(1 / var(--scale-x))) translateX(calc(-1 * var(${S})))}:host > :not(.number){z-index:5}.section,.symbol{display:inline-block;position:relative;isolation:isolate}.section::after{content:'\200b';display:inline-block}.section--justify-left{transform-origin:center left}.section--justify-right{transform-origin:center right}.section > [inert],.symbol > [inert]{margin:0 !important;position:absolute !important;z-index:-1}.digit{display:inline-block;position:relative;--c:var(--current) + var(${j})}.digit__num,.number .section::after{padding:${A} 0}.digit__num{display:inline-block;--offset-raw:mod(var(--length) + var(--n) - mod(var(--c),var(--length)),var(--length));--offset:calc( var(--offset-raw) - var(--length) * round(down,var(--offset-raw) / (var(--length) / 2),1) );--y:clamp(-100%,var(--offset) * 100%,100%);transform:translateY(var(--y))}.digit__num[inert]{position:absolute;top:0;left:50%;transform:translateX(-50%) translateY(var(--y))}.digit:not(.is-spinning) .digit__num[inert]{display:none}.symbol__value{display:inline-block;mix-blend-mode:plus-lighter;white-space:pre}.section--justify-left .symbol > [inert]{left:0}.section--justify-right .symbol > [inert]{right:0}.animate-presence{opacity:calc(1 + var(${$}))}`, Z = v ? HTMLElement : class {
}, q = V`:host{display:inline-block;direction:ltr;white-space:nowrap;line-height:${U} !important}span{display:inline-block}:host([data-will-change]) span{will-change:transform}.number,.digit{padding:${A} 0}.symbol{white-space:pre}`, G = (n) => `<span class="${n.type === "integer" || n.type === "fraction" ? "digit" : "symbol"}" part="${n.type === "integer" || n.type === "fraction" ? `digit ${n.type}-digit` : `symbol ${n.type}`}">${n.value}</span>`, w = (n, t) => `<span part="${t}">${n.reduce((e, i) => e + G(i), "")}</span>`, nt = (n) => (
  ...
  D`<template shadowroot="open" shadowrootmode="open"
			...
			></template
		><span
			style="font-kerning: none; display: inline-block; line-height: ${U} !important; padding: ${u} 0;"
...
class K extends Z {
  ...
  set data(t) {
    ...
    if (this.created) {
      const p = this._data;
      this._data = t, this.computedTrend = typeof this.trend == "function" ? this.trend(p.value, o) : this.trend, this.computedAnimated = J && this._animated && (!this.respectMotionPreference || !(T != null && T.matches)) && // https://github.com/barvian/number-flow/issues/9
      O(this), (e = this.plugins) == null || e.forEach((d) => {
        var c;
        return (c = d.onUpdate) == null ? void 0 : c.call(d, t, p, this);
      ...
      if (typeof CSSStyleSheet < "u" && this.shadowRoot.adoptedStyleSheets)
        b || (b = new CSSStyleSheet(), b.replaceSync(N)), this.shadowRoot.adoptedStyleSheets = [b];
      else {
        const p = document.createElement("style");
        p.textContent = N, this.shadowRoot.appendChild(p);
      }
      this._pre = new L(this, i, {
        justify: "right",
        part: "left"
      }), this.shadowRoot.appendChild(this._pre.el), this._num = new Q(this, s, r), this.shadowRoot.appendChild(this._num.el), this._post = new L(this, a, {
        justify: "left",
        part: "right"
...
class X extends W {
  constructor(t, e, i, s, r) {
    var a, o;
    const p = (((o = (a = t.flow.digits) == null ? void 0 : a[s]) == null ? void 0 : o.max) ?? 9) + 1, d = Array.from({ length: p }).map((x, g) => {
    ...
    c.style.setProperty("--current", String(i)), c.style.setProperty("--length", String(p)), super(t, i, c, r), this.pos = s, this._onAnimationsFinish = () => {
      this.el.classList.remove("is-spinning");
    }, this._numbers = d, this.length = p;
  }
  willUpdate(t) {
...
class tt extends W {
  constructor(t, e, i, s) {
    ...
    }, this._children.set(i, new E(this.flow, r, {
