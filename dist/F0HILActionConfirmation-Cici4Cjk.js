...
import { G as qe, J as We, K as He, M as je, N as Ne, Q as Ke, R as Qe, V as Je, W as Ze, X as et, Y as Ce, Z as tt, _ as rt, $ as nt, a0 as ot, a1 as it, a2 as at, a3 as st, a4 as oe, a5 as ct, a6 as ut, a7 as lt, a8 as ft, a9 as mt, aa as dt, ab as fe, ac as ht } from "./F0AiChat-CfZjKrZg.js";
...
function bt(t, { defaultTransition: e = {}, ...r } = {}, n, l) {
  ...
  for (let c = 0; c < t.length; c++) {
    ...
    let L = 0;
    const C = (A, S, I, F = 0, V = 0) => {
      const T = Rt(A), { delay: d = 0, times: P = Ke(T), type: W = "keyframes", repeat: k, repeatType: ue, repeatDelay: ie = 0, ...H } = S;
      let { ease: U = e.ease || "easeOut", duration: M } = S;
      const $ = typeof d == "function" ? d(F, V) : d, Q = T.length, J = Ze(W) ? W : l?.[W];
      if (Q <= 2 && J) {
        let Y = 100;
        if (Q === 2 && Lt(T)) {
          const q = T[1] - T[0];
      ...
      if (ee > 0 && Je(P, ee), T.length === 1 && T.unshift(null), k) {
        ...
        for (let q = 0; q < k; q++) {
          ...
          for (let o = 0; o < Y.length; o++)
            P.push(z[o] + (q + 1)), U.push(o === 0 ? "linear" : Oe(j, o - 1));
        }
        wt(P, k);
      }
      const te = Z + M;
      Tt(I, T, U, P, Z, te), L = Math.max($ + M, L), x = Math.max(te, x);
    };
    if (Ne(N)) {
      const A = de(N, w);
      C(m, R, he("default", A));
...
function Nt(t) {
  ...
  }, r = ot(t) && !it(t) ? new at(e) : new st(e);
  r.mount(t), oe.set(t, r);
}
function Ct(t) {
  const e = {
    presenceContext: null,
...
class Kt {
  ...
  load = async (e) => {
    ...
    const E = 0, x = r.RGBA, c = 1, v = 1, N = 0, m = r.RGBA, R = r.UNSIGNED_BYTE, L = new Uint8Array([255, 255, 255, 0]), C = r.createTexture();
...
const X = (t) => `react-shaders: ${t}`, Fe = (t) => {
...
function Jt({
  ...
  const c = _(null), v = _(null), N = _(null), m = _(null), R = _(void 0), L = _(void 0), C = _(!1), A = _(void 0), S = _(0), I = _([0, 0]), F = _([]), V = _(0), T = _(void 0), d = _({
    ...
    const i = "width" in o ? o.width ?? 0 : 0, u = "height" in o ? o.height ?? 0 : 0, s = d.current.iChannelResolution;
    if (!s) return;
    const p = Array.isArray(s.value) ? s.value : s.value = [];
    p[a * 3] = i * b, p[a * 3 + 1] = u * b, p[a * 3 + 2] = 0;
  }, k = () => {
    ...
    const o = v.current;
    N.current = o?.createBuffer() ?? null, o?.bindBuffer(o.ARRAY_BUFFER, N.current);
    const a = [
      1,
      1,
      0,
    ...
    const [a = 0, i = 0] = Fe(o), u = a - (A.current?.left ?? 0) - window.pageXOffset, s = (A.current?.height ?? 0) - i - (A.current?.top ?? 0) - window.pageYOffset;
    C.current = !0;
    ...
    const [a = 0, i = 0] = Fe(o), u = a - (A.current?.left ?? 0), s = (A.current?.height ?? 0) - i - (A.current?.top ?? 0);
    if (g !== 1)
    ...
    if (!o) return;
    A.current = c.current?.getBoundingClientRect();
    const a = b, i = Math.floor(
      (A.current?.width ?? 1) * a
    ), u = Math.floor(
      (A.current?.height ?? 1) * a
    );
    if (o.canvas.width = i, o.canvas.height = u, d.current.iResolution?.isNeeded && m.current) {
      const s = o.getUniformLocation(
        m.current,
        Ae
      );
      o.uniform2fv(s, [o.canvas.width, o.canvas.height]);
    }
  }, Q = (o, a) => {
    const i = v.current;
    if (!i) return null;
    ...
    if (!u) return null;
    if (i.shaderSource(u, a), i.compileShader(u), !i.getShaderParameter(u, i.COMPILE_STATUS)) {
      x?.(X(`Error compiling the shader:
${a}`));
      const s = i.getShaderInfoLog(u);
      i.deleteShader(u), E?.(X(`Shader compiler log: ${s}`));
    }
    return u;
  }, J = (o, a) => {
    const i = v.current;
    if (!i) return;
    const u = Q(i.FRAGMENT_SHADER, o), s = Q(i.VERTEX_SHADER, a);
    if (m.current = i.createProgram(), !(!m.current || !s || !u)) {
      if (i.attachShader(m.current, s), i.attachShader(m.current, u), i.linkProgram(m.current), !i.getProgramParameter(m.current, i.LINK_STATUS)) {
        E?.(
          X(
      ...
      i.useProgram(m.current), R.current = i.getAttribLocation(
        m.current,
        "aVertexPosition"
      ), i.enableVertexAttribArray(R.current);
    }
  }, Z = () => {
    if (n)
      for (const o of Object.keys(n)) {
        const a = n[o];
        if (!a) continue;
        const { value: i, type: u } = a, s = Ht(u);
        if (!s) continue;
        const p = {};
        if (Yt(u, i)) {
          const O = u.length, B = Number.parseInt(u.charAt(O - 3)), K = Math.floor(i.length / (B * B));
          i.length > B * B && (p.arraySize = `[${K}]`);
        } else zt(u, i) && (p.arraySize = `[${Math.floor(i.length / Number.parseInt(u.charAt(0)))}]`);
        d.current[o] = {
          type: s,
          isNeeded: !1,
          value: i,
      ...
      if (r && r.length > 0) {
        ...
        const a = r.map(
          (i, u) => (d.current[`${$t}${u}`] = {
            type: "sampler2D",
            isNeeded: !1
          }, W(i, u), F.current[u] = new Kt(o), F.current[u]?.load(i).then((s) => {
            W(s, u);
          }))
        );
        Promise.all(a).then(() => {
    ...
    const a = pe.includes(f ?? "highp"), i = `precision ${a ? f : pe[1]} float;
`;
    a || x?.(
      X(
...
`,
    ...
    return o.includes("mainImage") && (u = u.concat(Gt)), u;
  }, Y = (o) => {
    const a = v.current;
    if (!a || !m.current) return;
    const i = V.current ? (o - V.current) / 1e3 : 0;
    V.current = o;
    const u = P.current;
    if (u)
    ...
    a.viewport(0, 0, a.drawingBufferWidth, a.drawingBufferHeight), a.clear(a.COLOR_BUFFER_BIT | a.DEPTH_BUFFER_BIT), a.bindBuffer(a.ARRAY_BUFFER, N.current), a.vertexAttribPointer(
      ...
      0,
      0
    ), Y(o), a.drawArrays(a.TRIANGLE_STRIP, 0, 4);
    const i = d.current.iMouse?.value;
    if (d.current.iMouse?.isNeeded && g !== 1 && Array.isArray(i)) {
      const u = i[0] ?? 0, s = i[1] ?? 0;
      i[0] = Le(u, I.current[0] ?? 0, g), i[1] = Le(s, I.current[1] ?? 0, g);
    }
    L.current = requestAnimationFrame(z);
  }, j = () => {
    const o = { passive: !0 };
    d.current.iMouse?.isNeeded && c.current && (c.current.addEventListener("mousemove", U, o), c.current.addEventListener("mouseout", M, o), c.current.addEventListener("mouseup", M, o), c.current.addEventListener("mousedown", H, o), c.current.addEventListener("touchmove", U, o), c.current.addEventListener("touchend", M, o), c.current.addEventListener("touchstart", H, o)), d.current.iDeviceOrientation?.isNeeded && window.addEventListener(
      "deviceorientation",
      ie,
      o
    ), c.current && (T.current = new ResizeObserver($), T.current.observe(c.current), window.addEventListener("resize", $, o));
  }, q = () => {
    const o = { passive: !0 };
    d.current.iMouse?.isNeeded && c.current && (c.current.removeEventListener("mousemove", U, o), c.current.removeEventListener("mouseout", M, o), c.current.removeEventListener("mouseup", M, o), c.current.removeEventListener("mousedown", H, o), c.current.removeEventListener("touchmove", U, o), c.current.removeEventListener("touchend", M, o), c.current.removeEventListener("touchstart", H, o)), d.current.iDeviceOrientation?.isNeeded && window.removeEventListener(
      "deviceorientation",
      ie,
      o
    ), T.current && (T.current.disconnect(), window.removeEventListener("resize", $, o));
  };
  return ne(() => {
    P.current = n;
  }, [n]), ne(() => {
    ...
    function a() {
      k();
      const i = v.current;
      i && c.current && (i.clearColor(...l), i.clearDepth(1), i.enable(i.DEPTH_TEST), i.depthFunc(i.LEQUAL), i.viewport(0, 0, c.current.width, c.current.height), c.current.height = c.current.clientHeight, c.current.width = c.current.clientWidth, Z(), ee(), J(te(t || kt), e || ve), ue(), requestAnimationFrame(z), j(), $());
    }
    return requestAnimationFrame(a), () => {
      const i = v.current;
      if (i) {
        if (i.getExtension("WEBGL_lose_context")?.loseContext(), i.useProgram(null), i.deleteProgram(m.current ?? null), o.length > 0)
          for (const u of o)
...
}`, er = 10, tr = 2, rr = 0.5, nr = 0.2, or = 1.5, D = {
  duration: 0.5,
  ease: "easeOut"
}, Me = {
  duration: 0.35,
...
function ir(t, e) {
  const [r, n] = _e(er), {
    ...
    animate: f,
    motionValue: h
  } = re(nr), { value: w, animate: g } = re(tr), { value: b, animate: y } = re(rr), { value: E, animate: x } = re(or), c = pt(e, {
    fftSize: 512,
...
function gr({
  ...
  const { speed: b, scale: y, amplitude: E, frequency: x, brightness: c } = ir(e, l);
  return /* @__PURE__ */ G(
    Ve,
    {
      ref: w,
