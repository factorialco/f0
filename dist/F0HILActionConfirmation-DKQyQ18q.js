...
import { G as Oe, J as De, K as Be, M as Ve, N as ee, Q as ke } from "./F0AiChat-DuaJFO02.js";
import { useTrackVolume as Xe } from "@livekit/components-react";
function $e(r, t, e) {
  Ce(() => r.on(t, e), [r, t, e]);
}
const mt = ["xs", "sm", "md", "lg"], pt = [
  "inProgress",
  "executing",
  "completed"
], vt = {
...
function gt({
  ...
  return /* @__PURE__ */ M(Te.Provider, { value: t, children: r });
}
function Tt() {
  const r = Me(Te);
  if (r === null)
...
const te = ["lowp", "mediump", "highp"], qe = `
void main(void){
    vec4 color = vec4(0.0,0.0,0.0,1.0);
    mainImage( color, gl_FragCoord.xy );
    gl_FragColor = color;
}`, Ge = `void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
...
}, H = 9729, fe = 9728, Qe = 9987, de = 33071, he = 10497;
class Je {
  gl;
  url;
  wrapS;
  ...
  updateTexture = (t, e, n) => {
    const { gl: d } = this, m = 0, g = d.RGBA, y = d.RGBA, p = d.UNSIGNED_BYTE;
    d.bindTexture(d.TEXTURE_2D, t), d.pixelStorei(d.UNPACK_FLIP_Y_WEBGL, n), d.texImage2D(
      d.TEXTURE_2D,
      m,
      g,
      y,
  ...
  setupVideo = (t) => {
    ...
    return e.addEventListener(
      "playing",
      () => {
        n = !0, this.width = e.videoWidth || 0, this.height = e.videoHeight || 0, m();
      },
      !0
    ), e.addEventListener(
      "timeupdate",
  ...
  load = async (t) => {
    ...
    Object.assign(this, { url: n, wrapS: d, wrapT: m, minFilter: g, magFilter: y, flipY: p });
    const A = 0, E = e.RGBA, u = 1, T = 1, B = 0, h = e.RGBA, N = e.UNSIGNED_BYTE, k = new Uint8Array([255, 255, 255, 0]), P = e.createTexture();
    if (e.bindTexture(e.TEXTURE_2D, P), e.texImage2D(
      e.TEXTURE_2D,
      A,
      E,
      u,
      ...
      return e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, e.CLAMP_TO_EDGE), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, e.CLAMP_TO_EDGE), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, e.LINEAR), this._webglTexture = P, this.source = _, this.isVideo = !0, _.play().then(() => this);
    }
    async function S() {
    ...
    let L = await S(), F = (L.width & L.width - 1) === 0 && (L.height & L.height - 1) === 0;
    return (t.wrapS !== de || t.wrapT !== de || t.minFilter !== fe && t.minFilter !== H) && !F && (L = this.makePowerOf2(L), F = !0), e.bindTexture(e.TEXTURE_2D, P), e.pixelStorei(e.UNPACK_FLIP_Y_WEBGL, p), e.texImage2D(
      e.TEXTURE_2D,
      A,
      E,
      h,
      N,
      L
    ), F && t.minFilter !== fe && t.minFilter !== H && e.generateMipmap(e.TEXTURE_2D), e.texParameteri(
      e.TEXTURE_2D,
      e.TEXTURE_WRAP_S,
      this.wrapS || he
    ), e.texParameteri(
      ...
      this.magFilter || H
    ), this._webglTexture = P, this.source = L, this.isVideo = !1, this.isLoaded = !0, this.width = L.width || 0, this.height = L.height || 0, this;
  };
}
const U = (r) => `react-shaders: ${r}`, me = (r) => {
...
function et({
  ...
  onError: A = console.error,
  onWarning: E = console.warn
}) {
  const u = w(null), T = w(null), B = w(null), h = w(null), N = w(void 0), k = w(void 0), P = w(!1), S = w(void 0), L = w(0), F = w([0, 0]), _ = w([]), V = w(0), I = w(void 0), l = w({
    [ne]: { type: "float", isNeeded: !1, value: 0 },
    [oe]: { type: "float", isNeeded: !1, value: 0 },
    [ie]: { type: "vec4", isNeeded: !1, value: [0, 0, 0, 0] },
    [se]: { type: "vec4", isNeeded: !1, value: [0, 0, 0, 0] },
    [ce]: { type: "vec2", isNeeded: !1, value: [0, 0] },
    ...
    u.current && (T.current = u.current.getContext("webgl", y) || u.current.getContext(
      "experimental-webgl",
      y
    ...
    const [a = 0, o = 0] = me(i), c = a - (S.current?.left ?? 0) - window.pageXOffset, s = (S.current?.height ?? 0) - o - (S.current?.top ?? 0) - window.pageYOffset;
    P.current = !0;
    const f = Array.isArray(l.current.iMouse?.value) ? l.current.iMouse.value : void 0;
    f && (f[2] = c, f[3] = s), F.current[0] = c, F.current[1] = s;
  }, $ = (i) => {
    S.current = u.current?.getBoundingClientRect();
    const [a = 0, o = 0] = me(i), c = a - (S.current?.left ?? 0), s = (S.current?.height ?? 0) - o - (S.current?.top ?? 0);
    if (p !== 1)
      F.current[0] = c, F.current[1] = s;
    else {
      const f = Array.isArray(l.current.iMouse?.value) ? l.current.iMouse.value : void 0;
      f && (f[0] = c, f[1] = s);
    }
    ...
    const a = b, o = Math.floor(
      ...
      (S.current?.height ?? 1) * a
    );
    if (i.canvas.width = o, i.canvas.height = c, l.current.iResolution?.isNeeded && h.current) {
...
${a}`));
    ...
    if (h.current = o.createProgram(), !(!h.current || !s || !c)) {
      if (o.attachShader(h.current, s), o.attachShader(h.current, c), o.linkProgram(h.current), !o.getProgramParameter(h.current, o.LINK_STATUS)) {
        A?.(
          U(
      ...
      o.useProgram(h.current), N.current = o.getAttribLocation(
        h.current,
        "aVertexPosition"
      ), o.enableVertexAttribArray(N.current);
    }
  }, Re = () => {
    if (n)
      ...
      if (e && e.length > 0) {
        ...
        const a = e.map(
          (o, c) => (l.current[`${Ye}${c}`] = {
            type: "sampler2D",
            isNeeded: !1
          }, j(o, c), _.current[c] = new Je(i), _.current[c]?.load(o).then((s) => {
            j(s, c);
          }))
        );
        Promise.all(a).then(() => {
    ...
    const a = te.includes(m ?? "highp"), o = `precision ${a ? m : te[1]} float;
`;
    a || E?.(
      U(
        `wrong precision type ${m}, please make sure to pass one of a valid precision lowp, mediump, highp, by default you shader precision will be set to highp.`
      )
    );
...
`).concat(i.replace(/texture\(/g, "texture2D("));
    for (const f of Object.keys(l.current))
      if (i.includes(f)) {
        ...
        c = Ze(
          c,
          `uniform ${x.type} ${f}${x.arraySize || ""}; 
`,
          c.lastIndexOf(o) + o.length
        ), x.isNeeded = !0;
      }
    return i.includes("mainImage") && (c = c.concat(qe)), c;
  }, Se = (i) => {
    const a = T.current;
    ...
    if (l.current.iDate?.isNeeded) {
      const s = /* @__PURE__ */ new Date(), f = s.getMonth() + 1, x = s.getDate(), C = s.getFullYear(), D = s.getHours() * 60 * 60 + s.getMinutes() * 60 + s.getSeconds() + s.getMilliseconds() * 1e-3, z = a.getUniformLocation(
        h.current,
        ie
      );
    ...
    if (l.current.iFrame?.isNeeded) {
      const s = a.getUniformLocation(
        h.current,
        ae
      ), f = l.current.iFrame.value;
      a.uniform1i(s, f), l.current.iFrame.value = f + 1;
    }
    if (_.current.length > 0)
      for (let s = 0; s < _.current.length; s++) {
        const f = _.current[s];
        if (!f) return;
        const { isVideo: x, _webglTexture: C, source: D, flipY: z, isLoaded: Fe } = f;
        if (!Fe || !C || !D) return;
        if (l.current[`iChannel${s}`]?.isNeeded) {
          if (!h.current) return;
          const Ie = a.getUniformLocation(
            h.current,
            `iChannel${s}`
          );
          a.activeTexture(a.TEXTURE0 + s), a.bindTexture(a.TEXTURE_2D, C), a.uniform1i(Ie, s), x && f.updateTexture(
            C,
            D,
            z
          );
    ...
    const a = T.current;
    if (!a) return;
    a.viewport(0, 0, a.drawingBufferWidth, a.drawingBufferHeight), a.clear(a.COLOR_BUFFER_BIT | a.DEPTH_BUFFER_BIT), a.bindBuffer(a.ARRAY_BUFFER, B.current), a.vertexAttribPointer(
      N.current ?? 0,
      3,
      a.FLOAT,
      !1,
      0,
      0
    ), Se(i), a.drawArrays(a.TRIANGLE_STRIP, 0, 4);
    const o = l.current.iMouse?.value;
    if (l.current.iMouse?.isNeeded && p !== 1 && Array.isArray(o)) {
      const c = o[0] ?? 0, s = o[1] ?? 0;
      o[0] = pe(c, F.current[0] ?? 0, p), o[1] = pe(s, F.current[1] ?? 0, p);
    }
    k.current = requestAnimationFrame(J);
  }, Le = () => {
    const i = { passive: !0 };
    l.current.iMouse?.isNeeded && u.current && (u.current.addEventListener("mousemove", $, i), u.current.addEventListener("mouseout", O, i), u.current.addEventListener("mouseup", O, i), u.current.addEventListener("mousedown", X, i), u.current.addEventListener("touchmove", $, i), u.current.addEventListener("touchend", O, i), u.current.addEventListener("touchstart", X, i)), l.current.iDeviceOrientation?.isNeeded && window.addEventListener(
      "deviceorientation",
      K,
      i
    ), u.current && (I.current = new ResizeObserver(q), I.current.observe(u.current), window.addEventListener("resize", q, i));
  }, _e = () => {
    const i = { passive: !0 };
    l.current.iMouse?.isNeeded && u.current && (u.current.removeEventListener("mousemove", $, i), u.current.removeEventListener("mouseout", O, i), u.current.removeEventListener("mouseup", O, i), u.current.removeEventListener("mousedown", X, i), u.current.removeEventListener("touchmove", $, i), u.current.removeEventListener("touchend", O, i), u.current.removeEventListener("touchstart", X, i)), l.current.iDeviceOrientation?.isNeeded && window.removeEventListener(
      "deviceorientation",
      K,
      i
    ), I.current && (I.current.disconnect(), window.removeEventListener("resize", q, i));
  };
  return Y(() => {
    W.current = n;
  }, [n]), Y(() => {
    ...
    function a() {
      we();
      const o = T.current;
      o && u.current && (o.clearColor(...d), o.clearDepth(1), o.enable(o.DEPTH_TEST), o.depthFunc(o.LEQUAL), o.viewport(0, 0, u.current.width, u.current.height), u.current.height = u.current.clientHeight, u.current.width = u.current.clientWidth, Re(), be(), xe(Ae(r || Ge), t || re), ye(), requestAnimationFrame(J), Le(), q());
    }
    return requestAnimationFrame(a), () => {
      ...
      _e(), cancelAnimationFrame(k.current ?? 0);
    };
  }, []), /* @__PURE__ */ M(
    "canvas",
    {
      ref: u,
      style: { height: "100%", width: "100%", ...g }
    }
  );
}
const tt = `
...
function G(r) {
  ...
  return { value: t, motionValue: n, controls: d, animate: m };
}
function st(r, t) {
  const [e, n] = ge(rt), {
    value: d,
    animate: m,
    motionValue: g
  } = G(it), { value: y, animate: p } = G(nt), { value: b, animate: v } = G(ot), { value: A, animate: E } = G(at), u = Xe(t, {
    fftSize: 512,
  ...
  return Y(() => {
    switch (r) {
      ...
      case "failed":
      case "disconnected":
        n(10), m(0.2, R), p(1.2, R), v(0.4, R), E(1, R);
        return;
      case "listening":
      case "pre-connect-buffering":
        n(20), m(0.3, { type: "spring", duration: 1, bounce: 0.35 }), p(1, R), v(0.7, R), E([1.5, 2], ve);
        return;
      case "thinking":
      case "connecting":
      case "initializing":
        n(30), m(0.3, R), p(0.5, R), v(1, R), E([0.5, 2.5], ve);
        return;
      case "speaking":
        n(70), m(0.3, R), p(0.75, R), v(1.25, R), E(1.5, R);
        return;
    }
  }, [
    r,
    ...
    r === "speaking" && u > 0 && !g.isAnimating() && m(0.2 + 0.2 * u, { duration: 0 });
  }, [
    r,
    u,
    g,
...
function Ee({
  ...
  const u = Pe(() => ut(g), [g]);
  return /* @__PURE__ */ M("div", { ref: v, className: A, ...E, children: /* @__PURE__ */ M(
    et,
    {
      fs: tt,
