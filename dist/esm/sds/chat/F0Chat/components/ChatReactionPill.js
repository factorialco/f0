import { cn as e } from "../../../../lib/utils.js";
import { EmojiImage as t, getEmojiLabel as n } from "../../../../lib/emojis.js";
import { TooltipInternal as r } from "../../../../experimental/Overlays/Tooltip/index.js";
import { Action as i } from "../../../../ui/Action/Action.js";
import { useEmojiConfetti as a } from "../../../../lib/emoji-confetti.js";
import { useEffect as o, useRef as s, useState as c } from "react";
import { jsx as l } from "react/jsx-runtime";
import u from "@number-flow/react";
//#region src/sds/chat/F0Chat/components/ChatReactionPill.tsx
var d = ({ emoji: d, initialCount: f, hasReacted: p = !1, users: m, loadUsers: h, onInteraction: g, size: _ = "md" }) => {
	let [v, y] = c(p), [b, x] = c(f), [S, C] = c(m), w = s(null), T = s(null), E = s(0), { fireEmojiConfetti: D } = a();
	o(() => {
		x(f);
	}, [f]), o(() => {
		y(p);
	}, [p]), o(() => {
		E.current += 1, C(m), T.current = null;
	}, [
		d,
		f,
		m
	]);
	let O = () => {
		if (!h || T.current || (S?.length ?? 0) >= f) return;
		let e = h(), t = E.current;
		T.current = e, e.then((e) => {
			E.current === t && C(e);
		}).catch(() => {}).finally(() => {
			T.current === e && (T.current = null);
		});
	}, k = (e) => {
		e.stopPropagation(), x(b + (v ? -1 : 1)), y(!v), g?.(d), v || D(d, w);
	}, A = S?.map((e) => e.name).join(", ") || "", j = /* @__PURE__ */ l(i, {
		ref: w,
		variant: "outline",
		size: "md",
		compact: !0,
		onClick: k,
		className: e("flex items-center gap-1 px-0.5 font-medium leading-tight shadow-none transition-all active:scale-90 motion-reduce:transition-none motion-reduce:active:scale-100", v && "border-f1-border-selected bg-f1-background-selected hover:border-f1-border-selected-bold"),
		"aria-label": `${n(d)}: ${b}`,
		"aria-pressed": v,
		prepend: /* @__PURE__ */ l(t, {
			emoji: d,
			mode: "native",
			size: _
		}),
		children: /* @__PURE__ */ l(u, {
			value: b,
			spinTiming: {
				duration: 200,
				easing: "cubic-bezier(0.175, 0.885, 0.32, 1.275)"
			},
			className: e("tabular-nums", v ? "text-f1-foreground-selected" : "text-f1-foreground")
		})
	});
	return /* @__PURE__ */ l(r, {
		label: A || n(d),
		onOpen: O,
		children: j
	});
};
//#endregion
export { d as ChatReactionPill };
