...
import { r as Ke, m as je, n as Qe, o as Je, p as Me, q as Ze, s as et, t as tt, v as rt, w as nt, x as Ne, y as ot, V as it, z as st, A as at, B as ct, S as ut, H as lt, D as se, E as ft, G as dt, J as ht, K as mt, L as pt, M as vt, N as fe, O as gt } from "./F0AiChat-Bgyj2Zb4.js";
...
function It(t, { defaultTransition: e = {}, ...r } = {}, n, o) {
  ...
  for (let l = 0; l < t.length; l++) {
    ...
    const U = (y, w, _, P = 0, V = 0) => {
      const b = _t(y), { delay: f = 0, times: F = Ze(b), type: O = "keyframes", repeat: k, repeatType: W, repeatDelay: H = 0, ...z } = w;
      ...
      if (re > 0 && tt(F, re), b.length === 1 && b.unshift(null), k) {
        Ne(k < St, "Repeat count too high, must be less than 20"), S = yt(S, k);
        const j = [...b], Q = [...F];
...
function pe(t, e, r, n) {
  const o = Math.max(1, Math.min(t, e)), c = Math.min(r, 20), E = Math.min(c + n, o), v = Math.min(E, Math.floor(t / 2)), R = Math.min(E, Math.floor(e / 2)), p = ($) => $ / t * 2 - 1, g = ($) => $ / e * 2 - 1, x = 0, l = t, T = 0, C = e, h = v, L = t - v, I = R, U = e - R, y = p(x), w = p(l), _ = g(T), P = g(C), V = p(h), b = p(L), f = g(I), F = g(U), O = 0, k = 0, W = 1, H = 1, z = v / t, M = 1 - v / t, S = R / e, N = 1 - R / e, K = new Float32Array([
    // Top strip
    y,
    _,
    w,
