...
import { r as Ke, l as je, m as Qe, n as Je, o as Ne, p as Ze, q as et, s as tt, t as rt, v as nt, w as Me, x as ot, V as it, y as st, z as at, A as ct, S as ut, H as lt, B as se, D as ft, E as dt, G as ht, J as mt, K as pt, L as vt, M as fe, N as gt, O as wt } from "./F0AiChat-Cj87jxVv.js";
...
function Be(t, e) {
  return je(t) ? t[At(0, t.length, e)] : t;
}
function bt(t, e, r) {
...
const St = "easeInOut", It = 20;
function _t(t, { defaultTransition: e = {}, ...r } = {}, n, o) {
  const c = e.duration || 0.3, h = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), v = {}, R = /* @__PURE__ */ new Map();
  let p = 0, g = 0, x = 0;
  for (let l = 0; l < t.length; l++) {
    ...
    const U = (y, w, _, P = 0, V = 0) => {
      const b = Ft(y), { delay: f = 0, times: F = Ze(b), type: O = "keyframes", repeat: z, repeatType: W, repeatDelay: H = 0, ...k } = w;
      let { ease: N = e.ease || "easeOut", duration: S } = w;
      const M = typeof f == "function" ? f(P, V) : f, K = b.length, Z = rt(O) ? O : o?.[O];
      if (K <= 2 && Z) {
        let j = 100;
        if (K === 2 && Ut(b)) {
          const J = b[1] - b[0];
      ...
      if (re > 0 && tt(F, re), b.length === 1 && b.unshift(null), z) {
        Me(z < It, "Repeat count too high, must be less than 20"), S = Et(S, z);
        const j = [...b], Q = [...F];
        ...
        for (let J = 0; J < z; J++) {
          ...
          for (let i = 0; i < j.length; i++)
            F.push(Q[i] + (J + 1)), N.push(i === 0 ? "linear" : Be(ee, i - 1));
        }
        xt(F, z);
      }
      const ne = $ + S;
      Rt(_, b, N, F, $, ne), I = Math.max(M + S, I), x = Math.max(ne, x);
    };
    if (Ne(C)) {
      const y = he(C, E);
      U(m, L, me("default", y));
    } else {
      const y = Oe(C, m, n, v), w = y.length;
      for (let _ = 0; _ < w; _++) {
        m = m, L = L;
        const P = y[_], V = he(P, E);
        for (const b in m)
          U(m[b], Ct(L, b), me(b, V), _, w);
      }
    }
    p = g, g += I;
  }
  return E.forEach((l, T) => {
    for (const C in l) {
      ...
      for (let w = 0; w < m.length; w++) {
        const { at: _, value: P, easing: V } = m[w];
        L.push(P), I.push(ot(0, x, _)), U.push(V || "easeOut");
      }
      I[0] !== 0 && (I.unshift(0), L.unshift(L[0]), U.unshift(St)), I[I.length - 1] !== 1 && (I.push(1), L.push(null)), h.has(T) || h.set(T, {
        keyframes: {},
        transition: {}
      });
      const y = h.get(T);
...
const Pt = (t) => typeof t == "number", Ut = (t) => t.every(Pt);
function Nt(t, e) {
  return t in e;
}
class Mt extends it {
...
function Vt(t, e) {
  return Ne(t) || typeof t == "number" || typeof t == "string" && !ue(e);
}
function Ve(t, e, r, n) {
  const o = [];
  if (Vt(t, e))
    ...
    const c = Oe(t, e, n), h = c.length;
    Me(!!h, "No valid elements provided.");
    for (let E = 0; E < h; E++) {
      const v = c[E], R = v instanceof Element ? Ot : Bt;
      se.has(v) || R(v);
      const p = se.get(v), g = { ...r };
      "delay" in g && typeof g.delay == "function" && (g.delay = g.delay(E, h)), o.push(...dt(p, { ...e, transition: g }, {}));
    }
  }
  return o;
}
function Dt(t, e, r) {
  const n = [];
  return _t(t, e, r, { spring: ht }).forEach(({ keyframes: c, transition: h }, E) => {
    n.push(...Ve(E, c, h));
  }), n;
}
class Gt {
...
function kt(t) {
  function e(r, n, o) {
    let c = [];
    return zt(r) ? c = Dt(r, n, t) : c = Ve(r, n, o, t), new Wt(c);
  }
  return e;
}
const Xt = kt(), Ar = ["xs", "sm", "md", "lg"], br = [
  "inProgress",
  "executing",
  "completed"
], Rr = {
...
function pe(t, e, r, n) {
  const o = Math.max(1, Math.min(t, e)), c = Math.min(r, 20), E = Math.min(c + n, o), v = Math.min(E, Math.floor(t / 2)), R = Math.min(E, Math.floor(e / 2)), p = ($) => $ / t * 2 - 1, g = ($) => $ / e * 2 - 1, x = 0, l = t, T = 0, C = e, m = v, L = t - v, I = R, U = e - R, y = p(x), w = p(l), _ = g(T), P = g(C), V = p(m), b = p(L), f = g(I), F = g(U), O = 0, z = 0, W = 1, H = 1, k = v / t, N = 1 - v / t, S = R / e, M = 1 - R / e, K = new Float32Array([
    // Top strip
    y,
    _,
    w,
...
function Yt(t, e, r) {
  ...
  if (t.attachShader(c, n), t.attachShader(c, o), t.linkProgram(c), !t.getProgramParameter(c, t.LINK_STATUS)) {
    const h = t.getProgramInfoLog(c) || "Unknown link error";
    throw t.deleteProgram(c), t.deleteShader(n), t.deleteShader(o), new Error(h);
  }
...
class Sr {
  ...
  resize(e, r, n) {
    ...
    const { gl: o, program: c, vao: h, positionBuffer: E, uvBuffer: v, uResolution: R } = this.glr, p = n ?? this.options.ratio ?? window.devicePixelRatio ?? 1, g = Math.max(1, Math.floor(e * p)), x = Math.max(1, Math.floor(r * p));
    ...
    o.bindVertexArray(h), o.bindBuffer(o.ARRAY_BUFFER, E), o.bufferData(o.ARRAY_BUFFER, l, o.STATIC_DRAW);
    ...
    const m = o.getAttribLocation(c, "aUV");
  ...
  setupGL() {
    ...
    const o = this.canvas.width || 2, c = this.canvas.height || 2, { positions: h, uvs: E } = pe(
      o,
      c,
    ...
    const x = e.getUniformLocation(r, "uResolution"), l = e.getUniformLocation(r, "uTime"), T = e.getUniformLocation(r, "uBorderWidth"), C = e.getUniformLocation(r, "uGlowWidth"), m = e.getUniformLocation(r, "uBorderRadius"), L = e.getUniformLocation(r, "uColors"), I = e.getUniformLocation(r, "uGlowExponent"), U = e.getUniformLocation(r, "uGlowFactor");
    e.useProgram(r), e.uniform1f(T, this.options.borderWidth), e.uniform1f(C, this.options.glowWidth), e.uniform1f(m, this.options.borderRadius), this.options.mode === "dark" ? (e.uniform1f(I, 2), e.uniform1f(U, 1.8)) : (e.uniform1f(I, 1), e.uniform1f(U, 1));
    const y = (this.options.colors || Ht).map(Kt);
...
}`, Te = "iTime", ye = "iTimeDelta", Ee = "iDate", Ae = "iFrame", be = "iMouse", Re = "iResolution", Jt = "iChannel", xe = "iChannelResolution", Le = "iDeviceOrientation";
function Zt(t, e) {
...
}, ce = 9729, Se = 9728, or = 9987, Ie = 33071, _e = 10497;
class ir {
  gl;
  url;
  wrapS;
  ...
  updateTexture = (e, r, n) => {
    const { gl: o } = this, c = 0, h = o.RGBA, E = o.RGBA, v = o.UNSIGNED_BYTE;
  ...
  load = async (e) => {
    ...
    Object.assign(this, { url: n, wrapS: o, wrapT: c, minFilter: h, magFilter: E, flipY: v });
    const g = 0, x = r.RGBA, l = 1, T = 1, C = 0, m = r.RGBA, L = r.UNSIGNED_BYTE, I = new Uint8Array([255, 255, 255, 0]), U = r.createTexture();
...
}, Ce = (t, e, r) => t * (1 - r) + e * r, sr = (t, e, r) => r > 0 ? t.substring(0, r) + e + t.substring(r, t.length) : e + t;
function ar({
  fs: t,
  vs: e = we,
  textures: r = [],
  ...
  const l = B(null), T = B(null), C = B(null), m = B(null), L = B(void 0), I = B(void 0), U = B(!1), y = B(void 0), w = B(0), _ = B([0, 0]), P = B([]), V = B(0), b = B(void 0), f = B({
    ...
    if (i.canvas.width = s, i.canvas.height = d, f.current.iResolution?.isNeeded && m.current) {
...
${a}`));
    ...
    if (m.current = s.createProgram(), !(!m.current || !u || !d)) {
      if (s.attachShader(m.current, u), s.attachShader(m.current, d), s.linkProgram(m.current), !s.getProgramParameter(m.current, s.LINK_STATUS)) {
        g?.(
          Y(
      ...
      s.useProgram(m.current), L.current = s.getAttribLocation(
        m.current,
        "aVertexPosition"
      ...
      if (r && r.length > 0) {
        ...
        const a = r.map(
          (s, d) => (f.current[`${Jt}${d}`] = {
            type: "sampler2D",
            isNeeded: !1
          }, O(s, d), P.current[d] = new ir(i), P.current[d]?.load(s).then((u) => {
            O(u, d);
          }))
        );
        Promise.all(a).then(() => {
...
`,
    ...
    return i.includes("mainImage") && (d = d.concat(jt)), d;
  }, j = (i) => {
    const a = T.current;
    ...
    const i = { passive: !0 };
    f.current.iMouse?.isNeeded && l.current && (l.current.addEventListener("mousemove", N, i), l.current.addEventListener("mouseout", S, i), l.current.addEventListener("mouseup", S, i), l.current.addEventListener("mousedown", k, i), l.current.addEventListener("touchmove", N, i), l.current.addEventListener("touchend", S, i), l.current.addEventListener("touchstart", k, i)), f.current.iDeviceOrientation?.isNeeded && window.addEventListener(
      "deviceorientation",
      H,
      i
    ), l.current && (b.current = new ResizeObserver(M), b.current.observe(l.current), window.addEventListener("resize", M, i));
  }, J = () => {
    const i = { passive: !0 };
    f.current.iMouse?.isNeeded && l.current && (l.current.removeEventListener("mousemove", N, i), l.current.removeEventListener("mouseout", S, i), l.current.removeEventListener("mouseup", S, i), l.current.removeEventListener("mousedown", k, i), l.current.removeEventListener("touchmove", N, i), l.current.removeEventListener("touchend", S, i), l.current.removeEventListener("touchstart", k, i)), f.current.iDeviceOrientation?.isNeeded && window.removeEventListener(
      "deviceorientation",
      H,
      i
    ), b.current && (b.current.disconnect(), window.removeEventListener("resize", M, i));
  };
  return ie(() => {
    F.current = n;
  }, [n]), ie(() => {
    ...
    function a() {
      z();
      const s = T.current;
      s && l.current && (s.clearColor(...o), s.clearDepth(1), s.enable(s.DEPTH_TEST), s.depthFunc(s.LEQUAL), s.viewport(0, 0, l.current.width, l.current.height), l.current.height = l.current.clientHeight, l.current.width = l.current.clientWidth, $(), re(), Z(ne(t || Qt), e || we), W(), requestAnimationFrame(Q), ee(), M());
    }
    return requestAnimationFrame(a), () => {
      const s = T.current;
      if (s) {
        if (s.getExtension("WEBGL_lose_context")?.loseContext(), s.useProgram(null), s.deleteProgram(m.current ?? null), i.length > 0)
          for (const d of i)
...
}`, ur = 10, lr = 2, fr = 0.5, dr = 0.2, hr = 1.5, G = {
  duration: 0.5,
  ease: "easeOut"
}, Pe = {
  duration: 0.35,
...
function mr(t, e) {
  const [r, n] = Ue(ur), {
    ...
    animate: c,
    motionValue: h
  } = oe(dr), { value: E, animate: v } = oe(lr), { value: R, animate: p } = oe(fr), { value: g, animate: x } = oe(hr), l = Tt(e, {
    fftSize: 512,
  ...
  return ie(() => {
    ...
    t === "speaking" && l > 0 && !h.isAnimating() && c(0.2 + 0.2 * l, { duration: 0 });
  }, [
    t,
    l,
    h,
...
function vr(t) {
  ...
  if (e) {
    const [, r, n, o] = e;
    return [r, n, o].map((h = "00") => parseInt(h, 16) / 255);
  }
}
function Ge({
  shape: t = 1,
...
function Ir({
  ...
  const { speed: R, scale: p, amplitude: g, frequency: x, brightness: l } = mr(e, o);
  return /* @__PURE__ */ q(
    Ge,
    {
      ref: E,
...
const _r = ({
  ...
  const h = c === "destructive";
  return /* @__PURE__ */ le("div", { className: "flex flex-col gap-2", children: [
