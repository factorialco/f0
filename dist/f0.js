...
import { defaultTranslations as iu } from "./i18n-provider-defaults.js";
import './f0.css';const co = {
  xs: 1,
  sm: 2,
  md: 2,
  lg: 2
}, uo = le(function({ widgets: e, children: t }, i) {
  const r = I(null);
  no(i, () => r.current);
...
}), ho = le(function({ children: e, sideContent: t, mainColumnPosition: i = "left", sticky: r = !1 }, s) {
  return g("div", {
    ref: s,
    className: "h-full",
    children: M("div", {
...
}), fo = ({ children: n, className: e }) => g("aside", {
...
const mo = ["noMove", "noResize", "locked", "w", "h", "x", "y"], Me = 200;
function vo(n) {
...
function yo({ children: n, options: e, onResizeStop: t, onChange: i, widgets: r }) {
  const [s, o] = F(null), a = I(null), l = I(!1), u = B(() => ({
    ...e,
    children: (r || []).map((C) => Ye(C))
  }), [e, r]), [c, d] = F(() => {
    const C = /* @__PURE__ */ new Map(), D = r || [], y = (b) => {
      var R;
      b.id && b.content && C.set(b.id, b.content), (R = b.subGridOpts) != null && R.children && b.subGridOpts.children.forEach((E) => {
        y(E);
      });
    };
    return D.forEach((b) => {
  ...
  const [f, x] = F(() => {
    const C = /* @__PURE__ */ new Map(), D = r || [], y = (b) => {
      var R;
      b.id && b._originalContent !== void 0 && C.set(b.id, b._originalContent), (R = b.subGridOpts) != null && R.children && b.subGridOpts.children.forEach((E) => {
        y(E);
      });
    };
    return D.forEach((b) => {
  ...
  const [w, S] = F(() => {
    const C = /* @__PURE__ */ new Map(), D = r || [], y = (b) => {
      ...
      (R = b.subGridOpts) != null && R.children && b.subGridOpts.children.forEach((E) => {
        y(E);
      });
    };
    return D.forEach((b) => {
  ...
  En(() => {
    ...
    if (!Array.isArray(C))
      return;
    const D = C.map((A) => A.id), y = r || [], b = y.map((A) => A.id), R = y.filter((A) => !D.includes(A.id));
    R.length > 0 && (R.forEach((A) => {
      ...
      const m = new Map(A);
      return R.forEach((_) => {
        _.content && m.set(_.id, _.content);
      }), m;
    }), x((A) => {
      const m = new Map(A);
      return R.forEach((_) => {
        _._originalContent !== void 0 && m.set(_.id, _._originalContent);
      }), m;
    }));
    const E = C.filter((A) => !b.includes(A.id));
    if (E.length > 0) {
      const A = E.map((m) => m.id).filter(Boolean);
      A.forEach((m) => {
        setTimeout(() => {
        ...
        return A.forEach((O) => {
          if (_.get(O)) {
            const V = s.el.querySelector(`[gs-id="${O}"] .grid-stack-item-content`);
            let X = "";
            V && (X = vo(V)), _.set(O, g(Ri.div, {
    ...
    const T = y.filter((A) => D.includes(A.id));
    if (T.length > 0) {
      const A = [];
      T.forEach((m) => {
        ...
        if (!_)
          return;
        const O = mo.filter((P) => _[P] !== m[P]);
        if (O.length > 0) {
          const P = {}, V = ["w", "h", "x", "y"], X = ["noMove", "noResize", "locked"], U = O.filter((G) => V.includes(G)), te = O.filter((G) => X.includes(G));
    ...
    if (!s || !u.handle) return;
    s.opts && (s.opts.handle = u.handle);
    const C = setTimeout(() => {
      var D;
      if (s && s.el && u.handle && s.el.querySelectorAll(u.handle).length > 0)
        try {
          !((D = s.opts) != null && D.disableResize) && (s.disable(!1), s.enable(!1));
        } catch {
        }
    }, 0);
    return () => clearTimeout(C);
  }, [s, u.handle, u.children]);
  const N = K(() => {
    if (!s)
      return;
    const C = s.save();
    if (Array.isArray(C)) {
      const D = C.map((y) => {
        const b = y.id;
        if (!b) return null;
        const R = h.current.get(b), E = v.current.get(b), T = y;
        return {
          ...y,
          id: b,
          w: y.w ?? 1,
...
const xo = Ne(null);
function wo() {
  const { _reactContentMap: n } = Yn(), { getWidgetContainer: e } = bo();
  return g(Vt, {
    children: Array.from(n.value.entries()).map(([t, i]) => {
      const r = e(t);
      return r ? g(xo.Provider, {
        value: {
          widget: {
            id: t
          }
  ...
  moveNode(e, t) {
    ...
    if (a.length) {
      ...
      if (d && h && ((c = (u = e.grid) == null ? void 0 : u.opts) != null && c.subGridDynamic) && !e.grid._isTemp) {
        const f = p.areaIntercept(t.rect, h._rect), x = p.area(t.rect), v = p.area(h._rect);
        f / (x < v ? x : v) > 0.8 && (h.grid.makeSubGrid(h.el, void 0, e), h = void 0);
      }
  ...
  cacheLayout(e, t, i = !1) {
    ...
    return e.forEach((s, o) => {
      if (s._id === void 0) {
        ...
        s._id = (a == null ? void 0 : a._id) ?? Se._idSeq++;
      }
...
function Ht(n) {
  xe.touchHandled || (xe.touchHandled = !0, It(n, "mousedown"));
}
function Bt(n) {
  xe.touchHandled && It(n, "mousemove");
}
function Ft(n) {
  if (!xe.touchHandled)
    return;
  ...
  It(n, "mouseup"), e || It(n, "click"), xe.touchHandled = !1;
}
function Wt(n) {
  n.pointerType !== "mouse" && n.target.releasePointerCapture(n.pointerId);
}
function nn(n) {
  z.dragElement && n.pointerType !== "mouse" && Vn(n, "mouseenter");
}
function rn(n) {
  z.dragElement && n.pointerType !== "mouse" && (xe.pointerLeaveTimeout = window.setTimeout(() => {
    delete xe.pointerLeaveTimeout, Vn(n, "mouseleave");
  }, 10));
}
class Jt {
  constructor(e, t, i) {
...
const _o = 'input,textarea,button,select,option,[contenteditable="true"],.ui-resizable-handle';
class mt extends Ni {
  constructor(e, t = {}) {
    var s;
  ...
  _mouseDown(e) {
    if (!z.mouseHandled)
      return e.button !== 0 || !this.dragEls.find((t) => t === e.target) && e.target.closest(_o) || this.option.cancel && e.target.closest(this.option.cancel) || (this.mouseDownEvent = e, delete this.dragging, delete z.dragElement, delete z.dropElement, document.addEventListener("mousemove", this._mouseMove, { capture: !0, passive: !0 }), document.addEventListener("mouseup", this._mouseUp, !0), fe && (e.currentTarget.addEventListener("touchmove", Bt), e.currentTarget.addEventListener("touchend", Ft)), e.preventDefault(), document.activeElement && document.activeElement.blur(), z.mouseHandled = !0), !0;
  }
...
class Oi {
  ...
  setupDroppable(e) {
    return this.ddDroppable ? this.ddDroppable.updateOption(e) : this.ddDroppable = new Eo(this.el, e), this;
  }
  cleanDroppable() {
...
Note: ".grid-stack" is required for proper CSS styling and drag/drop, and is the default selector.`)), i;
  ...
  get placeholder() {
    if (!this._placeholder) {
      this._placeholder = p.createDiv([this.opts.placeholderClass, ae.itemClass, this.opts.itemClass]);
  ...
  constructor(e, t = {}) {
    ...
    const s = {
      ...p.cloneDeep(ae),
      column: p.toNumber(e.getAttribute("gs-column")) || ae.column,
      minRow: i || p.toNumber(e.getAttribute("gs-min-row")) || ae.minRow,
      maxRow: i || p.toNumber(e.getAttribute("gs-max-row")) || ae.maxRow,
      staticGrid: p.toBool(e.getAttribute("gs-static")) || ae.staticGrid,
    ...
    const o = this.el.closest("." + ae.itemClass), a = o == null ? void 0 : o.gridstackNode;
    ...
    const l = t.engineClass || k.engineClass || Se;
  ...
  createWidgetDivs(e) {
    ...
    return p.lazyLoad(e) ? e.visibleObservable || (e.visibleObservable = new IntersectionObserver(([r]) => {
      var s, o;
      r.isIntersecting && ((s = e.visibleObservable) == null || s.disconnect(), delete e.visibleObservable, k.renderCB(i, e), (o = e.grid) == null || o.prepareDragDrop(e.el));
    }), window.setTimeout(() => {
      var r;
  ...
  makeSubGrid(e, t, i, r = !0) {
    ...
    let o, a = this;
    for (; a && !o; )
      o = (x = a.opts) == null ? void 0 : x.subGridOpts, a = (v = a.parentGridNode) == null ? void 0 : v.grid;
    t = p.cloneDeep({
    ...
    if (r && (this._removeDD(s.el), d = { ...s, x: 0, y: 0 }, p.removeInternalForSave(d), delete d.subGridOpts, s.content && (d.content = s.content, delete s.content), k.addRemoveCB ? c = k.addRemoveCB(this.el, d, !0, !1) : (c = p.createDiv(["grid-stack-item"]), c.appendChild(u), u = p.createDiv(["grid-stack-item-content"], s.el)), this.prepareDragDrop(s.el)), i) {
      const w = l ? t.column : s.w, S = s.h + i.h, N = s.el.style;
      N.transition = "none", this.update(s.el, { w, h: S }), setTimeout(() => N.transition = null);
    }
    const h = s.subGrid = k.addGrid(u, t);
  ...
  save(e = !0, t = !1, i = k.saveCB, r) {
    ...
    if (s.forEach((o) => {
      ...
      return delete o._alwaysShowResizeHandle, a !== void 0 ? o.alwaysShowResizeHandle = a : delete o.alwaysShowResizeHandle, p.removeInternalAndSame(o, ae), o.children = s, o;
    }
    return s;
  }
  /**
  ...
  load(e, t = k.addRemoveCB || !0) {
    ...
    l && this.setAnimation(!1), !a && t && [...this.engine.nodes].forEach((d) => {
      if (!d.id)
        return;
      p.find(e, d.id) || (k.addRemoveCB && k.addRemoveCB(this.el, d, !1, !1), o.push(d), this.removeWidget(d.el, !0, !1));
  ...
  update(e, t) {
    return k.getElements(e).forEach((i) => {
      ...
      if (o.some((d) => s[d] !== void 0 && s[d] !== r[d]) && (a = {}, o.forEach((d) => {
        ...
        d && d.textContent !== s.content && (r.content = s.content, k.renderCB(d, s), (c = r.subGrid) != null && c.el && (d.appendChild(r.subGrid.el), r.subGrid._updateContainerHeight())), delete s.content;
      }
      let l = !1, u = !1;
      for (const d in s)
      ...
      (a || l) && this._writeAttr(i, r), u && this.prepareDragDrop(r.el), k.updateCB && k.updateCB(r);
    }), this;
  }
  moveNode(e, t) {
    const i = e._updating;
  ...
  resizeToContent(e) {
    ...
    if (t.subGrid) {
      ...
      const x = t.subGrid.el.getBoundingClientRect(), v = e.getBoundingClientRect();
      u += x.top - v.top;
    } else {
  ...
  static setupDragIn(e, t, i, r = document) {
    (t == null ? void 0 : t.pause) !== void 0 && (z.pauseDrag = t.pause), t = { appendTo: "body", helper: "clone", ...t || {} }, (typeof e == "string" ? p.getElements(e, r) : e).forEach((o, a) => {
      Q.isDraggable(o) || Q.dragIn(o, t), i != null && i[a] && (o.gridstackNode = i[a]);
    });
  }
  /**
  ...
  _setupAcceptWidget() {
    ...
    return Q.droppable(this.el, {
      ...
      a._temporaryRemoved = !0) : (a.w = l, a.h = u, a._temporaryRemoved = !0), k._itemRemoving(a.el, !1), Q.on(s, "drag", i), i(r, s, o), !1;
      ...
      if (delete s._gridstackNodeOrig, l && (c != null && c.grid) && c.grid !== this) {
        const v = c.grid;
        v.engine.removeNodeFromLayoutCache(c), v.engine.removedNodes.push(c), v._triggerRemoveEvent()._triggerChangeEvent(), v.parentGridNode && !v.engine.nodes.length && v.opts.subGridDynamic && v.removeAsSubGrid();
      }
  ...
  _setupRemoveDrop() {
    ...
    return e ? (!this.opts.staticGrid && !Q.isDroppable(e) && Q.droppable(e, this.opts.removableOptions).on(e, "dropover", (t, i) => k._itemRemoving(i, !0)).on(e, "dropout", (t, i) => k._itemRemoving(i, !1)), this) : this;
  }
  /**
  ...
  prepareDragDrop(e, t = !1) {
    ...
    if (!i._initDD) {
      ...
      const u = (h, f) => {
        ...
        const f = i.w !== i._orig.w, x = h.target;
        if (!(!x.gridstackNode || x.gridstackNode.grid !== this)) {
          if (i.el = x, i._isAboutToRemove) {
            const v = e.gridstackNode.grid;
            v._gsEventHandler[h.type] && v._gsEventHandler[h.type](h, x), v.engine.nodes.push(i), v.removeWidget(e, !0, !0);
          } else
    ...
    return Q.draggable(e, r ? "disable" : "enable").resizable(e, s ? "disable" : "enable"), this;
  }
  ...
  _dragOrResize(e, t, i, r, s, o) {
    ...
    const f = Math.round(o * 0.1), x = Math.round(s * 0.1);
    if (u = Math.min(u, x), c = Math.min(c, x), d = Math.min(d, f), h = Math.min(h, f), t.type === "drag") {
      ...
      const S = i.position.left + (i.position.left > r._lastUiPosition.left ? -c : u), N = i.position.top + (i.position.top > r._lastUiPosition.top ? -h : d);
      ...
      if (a.x < 0 || (p.updateScrollResize(t, e, o), a.w = Math.round((i.size.width - u) / s), a.h = Math.round((i.size.height - d) / o), r.w === a.w && r.h === a.h) || r._lastTried && r._lastTried.w === a.w && r._lastTried.h === a.h)
        return;
      const w = i.position.left + u, S = i.position.top + d;
  ...
  _leave(e, t) {
    ...
    if (!i || (t.style.transform = t.style.transformOrigin = null, Q.off(e, "drag"), i._temporaryRemoved))
      return;
  ...
  commit() {
    return Co(this, this.batchUpdate(!1), "commit", "batchUpdate", "5.2"), this;
  }
}
k.renderCB = (n, e) => {
...
function Ro({ children: n }) {
  const { _gridStack: { value: e, set: t }, options: i } = Yn(), r = I(/* @__PURE__ */ new Map()), s = I(null), o = I(i), a = K((c, d) => {
    if (d.id && d.grid) {
      let h = kt.get(d.grid);
      h || (h = /* @__PURE__ */ new Map(), kt.set(d.grid, h)), h.set(d.id, c), r.current.set(d.id, c);
    }
  }, []), l = K(() => {
    if (s.current) {
      k.renderCB = a;
...
const Ai = ({ options: n, widgets: e, onChange: t, className: i }) => {
  const r = B(() => JSON.stringify(e.map((l) => {
    ...
    let d = c[0], h = 1 / 0;
    for (const f of c) {
      const x = f.w - l, v = f.h - u, w = x * x + v * v;
      w < h && (h = w, d = f);
    }
    return d;
  };
  return g(yo, {
    ...
    onResizeStop: (l, u) => {
      ...
      (c.w !== h.w || c.h !== h.h) && ((x = c.grid) == null || x.update(u, {
        w: h.w,
        h: h.h
      }));
    },
...
const So = (n, e, t) => g("div", {
  children: n
}), Zt = ({ widgets: n = [], editMode: e = !1, onChange: t = () => {
}, WidgetWrapper: i = So, main: r = !1, deps: s }) => {
  const o = K((y, b, R) => g(Ri.div, {
    className: "h-full w-full",
    initial: {
      opacity: 0,
      scale: 0.8,
    ...
    if (typeof y.content == "function" && y.deps && b) {
      ...
      return y.deps.forEach((E) => {
        typeof E == "string" && b[E] !== void 0 && (R[E] = b[E]);
      }), y.content(R);
    }
  ...
  }), [c, d] = F(u(n, e)), h = I(e), f = I(n), x = I(!1), v = I(/* @__PURE__ */ new Map()), w = I(n);
  w.current = n;
  const S = I(s), N = B(() => {
    const y = /* @__PURE__ */ new Map();
    return !s || Object.keys(s).length === 0 || n.forEach((b) => {
      if (b.deps && b.deps.length > 0) {
        const R = b.deps.map((E) => typeof E == "string" && s[E] !== void 0 ? s[E] : E).filter((E) => E !== null);
        y.set(b.id, R);
      }
    }), y;
  }, [n, s]), C = K((y) => {
    d(y), x.current || t(y.map((b) => {
      const R = w.current.find((E) => E.id === b.id);
      return {
        id: b.id,
        w: b.w ?? 1,
        h: b.h ?? 1,
        ...
        meta: b.meta,
        content: typeof (R == null ? void 0 : R.content) == "function" ? R.content : b._originalContent,
        x: b.x ?? 0,
        y: b.y ?? 0,
        locked: b.locked,
  ...
  }, [t]), D = (y, b) => !y && !b ? !1 : !y || !b || y.length !== b.length ? !0 : y.some((R, E) => R !== b[E]);
  return H(() => {
    const y = h.current !== e, b = f.current !== n, R = S.current !== s && (S.current === void 0 || s === void 0 || Object.keys(S.current).length !== Object.keys(s).length || Object.keys(s).some((m) => {
      var _;
      return ((_ = S.current) == null ? void 0 : _[m]) !== s[m];
    })), E = /* @__PURE__ */ new Map();
    n.forEach((m) => {
      if (m.deps && m.deps.length > 0) {
        const _ = v.current.get(m.id), O = N.get(m.id);
        E.set(m.id, D(_, O)), O ? v.current.set(m.id, O) : v.current.delete(m.id);
      }
    });
    const T = new Set(n.map((m) => m.id));
    v.current.forEach((m, _) => {
      T.has(_) || v.current.delete(_);
    });
    const A = Array.from(E.values()).some((m) => m) || R;
    y && !b && !A ? (x.current = !0, d((m) => m.map((_) => {
      const O = n.find((V) => V.id === _.id);
      if (!O)
        return _;
      const P = l(O, s);
      return {
        ...
        meta: O.meta,
        _originalContent: P,
        content: o(P, O.meta, e)
      };
    }))) : (b || A) && d((m) => {
      const _ = new Map(m.map((O) => [O.id, O]));
      return n.map((O) => {
        const P = _.get(O.id), V = E.get(O.id) ?? !1;
        let X;
        V || !P ? X = l(O, s) : X = P._originalContent ?? l(O, s);
        const U = {
          id: O.id,
          h: (P == null ? void 0 : P.h) ?? O.h ?? 1,
          w: (P == null ? void 0 : P.w) ?? O.w ?? 1,
        ...
        }, te = (P == null ? void 0 : P.x) ?? O.x, G = (P == null ? void 0 : P.y) ?? O.y;
...
}), Jn = le(Ao), ko = ["append", "className", "pressed", "compact", "noTitle", "noAutoTooltip", "style", "variant", "loading", "emoji"], Zn = le((n, e) => {
  const t = ko.reduce((i, r) => {
    const { [r]: s, ...o } = i;
    return o;
  }, n);
  return g(Si, {
...
const hi = Di({
  ...
  "info-strong": "p",
  code: "code"
}, ki = le(({ content: n, variant: e, align: t, className: i, as: r, ellipsis: s, noEllipsisTooltip: o, markdown: a, ...l }, u) => {
  const c = r ?? To[e ?? "body"];
  if (s !== void 0)
    return g(jr, {
      ref: u,
...
const Dc = [
  ...
  const [u, c] = F(!1), [d, h] = F(!1), f = Rn(), x = (w) => {
    c(w);
  }, v = d || u;
  return H(() => {
    if (!r || !i) return;
...
const Po = Kr(er, zo), Mo = ({ children: n, title: e, draggable: t = !1, actions: i, aiButton: r }) => g(Po, {
  title: e,
  draggable: t,
  actions: i,
  AIButton: r,
...
const Lo = Oo("Dashboard", tr), Rc = Re("Dashboard", Lo), Io = Di({
...
}), Ho = (n) => (n || []).map((e) => e.items).reduce((e, t) => (e.length > 0 && e.push({
  type: "separator"
}), e.push(...t), e), []), Bo = (n) => {
  ...
  const d = B(() => Bo(u.actions || []), [u.actions]), h = B(() => d.flatMap((x) => x.items), [d]), f = B(() => h.length > 0 || !!l, [h, l]);
  return M("div", {
    ref: c,
    className: j(Io({
      variant: e
...
const Fo = ({ title: n = "", description: e, titleLevel: t = "h2", children: i, className: r, ...s }) => {
  if (!n) return null;
  const o = t;
  return M(Qt, {
    ...s,
...
}, Wo = No("BlockContent", Fo), Go = (n) => !jn(n) || !n.type || typeof n.type == "string" || typeof n.type == "symbol" ? !1 : "__isPageLayoutBlock" in n.type, $o = (n) => !jn(n) || !n.type || typeof n.type == "string" || typeof n.type == "symbol" ? !1 : "__isPageLayoutGroup" in n.type, ir = (n, e, t) => {
  const i = Lt.toArray(e);
  for (const r of i)
    t.includes("block") && Go(r) || t.includes("group") && $o(r) || console.warn(
      `${n}: Children components must inherit from PageLayoutBlock or PageLayoutGroup. Found:`,
      r
    );
}, Ti = le(({ children: n, onSort: e, ...t }, i) => {
  ir("GroupLinear", n, ["block"]);
  const [r, s] = F(Lt.toArray(n));
  return H(() => {
    s(Lt.toArray(n));
  }, [n]), H(() => {
    e == null || e(r);
...
const qe = /* @__PURE__ */ rr(1), yt = /* @__PURE__ */ rr(-1);
function Ko(n) {
...
function ea() {
  const [n] = F(() => /* @__PURE__ */ new Set()), e = K((i) => (n.add(i), () => n.delete(i)), [n]);
  return [K((i) => {
    let {
      type: r,
      event: s
...
function na(n) {
  let {
    ...
    screenReaderInstructions: r = ta
  } = n;
  const {
    announce: s,
    announcement: o
  } = Zo(), a = Ct("DndLiveRegion"), [l, u] = F(!1);
  if (H(() => {
    u(!0);
  }, []), Qo(B(() => ({
    onDragStart(d) {
    ...
    return null;
  const c = L.createElement(L.Fragment, null, L.createElement(Vo, {
    id: i,
    value: r.draggable
  }), L.createElement(Jo, {
    id: a,
...
const ca = (n) => {
  ...
  for (const o of i) {
    ...
    if (l) {
      const u = an(l), c = r.reduce((h, f, x) => h + sa(u[x], f), 0), d = Number((c / 4).toFixed(4));
      s.push({
        id: a,
        data: {
...
function ni(n, e) {
  ...
  function i(r) {
    ...
    return r !== n && ba(r, s) && t.push(r), ya(r, s) ? t : i(r.parentNode);
  }
  return n ? i(n) : t;
}
function cr(n) {
...
function wa(n, e, t, i, r) {
  ...
  return !u && s <= e.top + v.height ? (f.y = q.Backward, x.y = i * Math.abs((e.top + v.height - s) / v.height)) : !c && l >= e.bottom - v.height && (f.y = q.Forward, x.y = i * Math.abs((e.bottom - v.height - l) / v.height)), !h && a >= e.right - v.width ? (f.x = q.Forward, x.x = i * Math.abs((e.right - v.width - a) / v.width)) : !d && o <= e.left + v.width && (f.x = q.Backward, x.x = i * Math.abs((e.left + v.width - o) / v.width)), {
    direction: f,
    speed: x
  };
}
...
function gr(n) {
  return n.reduce((e, t) => qe(e, bi(t)), pe);
}
function _a(n) {
...
const Da = [["x", ["left", "right"], _a], ["y", ["top", "bottom"], Ea]];
class Mi {
  constructor(e, t) {
...
class Li {
  constructor(e) {
    ...
    this.props = e, this.listeners = new dt(et(t)), this.windowListeners = new dt(ee(t)), this.handleKeyDown = this.handleKeyDown.bind(this), this.handleCancel = this.handleCancel.bind(this), this.attach();
  }
  attach() {
      ...
      if (d) {
        ...
        for (const v of x) {
          const w = e.code, {
            ...
            x: Math.min(w === W.Right ? R.right - R.width / 2 : R.right, Math.max(w === W.Right ? R.left : R.left + R.width / 2, d.x)),
            y: Math.min(w === W.Down ? R.bottom - R.height / 2 : R.bottom, Math.max(w === W.Down ? R.top : R.top + R.height / 2, d.y))
          }, T = w === W.Right && !N || w === W.Left && !C, A = w === W.Down && !D || w === W.Up && !S;
          if (T && E.x !== d.x) {
            const m = v.scrollLeft + h.x, _ = w === W.Right && m <= y.x || w === W.Left && m >= b.x;
            if (_ && !h.y) {
              v.scrollTo({
                left: m,
                behavior: a
            ...
            _ ? f.x = v.scrollLeft - m : f.x = w === W.Right ? v.scrollLeft - y.x : v.scrollLeft - b.x, f.x && v.scrollBy({
              left: -f.x,
              behavior: a
            });
            break;
          } else if (A && E.y !== d.y) {
            const m = v.scrollTop + h.y, _ = w === W.Down && m <= y.y || w === W.Up && m >= b.y;
            if (_ && !h.x) {
              v.scrollTo({
                top: m,
                behavior: a
            ...
            _ ? f.y = v.scrollTop - m : f.y = w === W.Down ? v.scrollTop - y.y : v.scrollTop - b.y, f.y && v.scrollBy({
              top: -f.y,
              behavior: a
            });
            break;
...
class Ii {
  constructor(e, t, i) {
    var r;
    i === void 0 && (i = Ra(e.event.target)), this.props = void 0, this.events = void 0, this.autoScrollEnabled = !0, this.document = void 0, this.activated = !1, this.initialCoordinates = void 0, this.timeoutId = null, this.listeners = void 0, this.documentListeners = void 0, this.windowListeners = void 0, this.props = e, this.events = t;
    const {
      event: s
    } = e, {
      target: o
    } = s;
    this.props = e, this.events = t, this.document = et(o), this.documentListeners = new dt(this.document), this.listeners = new dt(i), this.windowListeners = new dt(ee(o)), this.initialCoordinates = (r = jt(s)) != null ? r : pe, this.handleStart = this.handleStart.bind(this), this.handleMove = this.handleMove.bind(this), this.handleEnd = this.handleEnd.bind(this), this.handleCancel = this.handleCancel.bind(this), this.handleKeydown = this.handleKeydown.bind(this), this.removeTextSelection = this.removeTextSelection.bind(this), this.attach();
  }
  attach() {
    const {
      events: e,
  ...
  handleStart() {
    ...
    e && (this.activated = !0, this.documentListeners.add(he.Click, Sa, {
      capture: !0
...
function za(n) {
  ...
  const f = Ma({
    ...
    if (!b)
      return;
    const R = w.current.x * S.current.x, E = w.current.y * S.current.y;
    b.scrollBy(R, E);
  }, []), y = B(() => a === Kt.TreeOrder ? [...u].reverse() : u, [a, u]);
  H(
    () => {
      if (!s || !u.length || !N) {
        v();
      ...
      for (const b of y) {
        if ((i == null ? void 0 : i(b)) === !1)
          continue;
        const R = u.indexOf(b), E = c[R];
        if (!E)
          continue;
        const {
          direction: T,
        ...
        for (const m of ["x", "y"])
          f[m][T[m]] || (A[m] = 0, T[m] = 0);
        if (A.x > 0 || A.y > 0) {
          v(), C.current = b, x(D, o), w.current = A, S.current = T;
          return;
        }
      }
      w.current = {
...
function Ha(n, e) {
  ...
  const [s, o] = F(null), {
    ...
    measure: l,
    strategy: u
  } = r, c = I(n), d = w(), h = vt(d), f = K(function(S) {
    ...
    if (!S || S === hn || c.current !== n || s != null) {
      ...
      for (let C of n) {
        ...
        const D = C.node.current, y = D ? new Mi(l(D), D) : null;
...
function ja(n) {
  ...
  }, [r, n]), B(() => n.length ? e ? Array.from(e.values()).reduce((s, o) => qe(s, o), pe) : gr(n) : pe, [n, e]);
}
function pn(n, e) {
  e === void 0 && (e = []);
  const t = I(null);
...
function vr(n) {
  return B(() => n ? va(n) : null, [n]);
}
const mn = [];
function Xa(n, e) {
  e === void 0 && (e = tt);
  const [t] = n, i = vr(t ? ee(t) : null), [r, s] = F(mn);
  function o() {
...
const Ja = {
  ...
  over: null,
  measureDroppableContainers: Ut
}, _t = /* @__PURE__ */ Ne(br), xr = /* @__PURE__ */ Ne(Ja);
function Za() {
  return {
    draggable: {
      active: null,
...
function el(n) {
  ...
  const {
    ...
    activatorEvent: i,
    draggableNodes: r
  } = we(_t), s = $t(i), o = $t(t == null ? void 0 : t.id);
  return H(() => {
    if (!e && !i && s && o != null) {
...
const nl = /* @__PURE__ */ so(function(e) {
  var t, i, r, s;
  let {
    id: o,
    accessibility: a,
    ...
    children: u,
    sensors: c = qa,
    collisionDetection: d = ua,
    measuring: h,
    modifiers: f,
    ...x
  } = e;
  const v = oo(Qa, void 0, Za), [w, S] = v, [N, C] = ea(), [D, y] = F(Le.Uninitialized), b = D === Le.Initialized, {
    draggable: {
      active: R,
      nodes: E,
      translate: T
    ...
    return R != null ? {
      ...
      data: (Z = m == null ? void 0 : m.data) != null ? Z : Va,
      rect: _
    } : null;
  }, [R, m]), P = I(null), [V, X] = F(null), [U, te] = F(null), G = vt(x, Object.values(x)), ne = Ct("DndDescribedBy", o), Dt = B(() => A.getEnabled(), [A]), ie = tl(h), {
    droppableRects: Ce,
    ...
    config: ie.droppable
  }), ce = La(E, R), Rt = B(() => U ? jt(U) : null, [U]), Oe = Gr(), _e = Ba(ce, ie.draggable.measure);
  il({
    activeNode: R != null ? E.get(R) : null,
    config: Oe.layoutShiftCompensation,
    initialRect: _e,
    measure: ie.draggable.measure
  });
  const $ = fn(ce, ie.draggable.measure, _e), nt = fn(ce ? ce.parentElement : null), ve = I({
    activatorEvent: null,
    active: null,
    activeNode: ce,
    collisionRect: null,
  ...
  }), We = A.getNodeFor((t = ve.current.over) == null ? void 0 : t.id), Ee = Ya({
    measure: ie.dragOverlay.measure
  }), Ge = (i = Ee.nodeRef.current) != null ? i : ce, $e = b ? (r = Ee.rect) != null ? r : $ : null, Gi = !!(Ee.nodeRef.current && Ee.rect), $i = Ga(Gi ? null : $), ai = vr(Ge ? ee(Ge) : null), Ae = $a(b ? We ?? ce : null), St = Xa(Ae), Nt = wr(f, {
    transform: {
      x: T.x - $i.x,
      y: T.y - $i.y,
      scaleX: 1,
    ...
    windowRect: ai
  }), ji = Rt ? qe(Rt, T) : null, Ui = ja(Ae), Mr = pn(Ui), Lr = pn(Ui, [$]), je = qe(Nt, Mr), Ue = $e ? ga($e, Nt) : null, rt = O && Ue ? d({
    active: O,
    collisionRect: Ue,
    droppableRects: Ce,
    droppableContainers: Dt,
    pointerCoordinates: ji
  }) : null, Ki = or(rt, "id"), [ke, Xi] = F(null), Ir = Gi ? Nt : qe(Nt, Lr), Hr = ha(Ir, (s = ke == null ? void 0 : ke.rect) != null ? s : null, $), li = I(null), Yi = K(
    (Z, re) => {
      let {
        sensor: se,
        options: Te
      ...
      const oe = Z.nativeEvent, ye = new se({
        ...
        onStart(J) {
          ...
          At(() => {
            De == null || De(st), y(Le.Initializing), S({
              type: Y.DragStart,
        ...
        return async function() {
          ...
          if (be && ot) {
            ...
            Pe = {
              ...
              delta: ot,
              over: st
            }, J === Y.DragEnd && typeof at == "function" && await Promise.resolve(at(Pe)) && (J = Y.DragCancel);
          }
          P.current = null, At(() => {
            S({
              type: J
            }), y(Le.Uninitialized), Xi(null), X(null), te(null), li.current = null;
    ...
    const de = se.nativeEvent, oe = E.get(Te);
    if (
    ...
    }, P.current = Te, Yi(se, re));
  }, [E, Yi]), qi = Ia(c, Br);
  Ua(c), ge(() => {
    $ && D === Le.Initializing && y(Le.Initialized);
  }, [$, D]), H(
    () => {
      const {
        onDragMove: Z
    ...
    () => {
      ...
      const {
        onDragOver: oe
      } = G.current, ye = Te.get(Ki), Ke = ye && ye.rect.current ? {
        id: ye.id,
  ...
  }, [O, ce, rt, Ue, E, Ge, $e, Ce, A, ke, Ae, je]), za({
    ...Oe,
    delta: T,
    draggingRect: Ue,
    pointerCoordinates: ji,
  ...
  const Fr = B(() => ({
    ...
    measuringScheduled: it,
    windowRect: ai
  }), [O, ce, $, U, rt, nt, Ee, E, A, Ce, ke, Be, Ae, St, ie, it, ai]), Wr = B(() => ({
    activatorEvent: U,
    activators: qi,
    active: O,
    activeNodeRect: $,
  ...
  function Gr() {
    const Z = (V == null ? void 0 : V.autoScrollEnabled) === !1, re = typeof l == "object" ? l.enabled === !1 : l === !1, se = b && !Z && !re;
...
}), rl = /* @__PURE__ */ Ne(null), vn = "button", sl = "Draggable";
function ol(n) {
  let {
    id: e,
    data: t,
  ...
  const s = Ct(sl), {
    ...
    tabIndex: v = 0
  } = r ?? {}, w = (l == null ? void 0 : l.id) === e, S = we(w ? si : rl), [N, C] = Gt(), [D, y] = Gt(), b = Ka(o, e), R = vt(t);
  ge(
    () => (d.set(e, {
      id: e,
      key: s,
  ...
  const s = Ct(al), {
    ...
    () => {
      ...
      f.current != null && clearTimeout(f.current), f.current = setTimeout(() => {
        u(Array.isArray(S.current) ? S.current : [S.current]), f.current = null;
      }, w);
    },
    //eslint-disable-next-line react-hooks/exhaustive-deps
    [w]
    ...
    C && (T && (C.unobserve(T), d.current = !1), E && C.observe(E));
  }, [C]), [y, b] = Gt(D), R = vt(e);
  return H(() => {
...
function dl(n) {
  ...
  const [i, r] = F(null), [s, o] = F(null), a = $t(t);
  return !t && !i && a && r(a), ge(() => {
    if (!s)
      return;
  ...
  }, [e, i, s]), L.createElement(L.Fragment, null, t, i ? ao(i, {
    ref: o
  }) : null);
}
const ul = {
...
const fl = {
  position: "fixed",
  touchAction: "none"
}, gl = (n) => ii(n) ? "transform 250ms ease" : void 0, pl = /* @__PURE__ */ le((n, e) => {
  let {
    as: t,
    activatorEvent: i,
    adjustScale: r,
  ...
  const d = r ? u : {
    ...
    transformOrigin: r && i ? oa(i, a) : void 0,
  ...
  return [{
    transform: He.Transform.toString(e)
  }, {
...
const Cl = /* @__PURE__ */ L.memo((n) => {
  ...
  const {
    ...
    windowRect: y
  } = Cr(), b = we(si), R = wl(d == null ? void 0 : d.id), E = wr(o, {
    activatorEvent: c,
    active: d,
    activeNodeRect: h,
    containerNodeRect: f,
  ...
  return L.createElement(hl, null, L.createElement(dl, {
    animation: A
  }, d && R ? L.createElement(pl, {
    key: R,
    id: d.id,
    ref: m,
    as: a,
...
function Rl(n) {
  ...
  } = Cr(), d = Ct(Er, t), h = a.rect !== null, f = B(() => i.map((b) => typeof b == "object" && "id" in b ? b.id : b), [i]), x = o != null, v = o ? f.indexOf(o.id) : -1, w = u ? f.indexOf(u.id) : -1, S = I(f), N = !El(f, S.current), C = w !== -1 && v === -1 || N, D = Dl(s);
  ge(() => {
    N && x && c(f);
  }, [N, f, x, c]), H(() => {
    S.current = f;
  ...
  const y = B(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [v, d, D.draggable, D.droppable, C, f, w, l, h, r]
  );
...
const Sl = (n) => {
  ...
  duration: 200,
  easing: "ease"
}, Rr = "transform", Al = /* @__PURE__ */ He.Transition.toString({
  property: Rr,
  duration: 0,
  easing: "linear"
}), kl = {
...
function zl(n) {
  ...
  const {
    ...
    useDragOverlay: S,
    strategy: N
  } = we(Dr), C = Pl(i, f), D = c.indexOf(o), y = B(() => ({
    sortable: {
      containerId: d,
      index: D,
      items: c
    ...
    ...r
  }), [d, r, D, c]), b = B(() => c.slice(c.indexOf(o)), [c, o]), {
    rect: R,
    node: E,
    isOver: T,
    setNodeRef: A
  ...
  }), Dt = jo(A, V), ie = !!m, Ce = ie && !x && Tt(h) && Tt(w), Be = !S && U, it = Be && Ce ? ne : null, Rt = Ce ? it ?? (a ?? N)({
    rects: v,
    activeNodeRect: O,
    activeIndex: h,
    overIndex: w,
  ...
  return H(() => {
    ie && $.current.newIndex !== Oe && ($.current.newIndex = Oe), d !== $.current.containerId && ($.current.containerId = d), c !== $.current.items && ($.current.items = c);
  }, [ie, Oe, d, c]), H(() => {
    if (_e === $.current.activeId)
      return;
    if (_e != null && $.current.activeId == null) {
      $.current.activeId = _e;
      return;
    }
    const Ge = setTimeout(() => {
...
const Ml = [W.Down, W.Right, W.Up, W.Left], Ll = (n, e) => {
  let {
    context: {
      active: t,
      collisionRect: i,
  ...
  if (Ml.includes(n.code)) {
    ...
    if (c === (o == null ? void 0 : o.id) && u.length > 1 && (c = u[1].id), c != null) {
      ...
      if (x && f && d && h) {
        const w = ni(x).some((b, R) => a[R] !== b), S = Sr(d, h), N = Il(d, h), C = w || !S ? {
          x: 0,
          y: 0
        } : {
          x: N ? i.width - f.width : 0,
          y: N ? i.height - f.height : 0
        }, D = {
          x: f.left,
          y: f.top
        };
        return C.x && C.y ? D : yt(D, C);
      }
    }
  }
};
...
const bn = ({ id: n, children: e }) => {
  const { attributes: t, listeners: i, setNodeRef: r, transform: s, transition: o } = zl({
    id: n
  }), a = {
  ...
  const [o, a] = F(null), l = ra(on(Hi), on(Li, {
    coordinateGetter: Ll
  })), u = (h) => {
    a(h.active.id);
  }, c = (h) => {
    const { active: f, over: x } = h;
    a(null), x && f.id !== x.id && s((v) => {
      const w = v.findIndex((N) => N.id === f.id), S = v.findIndex((N) => N.id === x.id);
      return Fi(v, w, S);
    });
  };
  return g("div", {
...
const Hl = le(function({ children: e, aside: t, header: i, variant: r = "main-aside" }, s) {
  ...
  Page: Re("Page", Hl),
  Block: Re("Block", Qt),
  BlockContent: Re("BlockContent", Wo),
  Group: Re("Group", Ti),
...
function Wl(n, e, t) {
  return e && Fl(n.prototype, e), Object.defineProperty(n, "prototype", { writable: !1 }), n;
}
function Gl(n, e, t) {
  return e = Yt(e), $l(n, Nr() ? Reflect.construct(e, t || [], Yt(n).constructor) : e.apply(n, t));
}
function $l(n, e) {
...
var oi = /* @__PURE__ */ function(n) {
  function e() {
    return Bl(this, e), Gl(this, e, arguments);
  }
  return Ul(e, n), Wl(e, [{
    key: "render",
    value: function() {
...
function Vl(n) {
  var e = n.option, t = n.isActive, i = Yl(n, Xl);
  return typeof e == "string" ? /* @__PURE__ */ L.createElement(Vi, ft({
    option: /* @__PURE__ */ L.createElement(Zr, ft({
      type: e
    }, i)),
    isActive: t,
    shapeType: "symbols"
  }, i)) : /* @__PURE__ */ L.createElement(Vi, ft({
    option: e,
    isActive: t,
    shapeType: "symbols"
  }, i));
...
function Ql(n, e, t) {
  return e = qt(e), ec(n, kr() ? Reflect.construct(e, t || [], qt(n).constructor) : e.apply(n, t));
}
function ec(n, e) {
...
var Et = /* @__PURE__ */ function(n) {
  function e() {
    ...
    for (var i = arguments.length, r = new Array(i), s = 0; s < i; s++)
      r[s] = arguments[s];
    return t = Ql(this, e, [].concat(r)), Ie(t, "state", {
      isAnimationFinished: !1
    }), Ie(t, "handleAnimationEnd", function() {
      t.setState({
        isAnimationFinished: !0
      });
    }), Ie(t, "handleAnimationStart", function() {
      t.setState({
        isAnimationFinished: !1
      });
    }), Ie(t, "id", os("recharts-scatter-")), t;
  }
  return ic(e, n), Zl(e, [{
    ...
    value: function(i) {
      ...
      return i.map(function(c, d) {
        ...
        return /* @__PURE__ */ L.createElement(lt, gt({
          ...
          role: "img"
        }), /* @__PURE__ */ L.createElement(Vl, gt({
          option: f,
          isActive: h,
          key: "symbol-".concat(d)
        }, x)));
    ...
    value: function() {
      ...
      return /* @__PURE__ */ L.createElement(es, {
        begin: a,
        duration: l,
        isActive: o,
        easing: u,
        ...
        var f = h.t, x = s.map(function(v, w) {
          var S = d && d[w];
          if (S) {
            var N = Ot(S.cx, v.cx), C = Ot(S.cy, v.cy), D = Ot(S.size, v.size);
            return ue(ue({}, v), {}, {
              cx: N(f),
              cy: C(f),
              size: D(f)
        ...
        return /* @__PURE__ */ L.createElement(lt, null, i.renderSymbolsStatically(x));
      });
    }
  }, {
    key: "renderSymbols",
    ...
    value: function() {
      ...
      if (i && !this.state.isAnimationFinished)
        return null;
      var r = this.props, s = r.points, o = r.xAxis, a = r.yAxis, l = r.children, u = Nn(l, ts);
      return u ? u.map(function(c, d) {
        var h = c.props, f = h.direction, x = h.dataKey;
        return /* @__PURE__ */ L.cloneElement(c, {
          key: "".concat(f, "-").concat(x, "-").concat(s[d]),
          data: s,
          xAxis: o,
          yAxis: a,
    ...
    value: function() {
      ...
      if (o === "joint")
        ...
        var h = is(r), f = h.xmin, x = h.xmax, v = h.a, w = h.b, S = function(D) {
          return v * D + w;
        };
        c = [{
          x: f,
      ...
      return /* @__PURE__ */ L.isValidElement(s) ? d = /* @__PURE__ */ L.cloneElement(s, N) : ns(s) ? d = s(N) : d = /* @__PURE__ */ L.createElement(rs, gt({}, N, {
        type: a
    ...
    value: function() {
      ...
      if (r || !s || !s.length)
        return null;
      var w = this.state.isAnimationFinished, S = ss("recharts-scatter", a), N = l && l.allowDataOverflow, C = u && u.allowDataOverflow, D = N || C, y = Xe(x) ? this.id : x;
      return /* @__PURE__ */ L.createElement(lt, {
        className: S,
        clipPath: D ? "url(#clipPath-".concat(y, ")") : null
      }, N || C ? /* @__PURE__ */ L.createElement("defs", null, /* @__PURE__ */ L.createElement("clipPath", {
...
Ie(Et, "defaultProps", {
  ...
  shape: "circle",
  hide: !1,
  isAnimationActive: !as.isSsr,
  animationBegin: 0,
  animationDuration: 400,
  animationEasing: "linear"
});
Ie(Et, "getComposedData", function(n) {
  var e = n.xAxis, t = n.yAxis, i = n.zAxis, r = n.item, s = n.displayedData, o = n.xAxisTicks, a = n.yAxisTicks, l = n.offset, u = r.props.tooltipType, c = Nn(r.props.children, ls), d = Xe(e.dataKey) ? r.props.dataKey : e.dataKey, h = Xe(t.dataKey) ? r.props.dataKey : t.dataKey, f = i && i.dataKey, x = i ? i.range : oi.defaultProps.range, v = x && x[0], w = e.scale.bandwidth ? e.scale.bandwidth() : 0, S = t.scale.bandwidth ? t.scale.bandwidth() : 0, N = s.map(function(C, D) {
    var y = zt(C, d), b = zt(C, h), R = !Xe(f) && zt(C, f) || "-", E = [{
    ...
    var T = Ji({
      ...
      index: D,
      dataKey: h
    }), m = R !== "-" ? i.scale(R) : v, _ = Math.sqrt(Math.max(m, 0) / Math.PI);
    return ue(ue({}, C), {}, {
      cx: T,
      cy: A,
      x: T - _,
...
const sc = (n) => {
...
}, oc = ({ dataConfig: n, data: e, xAxis: t, yAxis: i = {
  hide: !0
}, label: r = !1, hideTooltip: s = !1, hideGrid: o = !1, aspect: a, legend: l, showValueUnderLabel: u = !1, bar: c, line: d, scatter: h, onClick: f }, x) => {
  var R, E, T, A;
  const v = hs(e), w = c != null && c.categories ? Array.isArray(c.categories) ? c.categories : [c.categories] : [], S = d != null && d.categories ? Array.isArray(d.categories) ? d.categories : [d.categories] : [], N = h != null && h.categories ? Array.isArray(h.categories) ? h.categories : [h.categories] : [], C = [...w, ...S, ...N], D = Math.max(...v.flatMap((m) => C.map((_) => fs(i != null && i.tickFormatter ? i.tickFormatter(`${m[_]}`) : `${m[_]}`)))), y = [c, d, h].filter((m) => (m == null ? void 0 : m.axisPosition) === "left"), b = [c, d, h].filter((m) => (m == null ? void 0 : m.axisPosition) === "right");
  return g(gs, {
    config: n,
    ref: x,
    aspect: a,
    children: M(rc, {
      ...
      onClick: (m) => {
        ...
        for (const O of m.activePayload)
      ...
      children: [!s && g(ps, {
        ...
        ...Zi(i),
        tick: !0,
        width: i.width ?? D + 20 + (b.length > 0 && ((R = y[0]) != null && R.axisLabel) ? 20 : 0),
        hide: i.hide || y.some((m) => m == null ? void 0 : m.hideAxis),
        ...
        orientation: "right",
        tick: !0,
        width: i.width ?? D + 20 + (y.length > 0 && ((T = b[0]) != null && T.axisLabel) ? 20 : 0),
        ...
        tick: u ? (m) => {
          var te, G;
          const { x: _, y: O, payload: P } = m, V = ((te = e.find((ne) => ne.label === P.value)) == null ? void 0 : te.values) || "", X = Object.keys(V).length === 1 ? (G = Object.values(V)) == null ? void 0 : G[0] : void 0, U = X !== void 0 && i.tickFormatter ? i.tickFormatter(`${X}`) : X.toLocaleString();
          return M("g", {
      ...
      }, `line-${String(m)}`)), N.map((m, _) => g(Et, {
        dataKey: String(m),
        fill: n[m].color ? ct(n[m].color) : ui(w.length + S.length + _),
        r: 4,
        isAnimationActive: !1,
        yAxisId: (h == null ? void 0 : h.axisPosition) === "right" ? "right" : void 0,
        shape: sc(String(m))
      }, `scatter-${String(m)}`)), l && g(ws, {
        content: g(Cs, {
          nameKey: "label"
        }),
        align: "center",
...
}, ac = zn(oc), lc = ({ value: n, max: e = 100, label: t, color: i }, r) => {
  ...
  (n) => n !== "ai"
), Fc = Pn, Wc = ["default", "outline", "neutral"], Gc = Pn, $c = ["sm", "md", "lg"], jc = ks, Ei = ({ count: n, list: e }) => {
  const [t, i] = F(!1), r = g(Mt, {
    label: `+${n}`
  });
  return e != null && e.length ? M(Mn, {
    open: t,
    onOpenChange: i,
    children: [g(Ln, {
      asChild: !0,
...
const zr = ({ chips: n, max: e = 4, remainingCount: t, layout: i = "compact" }) => {
  if (i === "fill")
    return g(zs, {
      items: n,
...
function uc({ filters: n, value: e, onChange: t, height: i, width: r = 600, className: s, showApplyButton: o = !0, applyButtonLabel: a }) {
  ...
  const f = (w, S) => {
    ...
    t(d);
  }, v = B(() => i || Object.entries(n).reduce((S, [N, C]) => {
    const D = Qi(C.type);
    return Math.max(S, (D == null ? void 0 : D.formHeight) || dc);
  }, 0), [n, i]);
  return !n || Object.keys(n).length === 0 ? null : g("div", {
...
hc.displayName = "F0Heading";
const Xc = Re(
  "F0GridStack",
  Ai
), fc = ({ benefits: n }) => g("div", {
  className: "flex flex-col gap-2",
  children: n.map((e, t) => g(gc, {
    text: e
  }, t))
}), gc = ({ text: n }) => M("div", {
...
}), Pr = le(({ title: n, image: e, benefits: t, actions: i, withShadow: r = !0, module: s, moduleName: o, tag: a, promoTag: l }, u) => M("div", {
  ref: u,
...
Pr.displayName = "ProductBlankslate";
function pc({ isOpen: n, onClose: e, title: t, children: i, module: r, portalContainer: s }) {
  const [o, a] = F(n);
  return H(() => {
    a(n);
  }, [n]), g(Bs, {
...
function Yc({ isOpen: n, onClose: e, title: t, image: i, benefits: r, errorMessage: s, successMessage: o, loadingState: a, nextSteps: l, closeLabel: u, primaryAction: c, modalTitle: d, modalModule: h, secondaryAction: f, portalContainer: x, tag: v, promoTag: w, showResponseDialog: S = !0 }) {
  const [N, C] = F(n), [D, y] = F(null), [b, R] = F(!1), E = async () => {
    if (c != null && c.onClick) {
      R(!0);
      try {
...
function mc({ mediaUrl: n, title: e, description: t, onClose: i, dismissible: r, width: s, trackVisibility: o, actions: a, showConfirmation: l = !0 }) {
  const [u, c] = F(!1), d = () => {
    c(!0), i && i();
  };
  H(() => {
...
const vc = le(function({ primaryAction: e, secondaryAction: t, ...i }, r) {
...
vc.displayName = "UpsellingBanner";
function qc({ isOpen: n, setIsOpen: e, label: t, variant: i = "promote", size: r = "md", showIcon: s = !0, side: o = "right", align: a = "center", icon: l = Ks, mediaUrl: u, title: c, description: d, width: h = "300px", trackVisibility: f, actions: x, onClick: v, hideLabel: w = !1 }) {
  const [S, N] = F(!1), [C, D] = F(null), [y, b] = F(null), R = (_) => {
    e(_), v && v();
  }, E = async (_) => {
    if (_.type === "upsell") {
      b(_);
      try {
        await _.onClick(), _.showConfirmation && (N(!0), D("success"));
      } catch {
        N(!0), D("error");
      }
    }
...
const yc = Ne(null), bc = ({ children: n, fullScreen: e = !0 }) => {
  const t = I(null), [i, r] = F(t.current);
  return io(() => {
    r(t.current);
  }, []), g(yc.Provider, {
    value: {
  ...
  reducedMotion: "user",
  children: n
}), Vc = ({ children: n, layout: e, link: t, privacyModeInitiallyEnabled: i, image: r, i18n: s, l10n: o, isDev: a = !1, dataCollectionStorageHandler: l, showExperimentalWarnings: u = !1 }) => g(xc, {
  children: g(Xs, {
    isDev: a,
...
export {
  ...
  Sc as Layout,
  Pc as LineChart,
  Od as OneFilterPicker,
  Mc as PieChart,
  Zs as PrivacyModeProvider,
  Pr as ProductBlankslate,
  Ad as ProductCard,
  Yc as ProductModal,
  mc as ProductWidget,
  Ic as ProgressBarChart,
  Nc as StandardLayout,
  kd as TagCounter,
  Oc as TwoColumnLayout,
  Gn as UpsellRequestResponseDialog,
  vc as UpsellingBanner,
  $n as UpsellingButton,
  qc as UpsellingPopover,
  Lc as VerticalBarChart,
  Dc as avatarVariants,
  Td as buildTranslations,
  Gc as buttonDropdownSizes,
  Wc as buttonDropdownVariants,
  Fc as buttonSizes,
  $c as buttonToggleSizes,
  Bc as buttonVariants,
  zd as createAtlaskitDriver,
  Pd as createDataSourceDefinition,
  No as createPageLayoutBlock,
  Oo as createPageLayoutBlockGroup,
  Jc as dataCollectionLocalStorageHandler,
  Kc as datepickerSizes,
  iu as defaultTranslations,
  Re as experimental,
  Md as getAnimationVariants,
  Ld as getDataSourcePaginationType,
  Id as getEmojiLabel,
  Hd as isInfiniteScrollPagination,
  Bd as isPageBasedPagination,
  jc as linkVariants,
  Fd as modules,
  Wd as predefinedPresets,
  Gd as selectSizes,
  $d as tagDotColors,
  jd as useData,
  Ud as useDataSource,
  Kd as useDndEvents,
  Xd as useDraggable,
  Yd as useDroppableList,
  qd as useEmojiConfetti,
  Vd as useGroups,
  Jd as usePrivacyMode,
  Zd as useReducedMotion,
  Qd as useSelectable,
  eu as useXRay
};
