...
import { NewColor } from '../tags/F0TagDot/types';
import { StatusVariant } from '../tags/F0TagStatus/types';
import { DataSourceDefinition, FiltersDefinition, FiltersState, GroupingDefinition, OnSelectItemsCallback, RecordType, SelectedItemsState, SortingsDefinition } from '../../hooks/datasource';
...
type F0SelectSelectionProps<T extends string, R = unknown> = F0SelectSingleSelectionProps<T, R> | {
    clearable: true;
    multiple?: false;
    value?: T;
    defaultItem?: F0SelectItemObject<T, ResolvedRecordType<R>>;
...
type F0SelectFieldProps<T extends string, R = unknown> = F0SelectPopupProps<T, R> & F0SelectSelectionProps<T, R> & {
...
type F0SelectInlineProps<T extends string, R = unknown> = F0SelectPopupProps<T, R> & F0SelectSingleSelectionProps<T, R> & Pick<InputFieldProps<T>, "label" | "placeholder" | "disabled"> & {
    /**
 ...
 */
export type F0SelectItemMetadata = {
    type: "dialCode";
    dialCode: string;
};
...
export type F0SelectItemProps<T, R = unknown> = F0SelectItemObject<T, R> | {
    type: "separator";
};
